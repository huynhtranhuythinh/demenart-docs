...
- **F1–F7 canonical merge complete.** *(HISTORICAL — prior merge of 2026-07-17: F1/F3/F4/F6/F7 updated; F2/F5 then byte-identical to V113F-C.)* **Current state after the Final Consistency Amendment (same date, below): F2/F3/F5/F6/F7 amended; F1/F4 byte-identical to the prior merge.**
...
- **Values:** ✅ CONFIRMED & FROZEN (V113F-C) — core brand/surface/ink hexes and both fonts (Playfair Display + Be Vietnam Pro) remain locked in F2. The Final Consistency Amendment changed documentation wording only and did not alter any token name or value. Champagne `#C8AA6A` remains decorative-only; gold text remains `#806A35`. Only non-blocking micro-tuning remains. Packs 1–4 were verified to conform to F2 (no teal/mint/blue/orange drift; two fonts only).
...
## 12. V113F FINAL CONSISTENCY AMENDMENT — 2026-07-17 GMT+7

Product Owner audit after the final merge found cross-file documentation contradictions that had been preserved because F2/F5 were previously required to stay byte-identical. That restriction was superseded and one narrowly scoped consistency patch was applied. Statements of record:

- **No design decision was reopened.** Tokens, fonts, routes, product behavior, authorization, governance predicates, consent semantics, responsive strategy — all unchanged.
- **All four correction packs remain approved L3**; the pack HTML files were not altered.
- The amendment corrected **stale/cross-file documentation only**: F2 §17 stale "sole remaining blocker" paragraph (now HISTORICAL); F3 A3 cross-reference "A7" → **D1 (L3)**; F3 A1/A3/A4 gap lines normalized to "Design gap: none" + implementation-acceptance notes; Memory Room order made explicitly **viewport-specific** and consistent across F3 B1 / F5 §5 / F6 R5 (desktop: return → media → title/story → provenance → voices → preserve → lifecycle; mobile: return → media → title/story → voices → preserve → provenance → lifecycle/admin — content-first invariant on both); F5 §3 nav-label continuity corrected (desktop "Thế giới của con" / mobile "Của con"); F5 §13 ChildSwitcher aligned to the semantic button-group + `aria-pressed` grammar (listbox wording SUPERSEDED); F5 §13 TruthStateBlock semantics expanded to the Pack 4 status/alert mapping.
- **F1 and F4 remained byte-identical**; F2, F3, F5, F6 and F7 were updated.
- **No code, backend, route, schema, consent, governance or deploy work occurred.** F6 IMPLEMENTATION QA remains NOT YET RUN; no checkbox was marked PASS; evidence debt (six items, §9) unchanged.
