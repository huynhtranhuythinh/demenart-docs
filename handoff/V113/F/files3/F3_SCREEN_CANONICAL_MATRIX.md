...
- **Gap:** Design gap: none. Implementation acceptance: mobile 4th navigation label must be **"Của con"** per F6 (R1).
...
- **Canonical ref:** **Image 3 (L3).** ✅ (Ký ức tab). **Thành viên** section → see D1 (L3).
- **Gap:** Design gap: none. Implementation acceptance: the "{n} ký ức" count must render only as quiet metadata and never precede memories, per F6 (R3).
...
- **Gap:** Design gap: none. Implementation acceptance: mobile bottom-nav 4th label must be **"Của con"** per F6 (R4). *(HISTORICAL: Image 5 mobile showed the long label — SUPERSEDED by the V113F-C label lock.)*
...
- **Order (content-first, viewport-specific):**
  - **Desktop / reference composition:** ① return context ("Quay lại Ký ức · Tháng…") → ② media/memory (large player) → ③ serif title + story → ④ provenance (creator/date/location/type/mã ký ức) → ⑤ family voices (contributions, reactions, audio) → ⑥ Preserve ("Giữ vào Hành trình") → ⑦ quiet lifecycle ("Quản trị ký ức": Sửa / Lưu trữ / Xuất bản riêng / Xóa).
  - **Mobile recomposition (intentional, not a contradiction):** return → media → title/story → **voices → preserve → provenance** → lifecycle/admin.
  - **Invariants on both viewports:** return, media and title/story always stay first; no later section may move before media or title/story; governance zones, predicates and content contracts are unchanged by recomposition.
