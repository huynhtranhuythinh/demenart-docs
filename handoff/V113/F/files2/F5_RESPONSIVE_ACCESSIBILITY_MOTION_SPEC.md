...
- The same **four destinations remain in the same order** across rail (desktop) and bottom nav (mobile). Desktop 4th label = **"Thế giới của con"**; mobile 4th label = **"Của con"** (intentionally shortened). Only chrome and that shortened mobile label differ.
...
**Invariant:** the content-first invariant holds on both viewports — return, media and title/story stay first everywhere. Desktop/reference later order = provenance → voices → preserve → lifecycle; **mobile intentionally recomposes** later sections to voices → preserve → provenance → admin (as in the table above). No later section may ever move before media or title/story.
...
- ChildSwitcher: semantic **button group** — each child is a `<button type="button">`; selected child `aria-pressed="true"`, others `aria-pressed="false"`; accessible name announces child name, age and selected state ("An, 3 tuổi 2 tháng, đang chọn"). *(Aligned with Pack 2 / F3 / F4; HISTORICAL listbox/options wording SUPERSEDED.)*
...
- TruthStateBlock (aligned with Pack 4 / F4 / F6): loading = `role="status"` + `aria-live="polite"` + `aria-busy="true"`; error = `role="alert"`; denied = `role="alert"` with generic non-enumerating copy; media-unavailable = `role="alert"`; consent-waiting / consent-needed / consent-granted = `role="status"` + `aria-live="polite"`; empty and archived = `aria-labelledby`/`aria-describedby` only (no unnecessary live region). Never combine `role="alert"` with `aria-live` on the same panel.
