...
- [ ] **Desktop order:** return → media → title/story → provenance → voices → preserve → lifecycle.
- [ ] **Mobile order (intentional recomposition):** return → media → title/story → voices → preserve → provenance → lifecycle/admin.
- [ ] Return/media/title-story remain first on **both** viewports; no section moves above media; governance zones and predicates unchanged by recomposition.
