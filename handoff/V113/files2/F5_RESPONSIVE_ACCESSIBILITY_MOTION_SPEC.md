...
### Tablet — 401–1023
- Rail **collapses**: either an icon-rail or a top bar (choose one system-wide; `⚠ CONFIRM` art-director pick — default: collapse to top bar + hamburger, reuse mobile chrome scaled up).
- Two-column heroes reflow to single column; right sidebars move **below** primary content, order preserved.
- Content column widens with generous gutters; no dense mode.
...
- Check the risk pairs: `--color-ink-metadata` on ivory; `--color-ink-on-emerald-muted` on emerald; **gold text** on ivory (gold-on-light is the highest failure risk — `⚠ CONFIRM` gold text passes 4.5:1, else darken to `--color-ink-gold`).
...
| Tablet | ⚠ rail-collapse strategy needs art-director pick |
| Safe-area / bottom-nav reservation | ✅ |
| A11y (contrast/focus/SR/tap) | ✅ specified; **gold-on-ivory contrast** = single confirm item |
