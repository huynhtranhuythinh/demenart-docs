...
- [ ] **G3 Typography** — one serif (titles) + one sans (mechanics); fixed scale; Vietnamese diacritics render correctly; no per-route font change; no script font.
- [ ] **G4 Rail/Nav** — identical 4 primaries + labels; utilities behind identity; desktop rail width consistent; mobile bottom nav = 4 items.
