...
| **Curriculum** | `programs`(**TOÀN CỤC** — D40) · `age_groups` · `levels` · `themes` · `lessons` · `lesson_versions`(immutable) · **`program_distributions`** · **`program_distribution_items`** | 🔒 chưa policy (cụm kế) |
| **Idea** | `ideas` | 🔒 chưa policy |
| **Sessions** | **`class_distributions`**(✅ READ mig 011) · `lesson_sessions` · **`session_teachers`** · **`session_media`** · `session_reports` · `child_observations` | 🔒 phần lớn chưa policy |
...
> **DB tầng cấu trúc = XONG** (44 bảng mig 001–007). **RLS helpers XONG** (008). **RLS cụm Org/People XONG** (mig 011–016, nghiệm thu login thật §7). **~36 bảng còn lại CHƯA policy → khóa kín** — viết theo cụm là việc chính.
...
**Hàm SECURITY DEFINER = 22:** 16 cũ (3 trigger nền + 12 helper + `rls_auto_enable` benign) + 6 mới (`class_school_id`, `child_in_my_school`, `create_child_and_enroll`, 3 guard). **Non-definer:** `is_school_admin()`.
**RLS policy = 30** (4 cũ mig 009 + 26 mới mig 011–015).
...
**Bước kế (DB-first, §14 C):** RLS cụm **Curriculum** (programs TOÀN CỤC: admin write, mọi auth read) → Sessions → Media → Journey/Privacy → Business → Ops + test login thật → Edge Functions media → UI Chặng 2.
...
- **⭐ `/portal/rls-test` (panel nghiệm thu RLS):** 3 card — user hiện tại · count theo RLS (schools/classes/children/enrollments/profiles/child_parents) · nút "tự nâng quyền lên super_admin" (xanh = guard giữ / đỏ = lỗ hổng). Fetch client-side `useEffect`. **Dùng để chấm ma trận scope bằng login thật.**
- **✅ NGHIỆM THU phiên v3 (4 login thật):** ma trận scope đúng từng ô (xem HANDOFF_v3 §2) + 3 banner escalation xanh (parent/master/teacher). 2 phép thử linh hồn (D48 admin-0-trẻ, D40 parent-0-trường-1-trẻ) ĐẠT.
...
*Skeleton v0.4 — **RLS cụm Org/People XONG (mig 011–016), nghiệm thu bằng login thật 4 vai trò.** Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
