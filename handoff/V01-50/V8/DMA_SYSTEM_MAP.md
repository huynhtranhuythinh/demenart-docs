...
## 3. BẢN ĐỒ BẢNG (theo Tài liệu C v2 — 46 bảng · ✅ ĐÃ THI CÔNG mig 001–025 · RLS bật · chi tiết §6)
...
| **Media** | `media_assets`(Bunny-aware) · `media_variants` | 🔒 **Edge-only CỐ Ý** (deny-by-default; serving qua Edge Phase 4 — D60-D69, KHÔNG lỗ hổng) |
...
| seed | `seed_007_ops_config` | catalog 10 `notification_types` (moment_new/child_lesson_done/consent_request/...) + 12 `app_settings` (footer/contact/social/pwa/i18n + 1 internal is_public=false) + demo: 4 noti (parent 2/teacher 1/master 1) · 2 support_request · 2 audit_log (admin). Idempotent ON CONFLICT/NOT EXISTS, không replica |
...
**Hàm SECURITY DEFINER = 29:** 16 cũ (3 trigger nền + 12 helper + `rls_auto_enable` benign) + 6 v3 (`class_school_id`, `child_in_my_school`, `create_child_and_enroll`, 3 guard) + 1 v4 (`current_school_id`) + 3 v5 (`cd_school_id`, `session_school_id`, `is_session_lead`) + 3 v8 (`moment_school_id`, `is_moment_parent`, `moment_is_approved`). **Non-definer:** `is_school_admin()`.
...
**Bước kế (DB-first, §14 C):** ✅ **8/8 CỤM RLS XONG** (Org/People · Curriculum · Sessions · Journey · Privacy · Business · Ops/Config · Moments). Còn lại: **Media serving = Edge Functions Phase 4** (`get_signed_media_url`/`upload_media` + engine consent min(trường,PH) + signed Bunny URL + watermark + audit) → **UI Chặng 2** (rời panel-test, CRUD vận hành qua RPC, RPC `create_notification`/ghi audit) → tách 4 portal → deploy demenart.com. **Hoãn có theo dõi:** anti-fraud device/GPS (D70, substrate audit_logs sẵn) · DRM (V2/V3) · parent community (V2).
...
- **CHƯA làm:** Edge Functions media (Phase 4); tách 4 portal; màn CRUD vận hành + RPC `create_notification`/ghi audit (rời panel-test); publish URL công khai; **dọn row test** (`child_observations.note`/`child_journey.entry_type`/`consents.source`='WRITE-BLOCK TEST (panel)' + License nếu admin từng bấm + **`learning_moments.caption='[panel] write-block test'`** do teacher/master bấm nút tạo moment đẻ ra — xem block dọn ở handoff v8).

---

*Skeleton v0.9 — **✅ 8/8 CỤM RLS XONG: Org/People (011–016) + Curriculum (017) + Sessions (018) + Journey (019) + Privacy/Consent (020) + Business/License (021) + Ops/Config (023) + Moments (024–025), đều nghiệm thu login thật 4 vai trò.** + móng config mig 022 (notification_types + app_settings). 46 bảng · 29 hàm definer · 125 policy. Media giữ Edge-only cố ý (Phase 4). Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
