...
- **CHƯA làm:** Media serving — nhánh **ảnh trẻ** (`get_signed_media_url` gọi `media_consent_check`) + **player học liệu Portal GV** (watermark động + ẩn nút tải) + `upload_media` (slice học liệu XONG v11); tách 4 portal; publish URL công khai; **dọn:** **xóa Edge `bunny-sign-test`** (smoke-test ký không gate) · row test treo (`child_observations.note`/`child_journey.entry_type`='WRITE-BLOCK TEST (panel)' + License nếu admin từng bấm + `learning_moments.caption='[panel] write-block test'` — block dọn handoff v8; consents đã dọn v10, consents_total=4; xác nhận `drop function public._neg_test()`) · sửa file seed_007 repo (body_template bỏ "Bé " thừa — đã UPDATE live) · **lưu mig 026/027/028/029 vào repo** cùng 001–025.

---

*Skeleton v0.12 — **✅ 8/8 CỤM RLS XONG** + ✅ Consent engine (mig 026, D71) + ✅ RPC vận hành (mig 027, D72) + ✅ RPC đọc curated (mig 028, D73) + ⭐ **MEDIA SERVING slice học liệu (mig 029 D75 + Edge `get_signed_media_url`, topology 3-zone D74)** — đều nghiệm thu thật. + móng config mig 022. **46 bảng · 34 hàm definer · 125 policy · mig 001→029 · seed 001→008.** Bunny vào lại → file trần=403, signed URL phát được; member-entitled allowed / parent denied. Nhánh ảnh trẻ (consent) + player Portal GV + watermark = slice sau. **UI Chặng 2 ĐÓNG 4/4 màn:** `/portal/notifications` + `/portal/consent` + `/portal/support` + `/portal/journal`. Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
