...
> **DB tầng cấu trúc = XONG** (44 bảng mig 001–007). **RLS helpers XONG** (008). **RLS cụm Org/People XONG** (mig 011–016) **+ Curriculum** (017) **+ Sessions** (018) **+ Journey** (019) **+ Privacy/Consent** (020) **+ Business/License** (021), đều nghiệm thu login thật §7. **6/8 cụm xong. ~20 bảng còn lại CHƯA policy → khóa kín** (Moments/Media/Ops) — viết theo cụm là việc chính.
...
**RLS policy = 104** (4 mig 009 + 26 mig 011–015 + 26 mig 017 + 15 mig 018 + 15 mig 019 + 9 mig 020 + 9 mig 021).
...
**Bước kế (DB-first, §14 C):** ✅ Curriculum/Sessions/Journey/Privacy/**Business** XONG (6/8) → **còn 2 cụm RLS:** Moments · Ops; + **Media** (mỏng ở RLS, phần thịt = Edge Functions — gộp với Phase 4 Edge). → test login thật → Edge Functions media → UI Chặng 2 (rời panel-test).
...
- **⭐ `/portal/rls-test` (panel nghiệm thu RLS) — 8 mục:** user hiện tại · count Org/People · nút escalation (xanh=guard giữ) · count Curriculum + nút write-block program · **count Sessions (buổi/GV/media/báo cáo/ghi chú PII) + nút write-block observation** · **count Journey (nhật ký/kỹ năng/huy hiệu/ĐN huy hiệu/HĐ ở nhà) + nút write-block entry nhật ký** · **count Privacy (consents/privacy_requests/share_links) + nút write-block consent** · **count License (pricing_config/school_subscriptions/school_subject_entitlements) + nút write-block subscription**. Fetch client-side `useEffect`. **Dùng để chấm ma trận scope bằng login thật.**
...
- **CHƯA làm:** tách 4 portal riêng; màn CRUD vận hành (rời panel-test); publish URL công khai; **dọn row test** (`child_observations.note` / `child_journey.entry_type` / `consents.source` = 'WRITE-BLOCK TEST (panel)'; License: nút write-block đẻ row `school_subscriptions.payment_status='WRITE-BLOCK TEST (panel)'` **chỉ khi admin bấm** — master/teacher/parent bị RLS chặn nên KHÔNG đẻ row).

---

*Skeleton v0.8 — **RLS cụm Org/People (011–016) + Curriculum (017) + Sessions (018) + Journey (019) + Privacy/Consent (020) + Business/License (021) XONG, nghiệm thu login thật 4 vai trò.** 6/8 cụm RLS. 26 hàm definer · 104 policy. Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
