...
## 3. BẢN ĐỒ BẢNG (theo Tài liệu C v2 — 46 bảng · ✅ ĐÃ THI CÔNG mig 001–029 · RLS bật · chi tiết §6)
...
| **Media** | `media_assets`(Bunny-aware) · `media_variants` | 🔒 deny-by-default (Edge-only) · ⭐ **consent engine `media_consent_check` (mig 026, D71)** · ⭐ **MEDIA SERVING SỐNG (slice học liệu, v11):** gate `check_curriculum_media_access` (mig 029, D75) + Edge `get_signed_media_url` ký token theo zone. **Topology 3-zone** `dma`/`dma-learning`/`dma-private` (D74). Ảnh trẻ (consent) chờ slice sau |
...
**Edge Functions (server-side, giữ secret):** ✅ **`get_signed_media_url` (slice học liệu XONG v11 — D75; ráp ảnh trẻ qua consent engine sau)** · ⬜ `upload_media` · ⬜ `create/resolve/revoke_private_share_link` · ⬜ `request_sensitive_access`. **Throwaway chờ xóa:** `bunny-sign-test` (smoke-test ký không gate).
...
| **029** | `029_curriculum_media_gate` | **GATE HỌC LIỆU (D75)** — secdef `check_curriculum_media_access(media_id, viewer_profile)` (resolve media→lesson_version→program; gate `school_subject_entitlements` + `school_subscriptions.state ∈ active/trial` còn hạn; trả verdict + path/zone/cờ). Grant `authenticated`+`service_role`, verify D15 `[]`. + **seed inline:** 1 `media_assets` (`/Chu_Vit_Con.mp3`, `private_curriculum`, zone `dma-learning`, gắn lesson_version môn có trường entitled, idempotent NOT EXISTS). Ráp với Edge `get_signed_media_url`. KHÔNG bảng/policy mới. verify029 member-allowed/parent-denied ĐẠT |
...
**Hàm SECURITY DEFINER = 34:** 16 cũ (3 trigger nền + 12 helper + `rls_auto_enable` benign) + 6 v3 (`class_school_id`, `child_in_my_school`, `create_child_and_enroll`, 3 guard) + 1 v4 (`current_school_id`) + 3 v5 (`cd_school_id`, `session_school_id`, `is_session_lead`) + 3 v8 (`moment_school_id`, `is_moment_parent`, `moment_is_approved`) + 3 v9 (`media_consent_check` — grant authenticated; `create_notification`, `write_audit_log` — grant CHỈ service_role) + 1 v10 (`get_child_journal` — grant authenticated; RPC đọc curated cho PH, D73) + **1 v11 (`check_curriculum_media_access` — grant authenticated+service_role; gate học liệu entitlement, D75)**. **Non-definer:** `is_school_admin()`. **Edge Functions (KHÔNG phải hàm Postgres):** `get_signed_media_url` (sống, slice học liệu).
...
**Bước kế (DB-first, §14 C):** ✅ **8/8 CỤM RLS XONG** (Org/People · Curriculum · Sessions · Journey · Privacy · Business · Ops/Config · Moments). ✅ **Consent engine XONG (mig 026, D71)** + ✅ **RPC vận hành XONG (mig 027, D72)** + ✅ **RPC đọc curated XONG (mig 028, D73)**. ✅ **UI Chặng 2 ĐÓNG 4/4 màn Bunny-độc-lập** (notifications · consent · support · journal). Còn lại: **Media serving — Bunny ĐÃ VÀO LẠI (v11):** ✅ slice học liệu XONG (`get_signed_media_url` + gate entitlement D75 + ký token 3-zone D74 + audit). **Kế tiếp:** player học liệu trong Portal GV (nút phát + watermark động + ẩn nút tải; login GV thật = lớp 4) → ráp nhánh **ảnh trẻ** (`get_signed_media_url` gọi `media_consent_check` D71, thay placeholder "Hình ảnh sẽ sớm có" ở journal) → `upload_media` → tách 4 portal → deploy demenart.com. **Hoãn có theo dõi:** anti-fraud device/GPS (D70, substrate audit_logs sẵn) · DRM (V2/V3) · parent community (V2).
...
- **CHƯA làm:** Media serving — **player học liệu Portal GV** (watermark động + ẩn nút tải, lớp 4 học liệu) + `upload_media` (đường nhập liệu thật); tách 4 portal; publish URL công khai; **dọn:** **xóa Edge `bunny-sign-test`** (smoke-test ký không gate) · caption "[seed]" còn ở moment thật trong DB (UI đã strip client) · row test treo (`child_observations.note`/`child_journey.entry_type`='WRITE-BLOCK TEST (panel)' + License nếu admin từng bấm + `learning_moments.caption='[panel] write-block test'`; consents đã dọn v10; xác nhận `drop function public._neg_test()`) · sửa file seed_007 repo (body_template bỏ "Bé " thừa — đã UPDATE live) · **lưu mig 026/027/028/029/030 vào repo** cùng 001–025.

---

*Skeleton v0.13 — **✅ 8/8 CỤM RLS XONG** + ✅ Consent engine (mig 026, D71) + ✅ RPC vận hành (mig 027, D72) + ✅ RPC đọc curated (mig 028, D73) + ⭐ **MEDIA SERVING 2 nhánh SỐNG: học liệu (mig 029 D75) + ẢNH TRẺ (mig 030, consent D71) qua Edge `get_signed_media_url` route-theo-cột-link, topology 3-zone D74** — đều nghiệm thu login thật. + móng config mig 022. **46 bảng · 34 hàm definer · 125 policy · mig 001→030 · seed 001→008.** Bunny: file trần=403 cả `dma-learning` lẫn `dma-private`; signed URL phát được nhạc (học liệu) + ảnh (trẻ); member-entitled allowed / parent-consent-min gác đúng. Player Portal GV (watermark động) + `upload_media` = slice sau. **UI Chặng 2 ĐÓNG 4/4 màn:** `/portal/notifications` + `/portal/consent` + `/portal/support` + `/portal/journal` (giờ có **ảnh thật** + fallback consent mềm). Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
