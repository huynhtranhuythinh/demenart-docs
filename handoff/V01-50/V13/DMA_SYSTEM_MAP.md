...
**Hàm SECURITY DEFINER = 36:** 16 cũ (3 trigger nền + 12 helper + `rls_auto_enable` benign) + 6 v3 (`class_school_id`, `child_in_my_school`, `create_child_and_enroll`, 3 guard) + 1 v4 (`current_school_id`) + 3 v5 (`cd_school_id`, `session_school_id`, `is_session_lead`) + 3 v8 (`moment_school_id`, `is_moment_parent`, `moment_is_approved`) + 3 v9 (`media_consent_check` — grant authenticated; `create_notification`, `write_audit_log` — grant CHỈ service_role) + 1 v10 (`get_child_journal` — grant authenticated; D73) + 1 v11 (`check_curriculum_media_access` — grant authenticated+service_role; D75) + **2 v13 (`list_curriculum_media` — grant authenticated, RPC đọc track entitled D75; `check_media_upload_access` — grant authenticated+service_role, gate upload D77)**. *(mig 030 KHÔNG thêm hàm — chỉ +cột `linked_moment_id` + CREATE OR REPLACE `get_child_journal`.)* **Non-definer:** `is_school_admin()`. **Edge Functions (KHÔNG phải hàm Postgres):** `get_signed_media_url` (2 nhánh: học liệu + ảnh trẻ) · `upload_media` (ghi ảnh trẻ, v13).
...
- **CHƯA làm:** **B2 upload học liệu** zone `dma-learning` (người upload = admin, cùng Edge `upload_media` đổi zone+gate) · **group-moment ≥2 trẻ** (MIN-multi-child LIVE) · tách 4 portal · publish URL công khai; **dọn:** **xóa Edge `bunny-sign-test`** (smoke-test ký không gate) · **ảnh test DMWS móc nhầm moment "vẽ tranh mùa xuân" Jenny** (xóa media_assets row + object Bunny — GV vừa upload screenshot test) · caption "[seed]" còn ở moment thật trong DB (UI đã strip client) · row test treo (`child_observations.note`/`child_journey.entry_type`='WRITE-BLOCK TEST (panel)' + License nếu admin từng bấm + `learning_moments.caption='[panel] write-block test'`; xác nhận `drop function public._neg_test()`) · sửa file seed_007 repo (body_template bỏ "Bé " thừa — đã UPDATE live) · **lưu mig 026–032 vào repo** cùng 001–025.

---

*Skeleton v0.14 — **✅ 8/8 CỤM RLS XONG** + ✅ Consent engine (mig 026, D71) + RPC vận hành (mig 027, D72) + RPC đọc curated (mig 028, D73) + ⭐ **MEDIA ĐỐI XỨNG HOÀN CHỈNH:** ĐỌC — học liệu (mig 029 D75 + player Portal GV mig 031) + ẢNH TRẺ (mig 030 D71, journal có ảnh thật) qua Edge `get_signed_media_url` route-theo-cột-link; GHI — ẢNH TRẺ upload (mig 032 D77 + Edge `upload_media` PUT Bunny Storage). Topology 3-zone D74; storage-key≠token-key per-zone D77 — đều nghiệm thu login thật. + móng config mig 022. **46 bảng · 36 hàm definer · 125 policy · mig 001→032 · seed 001→008.** Bunny: file trần=403 cả `dma-learning` lẫn `dma-private`; signed URL phát nhạc (học liệu+watermark) + ảnh (trẻ); upload GV→Storage→preview ngay; PH gác đúng (player rỗng / upload chặn). **UI Chặng 2 + Media UI:** `/portal/{notifications,consent,support,journal,curriculum,moments}`. Kế: B2 upload học liệu (admin) · group-moment · tách 4 portal · deploy. Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
