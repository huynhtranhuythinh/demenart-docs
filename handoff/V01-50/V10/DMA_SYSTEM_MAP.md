...
## 3. BẢN ĐỒ BẢNG (theo Tài liệu C v2 — 46 bảng · ✅ ĐÃ THI CÔNG mig 001–028 · RLS bật · chi tiết §6)
...
| **Media** | `media_assets`(Bunny-aware) · `media_variants` | 🔒 **Edge-only CỐ Ý** (deny-by-default; serving qua Edge Phase 4 — D60-D69, KHÔNG lỗ hổng) · ⭐ **consent engine `media_consent_check` XONG (mig 026, D71)** — chờ Edge ráp ký Bunny |
...
**Hàm SECURITY DEFINER = 33:** 16 cũ (3 trigger nền + 12 helper + `rls_auto_enable` benign) + 6 v3 (`class_school_id`, `child_in_my_school`, `create_child_and_enroll`, 3 guard) + 1 v4 (`current_school_id`) + 3 v5 (`cd_school_id`, `session_school_id`, `is_session_lead`) + 3 v8 (`moment_school_id`, `is_moment_parent`, `moment_is_approved`) + 3 v9 (`media_consent_check` — grant authenticated; `create_notification`, `write_audit_log` — grant CHỈ service_role) + **1 v10 (`get_child_journal` — grant authenticated; RPC đọc curated cho PH, D73)**. **Non-definer:** `is_school_admin()`.
...
**Bước kế (DB-first, §14 C):** ✅ **8/8 CỤM RLS XONG** (Org/People · Curriculum · Sessions · Journey · Privacy · Business · Ops/Config · Moments). ✅ **Consent engine XONG (mig 026, D71)** + ✅ **RPC vận hành XONG (mig 027, D72)** + ✅ **RPC đọc curated XONG (mig 028, D73)**. ✅ **UI Chặng 2 ĐÓNG 4/4 màn Bunny-độc-lập** (notifications · consent · support · journal). Còn lại: **Media serving = Edge Functions Phase 4** (`get_signed_media_url` ráp `media_consent_check` + ký Bunny URL + watermark + audit; `upload_media`) — **chờ Bunny vào lại** (Bunny vẫn tịt, nghi mạng VN) → tách 4 portal khi mỗi portal có nội dung → deploy demenart.com. **Hoãn có theo dõi:** anti-fraud device/GPS (D70, substrate audit_logs sẵn) · DRM (V2/V3) · parent community (V2).
...
- **CHƯA làm:** Edge Functions media serving (Phase 4 — `get_signed_media_url` ráp consent engine + ký Bunny, chờ Bunny vào lại); tách 4 portal; publish URL công khai; **dọn row test còn treo** (`child_observations.note`/`child_journey.entry_type`='WRITE-BLOCK TEST (panel)' + License nếu admin từng bấm + `learning_moments.caption='[panel] write-block test'` — xem block dọn handoff v8; **consents đã dọn v10** (consents_total=4); **xác nhận đã `drop function public._neg_test()`** sau test consent v9); **sửa file seed_007 trong repo** (body_template bỏ "Bé " thừa — đã UPDATE live); **lưu mig 026/027/028 vào repo** cùng 001–025.

---

*Skeleton v0.11 — **✅ 8/8 CỤM RLS XONG** (011–016 + 017 + 018 + 019 + 020 + 021 + 023 + 024–025) **+ ✅ Consent engine (mig 026, D71) + ✅ RPC vận hành (mig 027, D72) + ✅ RPC đọc curated (mig 028, D73)**, đều nghiệm thu thật. + móng config mig 022. **46 bảng · 33 hàm definer · 125 policy · mig 001→028 · seed 001→008.** Media giữ Edge-only cố ý — consent engine đã dựng, chờ Bunny ráp ký URL (Phase 4). **UI Chặng 2 ĐÓNG 4/4 màn Bunny-độc-lập:** `/portal/notifications` + `/portal/consent` + `/portal/support` + `/portal/journal` (trái tim DMA). Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
