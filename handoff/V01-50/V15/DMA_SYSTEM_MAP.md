...
*Skeleton v0.16 — **✅ 8/8 CỤM RLS XONG** + ✅ Consent engine (mig 026, D71) + RPC vận hành (mig 027, D72) + RPC đọc curated (mig 028, D73) + ⭐ **MEDIA ĐỐI XỨNG HOÀN CHỈNH 2 ZONE × 2 CHIỀU** (đọc/ghi học liệu+ảnh trẻ qua Edge `get_signed_media_url`+`upload_media`; topology 3-zone D74; storage-key≠token-key D77) + ⭐ **MIN-multi-child LIVE** (seed_009) + móng config mig 022. + ⭐ **DEPLOY LIVE (v15, D84):** app Lovable TanStack Start → GitHub `huynhtranhuythinh/demenart` → **Cloudflare Pages** → `https://demenart.com`+`www` (Active+SSL); fix 404 = `nitro:{preset:"cloudflare-pages"}` (Lovable plugin tắt nitro ngoài sandbox → thiếu `_worker.js`); DNS chuyển NS GoDaddy→Cloudflare (aria/armfazh.ns.cloudflare.com) **giữ email** `info@demenart.com` (9 record DNS-only — test nhận OK); Supabase Auth Site URL+Redirect cho domain mới. Dọn §4: xóa Edge `bunny-sign-test` + row media trùng (ảnh "Vịt Con"/`jenny_buoi1.jpg` = ảnh Jenny HỢP LỆ, GIỮ). **46 bảng · 38 hàm definer · 125 policy · mig 001→034 · seed 001→009 — KHÔNG đổi (v15 phiên hạ tầng thuần).** **UI Chặng 2 + Media UI:** `/portal/{notifications,consent,support,journal,curriculum,curriculum-admin,moments}`. Việc treo: SPF kép cần gộp, lưu mig repo. Kế: onboard data trường thật · `request_sensitive_access` · share link PH ngoài. Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge/policy mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
