...
> **DB tầng cấu trúc = XONG.** 44 bảng (migration 001–007), chạy sạch trên Supabase project **`dma`** (region Singapore). Toàn bộ **idempotent**.
> **⚠️ RLS = ĐÃ BẬT trên cả 44 bảng nhưng CHƯA có policy** → đang khóa kín, app/anon đọc ra rỗng. **Viết policy là tầng KẾ TIẾP — RLS CHƯA xong.**
...
**Bước kế (DB-first, §14 C):** RLS helpers (`current_profile` · `current_role` · `has_permission` · `is_admin` · `same_school` · `is_distribution_lead` · `is_session_teacher` · `user_class_ids` · `is_child_parent` · `has_subject_entitlement` · `has_active_seat`) → policies từng bảng → Edge Functions media → Lovable (shell + registry).

---

*Skeleton v0.2 — **DB cấu trúc XONG (44 bảng, mig 001–007); RLS policy = bước kế.** Nguồn: Tài liệu A–G UPDATED + tầm nhìn founder + DMWS v170. Cập nhật "tới đâu ghi tới đó". Khi thêm bảng/module/Edge Function mới → thêm dòng + cập nhật Trung Tâm Tra Cứu cùng nhịp (KỶ LUẬT VÀNG).*
