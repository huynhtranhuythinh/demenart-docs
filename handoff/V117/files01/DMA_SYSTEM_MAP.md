# 🗺️ DMA_SYSTEM_MAP.md — BẢN ĐỒ KIẾN TRÚC (skeleton V1 · v1.21)
...
## ⭐ CURRENT CANONICAL ENDPOINT — D333 / SYSTEM_MAP v1.21 / HANDOFF V117-M1 (Parent Outcome Loop — "Sau buổi học" CLOSED, 2026-07-27)
...
- **Frontend HEAD (accepted tip):** `faa03708` (V117-M1 · Parent session outcome module — 1 commit sau `b77697b0`). Lineage sau E3-MC: A115 Teacher Home V1 (`d6f3fb54`→`7afdbf2d`→`d36b57ce` pin resolvers 5.2.2→`dd3a49b6`→`96e9d9e4`) → V116 Session S0–S4 (`18db884c` npm lockfile → `d6a9cfff` S1 shell → `e3434bb4` S2 Chuẩn bị → `41d6732e` S3 Trong giờ học → `a1016721` S4 Ghi nhận + Hoàn tất → `b77697b0` QA-fix D330). Cloudflare xanh toàn chuỗi; Owner runtime QA PASS S0–S3 (video end-to-end) + S4 12/12 (submit thật thành công, authority 4/4 actor, 320px không vỡ).
- **Migration:** registry **116** · mới nhất `v117_parent_session_outcomes` (27/07/2026, 3-block D92 + VERIFY impersonation D333, PASS atomic) · re-pin 27/07: delta đúng expected, zero drift ngoài delta.
- **Inventory sống (27/07/2026):** **88 tables · 211 functions · 200 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · registry 116 · routes 52 · 16 Edge Functions.**
