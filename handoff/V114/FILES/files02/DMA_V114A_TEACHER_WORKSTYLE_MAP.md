...
**3 route Teacher tồn tại nhưng KHÔNG có trong nav:**
- `/teacher/moments` — **route mồ côi**, không đường vào từ shell, không đường vào từ Home, không đường vào từ session flow;
- `/teacher/classroom` — chỉ mở qua `window.open` từ Bước 2 (đúng thiết kế, màn TV);
- `/teacher/session/$id` — chỉ tới qua điều hướng, không nav (đúng thiết kế).

→ `/teacher/moments` là bề mặt chết ở tầng IA. Ghi nhận defect.
...
| 17. Phân loại tái dùng | **READY NOW** (chức năng) · **BLOCKED BY SEC1A** (Remote) · **BLOCKED BY SEC1B** (ranh giới đọc) |
...
| 8. **Điều kiện ẩn — nghiêm trọng** | `attendance_pending` chỉ đếm buổi **hôm nay** ***và*** state ∈ `('in_progress','taught_report_pending')`. Nghĩa là: buổi **chưa bấm "Vào dạy"** (`scheduled`) mà thiếu điểm danh → **không được đếm**; buổi **hôm qua** thiếu điểm danh → **không được đếm**. Trong khi đó `journal_pending` ở ngay dòng dưới lại đếm **toàn thời gian, không giới hạn ngày**. Hai dòng cạnh nhau dưới một tiêu đề **"Việc cần làm hôm nay"** chạy trên **hai khung thời gian khác nhau** — và không dòng nào nói ra |
...
**Ý nghĩa cho V114:** UI hiện tại **không dựng cửa** tới buổi của lớp khác — nên đây **không phải** lỗ hổng đang bị khai thác trong luồng bình thường. Nhưng nó có nghĩa là:

1. `Teacher Toàn bộ` **tuyệt đối không được** dựng trên bốn RPC vào-buổi này ở phạm vi hiện tại;
2. bất kỳ tính năng nào sinh ra **danh sách buổi rộng hơn phân công** (ví dụ "buổi của tổ chuyên môn", "buổi tôi dạy thay") sẽ **biến khiếm khuyết tiềm ẩn thành khiếm khuyết đang hoạt động**;
...
| **V114A-P1-11** | **P1** | **`TodoSection` trộn hai khung thời gian dưới một tiêu đề.** `attendance_pending` = chỉ hôm nay + chỉ state `in_progress`/`taught_report_pending`; `journal_pending` = **toàn thời gian**. Buổi `scheduled` thiếu điểm danh và buổi hôm qua thiếu điểm danh **đều không được đếm**. Tiêu đề vẫn là "Việc cần làm hôm nay" | ❌ MỞ |
...
| V114A-P2-7 | P2 | **`/teacher/moments` là route mồ côi** — không có trong nav shell, không có đường vào từ Home hay session flow | ❌ mở |
...
| Màn chiếu / TV | **READY NOW** (chức năng) |
| Remote / điều khiển điện thoại | **BLOCKED BY SEC1A** — và phải **gỡ hoặc nói thật cửa vào** trước đó |
