...
| P-13 | Khoảnh khắc + media đã ký | card + `/school/moments` | `get_school_moments` + Edge ký | same-school + **consent MIN** | **`get_signed_media_url`** | ✅ đúng | **FUNCTIONALLY REUSABLE** | đưa `/school/moments` vào nav | — | P2 (`P2-13`) | E3 | Edge #1 |
...
| T-11 | Phát / Màn chiếu TV | `teacher.classroom` | `get_lesson_guide` + Edge ký | — | **`get_signed_media_url`** | ✅ UX tốt | **FUNCTIONALLY REUSABLE** | — | — | P2 | — | Edge #1 |
...
| T-13 | Remote / điều khiển điện thoại | 3 nút vẫn dựng | `mint_session_remote_code` = **thân rỗng** | — | `capture_session_media` 503 | ❌ **nói dối + mời thử lại** | **BLOCKED** | gỡ/ẩn/nói thật **ngay**; năng lực chờ SEC1A | **SEC1A** | **P1** (`P1-9`) | **R** → S1 | **trước QA/demo Teacher kế** |
| T-14 | Học liệu của tôi (Drive) | `/teacher/media` | `drive_my_zone` | ngăn cá nhân | **`get_signed_media_url`**, `upload_media` | ✅ | **FUNCTIONALLY REUSABLE** | re-home vào Toàn bộ | — | P2 | E4 | Edge #1·#2 |
...
| S-10 | **Ghép thiết bị Kid** | mã 6 chữ số, **không đếm lần thử, không khoá**, Edge chỉ `sleep(300)` | **Edge #8 — phát hiện mới** | **P1** | **SEC1A** | chặn phát hành Kid |
| S-11 | Ghi công người thực hiện | `actor_id`/`uploaded_by` = **người gọi thật** ở mọi hàm còn hoạt động | Edge #1·2·3·4·12·13 | ✅ **PASS** | — | — |
| S-12 | Audit logging | `write_audit_log` ghi **cả cho phép lẫn từ chối**, không ghi mật khẩu/token | toàn bộ Edge | ✅ **PASS** | — | — |
...
**Kết luận nền:** `verify_jwt:false` ở 15/16 hàm **không phải khiếm khuyết** — 13 hàm tự xác thực bằng `auth.getUser()` và fail-closed 401; 2 hàm public là **cố ý** (luồng invite-only và share-link), có gate token riêng.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | `get_signed_media_url` | 23 | off | mọi portal | `getUser` → 401 | 6 nhánh gate riêng | entitlement · consent · guardian · space · **same-school** | `media_id` — **có kiểm** | đọc + audit | không | ✅ ảnh trẻ | `prof.id` | closed | **P1 — SEC1B** | SEC1B |
...
| 8 | `kid_gate` | 8 | off | Cổng Kid | device token + **PIN bcrypt** | RPC service-only | theo bé | session token — có kiểm | ghi + ký + storage | DB + Bunny | ✅ **album trẻ** | — | closed | 🔴 **P1 — SEC1A** | **SEC1A** |
...
| 10 | `capture_session_media` | 4 | **on** | — | — | — | — | **không parse body** | **không** | **không** | không | — | closed | **PASS** *(SEC0 — 503)* | SEC1A *(khôi phục)* |
...
**Chủ:** SEC1A — đúng phạm vi "pairing challenge · rate-limit · TTL · capability token". **Mở rộng phạm vi SEC1A:** trước đây chỉ nhắm Remote của GV; nay **phải bao gồm cả ghép thiết bị Cổng Kid**.

### 🔴 EDGE-P1-B — nhánh `private_school_resource` của `get_signed_media_url`

Gate chỉ là `prof.school_id === media.linked_school_id`. Đây là **bản sao ở tầng Edge của `V114A-P1-5`**: media buổi học của **mọi lớp trong trường** đều ký được bởi **mọi nhân sự trong trường**, không cần phân công.

Năm nhánh còn lại **đúng chuẩn** (entitlement · consent MIN · guardian · family card · family contribution). **Không tạo ID mới** — đây là cùng một khiếm khuyết, nay có thêm bằng chứng ở tầng Edge. **Chủ: SEC1B.**
...
| Năng lực Remote (T-13) | SEC1A |
| **Ghép thiết bị Cổng Kid (S-10)** | **SEC1A — phạm vi vừa mở rộng** |
...
| 6 | Phạm vi SEC1A cập nhật (thêm ghép thiết bị Kid) | ✅ ghi ở A.3 |

**Phán quyết:** **V114A ĐƯỢC PHÉP TIẾN TỚI CLOSEOUT** — và closeout sẽ là **PASS WITH BLOCKERS**, không phải PASS. Điều kiện 5 một mình đã đủ để loại bỏ khả năng SEALED sạch.
