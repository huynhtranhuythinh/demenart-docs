...
-- REVISION      : rev3 (Option C+ — full non-SELECT client lockdown, incl. MAINTAIN)
-- =====================================================================================
...
--             S0A-FINDING-02 RESOLVED: MAINTAIN included in the revoke set.
--
-- Scope     : REVOKE INSERT, UPDATE, DELETE, TRUNCATE, TRIGGER, REFERENCES, MAINTAIN
...
--   (MAINTAIN is NO LONGER retained — S0A-FINDING-02 resolved under Option C+.
--    No MAINTAIN debt remains on public.session_teachers.)
--
...
--     AFTER   postgres=arwdDxtm/postgres, anon=r/postgres,
--             authenticated=r/postgres, service_role=r/postgres
--     (r = SELECT, retained by design for the compatibility window)
--     21 direct ACL entries removed: 7 privileges x 3 client roles.
-- =====================================================================================
...
begin
  ...
  -- All seven target privileges must currently be present for all three roles. If any is
  ...
  foreach r in array array['authenticated','anon','service_role'] loop
    foreach p in array array['INSERT','UPDATE','DELETE','TRUNCATE','TRIGGER','REFERENCES','MAINTAIN'] loop
...
-- -------------------------------------------------------------------------------------
-- Table-level revocation of all seven approved privileges. Per D310-cand, table-level
...
declare
  ...
  v_probe_ok     boolean;
begin
  ---- 3.1 PRIMARY: direct ACL inspection — no entry for any of the seven privileges --------
  ...
  ---- 3.2 PRIMARY: PUBLIC must hold none of the seven ----------------------------------------
  ...
  ---- 3.4 corroborating: table-level checks, seven privileges x three roles ---------------------
  foreach r in array array['authenticated','anon','service_role'] loop
    foreach p in array array['INSERT','UPDATE','DELETE','TRUNCATE','TRIGGER','REFERENCES','MAINTAIN'] loop
  ...
  -- of the revoked privileges. MAINTAIN is deliberately EXCLUDED: it is not a column-level
  -- privilege in PostgreSQL, so has_column_privilege would be meaningless for it.
  ...
  --
  --   * NO functional MAINTAIN probe is run. Executing VACUUM/ANALYZE/REINDEX purely to
  --     probe would create side effects and needless load. MAINTAIN removal is proven by
  --     structural ACL evidence (3.1/3.2/3.4) ALONE, and no functional claim is made for it.
  --
  ...
  raise notice 'S0A BLOCK 3 OK — 7 privileges revoked for authenticated+anon+service_role = 21 direct ACL entries removed (ACL-verified, PRIMARY evidence)';
  raise notice 'S0A BLOCK 3 OK — column-level residue: none across 5 user columns x 3 roles x (INSERT,UPDATE,REFERENCES)';
  raise notice 'S0A BLOCK 3 OK — supplementary probes: TRUNCATE 3/3 = 42501; INSERT service_role = 42501. INSERT probe NOT run for authenticated/anon (RLS makes 42501 ambiguous); no MAINTAIN probe by design';
  raise notice 'S0A BLOCK 3 OK — preserved: rows=2 policies=4 constraints=5 indexes=2 definer_rpcs=5; SELECT retained';
  raise notice 'S0A BLOCK 3 OK — S0A-FINDING-02 RESOLVED: MAINTAIN revoked; no MAINTAIN debt remains on session_teachers';
end
$block3$;
