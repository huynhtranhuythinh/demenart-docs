# 🗺️ DMA_SYSTEM_MAP.md — BẢN ĐỒ KIẾN TRÚC (skeleton V1 · v1.14)
...
## ⭐ CURRENT CANONICAL ENDPOINT — D324 / SYSTEM_MAP v1.16 / HANDOFF V114B-E3-WP4-S3A (S3A closeout 2026-07-24)
...
> **Lineage E3:** track V114B-E3 (WP1→WP4) dùng D-rule *candidate* (D310–D323-cand + D-A2-1) sống trong các file closeout WP; chỉ **E3 milestone closeout** (sau S4) mới canonicalize chúng. S3A nâng **một** quyết định governance bền vững thành **D324** (RULES).
...
- **Frontend HEAD:** `d8178a55` (KHÔNG đổi — S3A là backend-only).
- **Migration:** registry **114** · mới nhất `20260724092921 v114b_e3_wp4_s3_authority_cutover` · apply **2026-07-24 16:29:21 ICT**.
- **Inventory sống (24/07/2026):** **88 tables · 210 functions · 199 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · registry 114 · routes 52 · 16 Edge Functions.**
- **⭐ Authority surface (LIVE, D324):** quyền hành động session-scoped (submit journal) đi theo **session responsibility hiện hành** (`session_teacher_assignments` dimension `responsible`), **KHÔNG** theo class lead hiện tại (không hồi tố lead). Cutover đã live: `submit_session_journal` (6 bước gate, KHÔNG fallback planned/taught_by/lead/admin) · `start_session` (birth-path tạo đúng 1 responsible `runtime_start_session`, audit coupled) · `get_session_detail` (`can_submit_journal`·`submit_block_reason`·`responsible_teacher`). `is_session_lead` byte-nguyên (md5 `8b4f91dd…`). **owner_attested ≠ db_proven.**
- **STA:** 13 total · 9 planned · 4 responsible (4 current) · evidence **db_proven 3 / owner_attested 1** · runtime residue 0. Audit anchors: `session_responsibility_backfilled` 4 · malformed historical `session_started` 3 · baseline observed 11056.
- **🔒 Skew window S3→S4: MỞ từ 2026-07-24 16:29:21 ICT** — CẤM đổi `class_distributions.lead_teacher_id` cho tới khi S4 deploy production.
- **Frontend mismatch (chủ đích, tới S4):** UI vẫn suy affordance journal từ legacy lead state; `session_reports` **CHƯA** nằm trong responsibility containment.
- **Parent shell:** mobile **≤479** · tablet **480–1023** · desktop **≥1024**.
- **Trạng thái:** **V114B-E3-WP4-S3A SEALED (authority cutover LIVE)** · **kế: S4 — frontend/product-surface alignment (CHƯA mở).**
