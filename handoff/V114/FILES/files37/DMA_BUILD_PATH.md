...
## 🧭 V114B-E3 EXECUTION STATE (cập nhật 2026-07-24 — WP4-S3A SEALED)

> Nhánh E3 (Chặng 4/6 hardening + authority): thu quyền ghi + cutover authority journal sang **session responsibility**. Track dùng D-rule *candidate* (D310–D323-cand + D-A2-1) sống trong file closeout WP; **E3 milestone closeout** (sau S4) mới canonicalize.

**Đã đóng:**
- **WP1** — attribution containment scaffolding (mig 105–106; D310/311/312-cand).
- **WP2/S2** — responsible backfill (STA dimension `responsible`; db_proven 3 / owner_attested 1).
- **WP3** — session write-path containment: `revoke insert/update/delete/truncate on lesson_sessions from authenticated, anon` (mig 111; E3-SG-02 CONTAINED; D314/323-cand + D-A2-1).
- **WP4-S3 / S3A** — ⭐ **AUTHORITY CUTOVER LIVE** (mig 114 `v114b_e3_wp4_s3_authority_cutover`, apply 2026-07-24 16:29:21 ICT; **D324** canonical). submit/start/detail cutover sang session responsibility; `is_session_lead` byte-nguyên. Gate: exact-artifact rehearsal 100% → apply once (atomic) → Layer B → QA Q01–Q15 → Layer B — zero residue. Inventory 88/210/199/166/33 · registry 114 · 0 frontend/route/policy/Edge đổi.

**🔒 Skew window S3→S4 — MỞ 2026-07-24 16:29:21 ICT:** CẤM đổi `class_distributions.lead_teacher_id` cho tới khi S4 deploy production.

**Kế → S4 (frontend/product-surface alignment · CHƯA mở · chưa được authorize bởi closeout này). S4 entry contract:**
- Thay affordance submit journal suy-từ-legacy-lead bằng **server capability**.
- Consume: `can_submit_journal` · `submit_block_reason` · `responsible_teacher` (từ `get_session_detail`).
- UI KHÔNG được tự suy authority từ class lead; server là nguồn sự thật.
- Test **explicit** S3→S4 skew window.
- Deploy **trước** khi cho phép bất kỳ mutation `lead_teacher_id` nào.
- KHÔNG mở rộng sang responsibility-transfer workflow trừ khi được authorize riêng.
- KHÔNG containment `session_reports` trừ khi được authorize riêng.

**Residual risks (giữ, KHÔNG đóng):** frontend legacy-lead tới S4 · `session_reports` uncontained · responsibility-transfer RPC vắng · E3-SG-01 OPEN · E3-SG-02 CONTAINED · R21 ACTIVE · `pg_default_acl` debt · sub_admin QA debt · Vũ Hoàng Nam + Trần Khánh Vy auth-persona debt.

**Ghi chú thực thi (implementation evidence — KHÔNG phải rule, theo CTO S3A):** artifact canonical regen từ byte-exact `pg_get_functiondef` của body đã rehearse. md5 pinned: submit `29623e1c…` (khớp accepted) · start `48e62a76…` (khớp accepted) · **detail `91a08743…`** (re-pinned: bằng bản compact đã rehearse, semantic-identical với body accepted; giá trị abbreviated/historical `37a3b4a1…` trong rehearsal report đầu byte không khôi phục được sau rollback). Chi tiết serialization/whitespace/md5 KHÔNG được nâng thành D-rule (chỉ ghi ở đây + HANDOFF).
