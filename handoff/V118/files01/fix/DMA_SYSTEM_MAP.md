# 🗺️ DMA_SYSTEM_MAP.md — BẢN ĐỒ KIẾN TRÚC (skeleton V1 · v1.23)
...
## ⭐ CURRENT CANONICAL ENDPOINT — D335 / SYSTEM_MAP v1.23 / HANDOFF V118-M1 (Parent–Teacher Appreciation "Lời cảm ơn" CLOSED, 2026-07-27)
...
- **Frontend HEAD (accepted tip):** `df958fc6` (V118-M1 · "Restored routeTree.gen.ts" — tip sau chuỗi `ade28d85` feature → `fa824baf` a11y → `5a1e3bfa` visual r1 → `48c62b32` love-wall → `3ec70199` grid ⚠️ kèm routeTree drift → `df958fc6` restore byte-exact; net routeTree zero-diff). Trước đó V117-M2: `e20bffa4` ("Styled Today block gradient" — QA-fix visual sau `f8a94d14` "Added school today dashboard", nối `faa03708`). Lineage sau E3-MC: A115 Teacher Home V1 (`d6f3fb54`→`7afdbf2d`→`d36b57ce` pin resolvers 5.2.2→`dd3a49b6`→`96e9d9e4`) → V116 Session S0–S4 (`18db884c` npm lockfile → `d6a9cfff` S1 shell → `e3434bb4` S2 Chuẩn bị → `41d6732e` S3 Trong giờ học → `a1016721` S4 Ghi nhận + Hoàn tất → `b77697b0` QA-fix D330). Cloudflare xanh toàn chuỗi; Owner runtime QA PASS S0–S3 (video end-to-end) + S4 12/12 (submit thật thành công, authority 4/4 actor, 320px không vỡ).
- **Migration:** registry **118** · mới nhất `v118_parent_teacher_appreciation` (27/07/2026, 3-block D92 + VERIFY 10 assertion groups impersonation D333, PASS atomic; bảng `session_appreciations` deny-all RLS + 3 RPC theo D335) · trước đó `v117_school_today_operations` (27/07/2026, 3-block D92 + VERIFY 20 assertion impersonation D333, PASS atomic) · trước đó `v117_parent_session_outcomes` (27/07/2026, 3-block D92 + VERIFY impersonation D333, PASS atomic) · re-pin 27/07: delta đúng expected, zero drift ngoài delta.
- **Inventory sống (27/07/2026 · sau V118-M1):** **89 tables · 214 functions · 203 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · registry 118 · routes 52 · 16 Edge Functions.**
...
- **Trạng thái:** **V118-M1 CLOSED (27/07/2026)** — V117-M2 CLOSED cùng ngày — V117-M1 CLOSED cùng ngày · V116 S0–S4 CLOSED (26/07) · sprint kế (candidates): **M2 "Teacher acknowledgement"** (cần CTO mở contract) · **V117-M3 Parent outcome mở rộng** · **G.4+ restyle** · **V114-SEC1** · **FMN E2E fixture session** · **Day-state semantics v2**. P2 carry: day-state semantics v2 khi có dữ liệu vận hành thật · 768px nav shell · nav lạc `/teacher/profile`→`/school/manage` · sticky CTA với bàn phím · residual E3-MC carry-forward xem RULES.
