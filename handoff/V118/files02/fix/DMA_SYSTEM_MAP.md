# 🗺️ DMA_SYSTEM_MAP.md — BẢN ĐỒ KIẾN TRÚC (skeleton V1 · v1.24)
...
## ⭐ CURRENT CANONICAL ENDPOINT — D336 / SYSTEM_MAP v1.24 / HANDOFF V118-M2 (Teacher Acknowledgement "Cô đã đọc" CLOSED, 2026-07-27)
...
- **Frontend HEAD (accepted tip):** `78b75e59` (V118-M2 · "Fixed teacher appreciation UI" — lineage `df958fc6` V118-M1 baseline → `56ee2005` V118-M2 implementation (3 file authorized, 7 hunk) → `78b75e59` Owner-QA CTA-affordance correction (1 file, 1 hunk). Mọi commit `get_diff` verified D134; routeTree.gen.ts/generated/package/bun.lock/package-lock.json **zero-diff** cả hai commit — agent regenerate routeTree lần 3 đã bị chặn TRƯỚC commit.)
- **Migration:** registry **119** · mới nhất `v118_m2_appreciation_acknowledgement` (`20260727115750`, 27/07/2026, 3-block D92 + VERIFY ~23 assertion gồm impersonation D333 ba persona, PASS atomic, rollback guard; cột additive `session_appreciations.acknowledged_at timestamptz NULL` + RPC `acknowledge_session_appreciation` + additive replace 2 projection; `notify pgrst` D289 đã gửi).
- **Inventory sống (27/07/2026 · sau V118-M2 · re-verified read-only tại closeout):** **89 tables · 215 functions · 204 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · registry 119 · routes 52 · 16 Edge Functions.**
...
- **Trạng thái:** **V118-M2 CLOSED (27/07/2026)** — V118-M1 CLOSED cùng ngày — V117-M2 CLOSED cùng ngày — V117-M1 CLOSED cùng ngày · V116 S0–S4 CLOSED (26/07) · **live data:** 2/2 appreciation rows acknowledged (Bình 16:50:49Z · An 16:58:19Z, actor GV Đặng Mỹ Linh) + 2 audit rows metadata-only · sprint kế (candidates, chưa mở): **V117-M3 Parent outcome mở rộng** · **G.4+ restyle (consent/settings/kid)** · **V114-SEC1** · **FMN E2E fixture session** · **Day-state semantics v2** · **Secondary-parent fixture**.
