...
-- ---------------------------------------------------------------------------
-- BLOCK 2 — REVOKE / GRANT deliberate hardening (C5).
--   Rationale is NOT "CREATE OR REPLACE resets the ACL": we treat the exact
--   allowlist as an explicit, verified security control. The new 4-arg proc has
--   no prior grants, and Supabase default privileges make EXECUTE grantable to
--   PUBLIC/anon on any (re)created function; we therefore REVOKE those, then
--   GRANT precisely {authenticated, service_role}. Verified in BLOCK 3.
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    expected_search := case when r.proname='get_session_detail' then 'search_path=public' else 'search_path=""' end;
    if r.proconfig is null or not (expected_search = any(r.proconfig)) then
      raise exception 'VERIFY C5: % search_path not hardened -> %', r.proname, r.proconfig; end if;
    -- no anon / PUBLIC EXECUTE
    select count(*) into v from aclexplode(coalesce(r.proacl, acldefault('f', r.proowner))) a
     where a.privilege_type='EXECUTE' and a.grantee in (0, (select oid from pg_roles where rolname='anon'));
    if v <> 0 then raise exception 'VERIFY C5: % grants EXECUTE to anon/PUBLIC', r.proname; end if;
    ...
    if v <> 1 then raise exception 'VERIFY C5: % missing authenticated EXECUTE', r.proname; end if;
    -- service_role has EXECUTE
    select count(*) into v from aclexplode(coalesce(r.proacl, acldefault('f', r.proowner))) a
     where a.privilege_type='EXECUTE' and a.grantee=(select oid from pg_roles where rolname='service_role');
    if v <> 1 then raise exception 'VERIFY C5: % missing service_role EXECUTE', r.proname; end if;
  end loop;
