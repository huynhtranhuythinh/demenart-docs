...
**Migration** `20260815101138` (`v128_b6_3_p3_intent_integrity`). Endpoint: RULES **D363** · SYSTEM_MAP **v1.51** · HANDOFF **V128-B6.3-PHASE-3** · backend tail `20260815101138` · FE main pin `2.8.5`.

### Architecture transition

**Before Phase 3 (v1.50):** replay/conflict identity = structural triple only (action_key + object_type + object_id); no fingerprint; execute (INVOKER) did the processing-INSERT directly; authenticated had ledger INSERT.

**After Phase 3 (v1.51):**

```
public.execute_mission_control_action                    [SECURITY INVOKER · search_path='']
   gates 1–9 (auth → profile → registry lookup → verdict → null → context → input → object → match)
   v
mc_internal._mc_begin_action(request_id, action_key, object_type, object_id,
                             school_id, program_id, lead_teacher_id)   [DEFINER · owner postgres]
   +-- derive actor := current_profile()          (internal; not a caller arg)
   +-- canonical intent {v:1, action_key, object_type, object_id, context{school_id}, input{program_id, lead_teacher_id}}
   +-- fingerprint := encode(extensions.digest(canonical::text,'sha256'),'hex')   version 1
   +-- INSERT processing WITH (version, fingerprint) ON CONFLICT(request_id) DO NOTHING
   v
   fresh  → static CASE (class.assign.v1 → assign_class_distribution) → mc_internal._mc_commit_action
   exists → replay: not-found / triple-mismatch / legacy-NULL / version≠1 / fingerprint-mismatch → MC_ACTION_REQUEST_CONFLICT
            ; equal + processing → REQUEST_IN_PROGRESS ; equal + completed → cached replay
```

### Ledger (mission_control_action_requests → 14 columns)
+`intent_hash_version smallint` +`intent_fingerprint text` (nullable) + pair CHECK `(both NULL) OR (version=1 AND fp NOT NULL AND length=64)`. 12 legacy rows fingerprint NULL (no backfill → non-replayable, fail-closed). No fingerprint index (lookup by unique request_id). Hash-only (no canonical-payload storage).

### Write-boundary ownership (both DEFINER)
- **Begin:** `mc_internal._mc_begin_action` (DEFINER, owner postgres, `search_path=''`) — sole processing-writer; derives actor + computes fingerprint internally; no actor/hash/status/terminal args.
- **Terminal:** `mc_internal._mc_commit_action` (DEFINER) — unchanged (md5 frozen), completed-only.
- authenticated: **SELECT-only** on ledger (INSERT revoked, `insert_own_processing` dropped, `select_own` kept). No client-forgeable ledger write path.

### Explicit invariants (v1.51)
- execute remains **SECURITY INVOKER**, signature BẤT BIẾN; gate order + registry lookup + validators + static adapter CASE + adapter exception map + commit-core call unchanged.
- `object_type` remains constant `'class'`; no dynamic SQL; registry = WHAT, adapter = WHO.
- Replay-conflict now compares **structural triple + intent fingerprint** (was triple-only). Legacy NULL / version≠1 / fingerprint-mismatch → `MC_ACTION_REQUEST_CONFLICT` (no new public error code).
- Terminal ownership unchanged; authenticated terminal UPDATE = 0.
- `get_mission_control_actions` / `_mc_lookup_action` / `get_available_actions` / `_mc_commit_action` / `assign_class_distribution` md5 unchanged; registry unchanged; FE unchanged.

### Canonicalization
UUID `::uuid::text` (case/lexical collapse) · `lead_teacher_id` absent ≡ null ≡ "" → JSON null · key order irrelevant (hash from parsed values).

### md5 (v1.51)
execute `7a526354c820ab5f767ee7403c6e917d` · _mc_begin_action `f47260ef3f06811ac2e83807989b26c7`
FROZEN: get_mission_control_actions `3596633c6f7f1b7ecdc81822691475d4` · lookup `5d940037687be0a398a232cf987bfcf6` · gaa `fd874243a90e20d171058f3ddb648356` · commit `ce36c5fe109e99a919158a4482940c6a` · adapter `03a1510bd827c03a650a3a88312fbe3a`

### Verification
- **PRE/POST P3-E1–E12 + regression PASS.** Allowed delta: same request_id + same triple + different semantic intent → `MC_ACTION_REQUEST_CONFLICT` (PRE incorrectly `replayed:true`). E2b OBJECT_NOT_FOUND (INVOKER RLS) preserved. get_mission_control_actions byte-identical.
- Forge closure verified: direct authenticated INSERT → `insufficient_privilege`; `_mc_begin_action` derives actor (=caller), fingerprint server-computed.

### Structural delta
ledger 12→14 cols · mc_internal 2/2→**3/3** · policies 168→**167** · authenticated ledger INSERT+SELECT→**SELECT** · tables 92 / functions 248 / secdef 236 / triggers 33 unchanged · no index delta.

### Gap disposition
G1 CLOSED · G2 CLOSED · **G3 CLOSED** · G4 CLOSED. B6.3 technical scope complete; overall milestone closeout = Owner decision.

### Next gate
Owner closeout of B6.3 milestone. No Phase 4 / new action / generic engine authorized.

**Rollback:** restore execute (md5 `cf446902587cd6d0dea9d578d645bf71`) · DROP `_mc_begin_action` · restore authenticated INSERT + recreate `insert_own_processing` · DROP 2 columns + pair CHECK · re-harden execute ACL · keep `select_own` · NOTIFY pgrst; no ledger-data repair beyond dropping Phase-3 columns. Khối v1.50/D362 (P2) & v1.49/D361 (P1) = HISTORICAL SNAPSHOTS (BẤT BIẾN).
