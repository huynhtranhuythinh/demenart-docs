...
## ⭐ CURRENT CANONICAL ENDPOINT — D358 / SYSTEM_MAP v1.46 / HANDOFF V128-B6.1.5 (Mission Control Action Foundation — CANONICAL RECONCILIATION: action layer đã sống trong live [registry `mission_control_action_registry` + ledger `mission_control_action_requests` + execute RPC INVOKER + class.assign adapter `assign_class_distribution` + audit→memory projection], object `class` có action, nay canonical hoá; backend migration tail `20260813113400`, inventory **92·248·236·169·33·1**, FE main pin `2.8.5` [FE HEAD accepted tip PENDING OWNER], 0 mutation — documentation reconcile only, 2026-08-14. Lineage prior: v1.45/D357 B6.1-Task-5.2 · v1.44/D356 B3.3 = HISTORICAL SNAPSHOT)
...
## 🧾 V127-M4.2.5 — SCHOOL PEOPLE WORKSPACE (IDENTITY PROJECTION) (SYSTEM_MAP v1.35 · D347 · FORENSIC RECONSTRUCTION, 2026-08-10)

> **Khối này nối tiếp V127-M3.7 làm authority hiện trạng tầng school-people / identity-projection.** Endpoint: RULES **D347** · SYSTEM_MAP **v1.35** · HANDOFF **V127-M4.2.5** · code HEAD **`8c0ca0a0`** · migration tail **`20260810074214`**. Provenance: structural + build + ACL verified trong phiên (RPC body + aclexplode + get_diff + tsgo/build); Owner-Gate runtime QA chưa ghi nhận. Các tầng trước (V124 media · V125 tooling · V126 parent-journey · M2.4 admin-audit · M3.7 teacher-identity) GIỮ nguyên authority.

### People = projection (KHÔNG model mới)
People Workspace đọc read-only qua 2 projection RPC từ `profiles` + `class_distributions`; KHÔNG entity Person canonical mới. Projection layer = source duy nhất.

### Projection RPC (2, secdef, anon-denied)
- `get_school_people(school, query, limit)` → `{ok, school_id, people[{person_id, identity, contexts, responsibilities}]}`
- `get_person_workspace(school, person)` → `{ok, identity, contexts, responsibilities}` | `{ok:false, error:not_authorized|not_found}`
- Gate `is_admin() OR profile-in-school`; `search_path=public,pg_temp`; REVOKE PUBLIC + anon (D231); GRANT authenticated/service_role.

### Privacy by source
RPC chỉ emit identity + contexts + responsibilities. KHÔNG children/journal/memory/media. Family context KHÔNG emit (hiện chỉ `school`). UI không thể leak thứ không projected.

### Frontend (consume-only)
`src/features/people/**` (adapter thuần + 2 hook rpcUntyped + 5 component + 2 page) · route `/school/people` + `/school/people/$personId` (opt-out nesting giữ school shell) · 1 nav "Con người". KHÔNG query bảng trực tiếp, KHÔNG `profiles.role` render, contexts render động (không bịa Family). Commit `8c0ca0a0`, tsgo=0/build=0, tooling 2.8.5.

### Inventory (verified live — V127 gap CLOSED)
**89 tables · 233 functions · 222 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · 16 Edge Functions.** Migration tail `20260810052804` → **`20260810074214`**. Function-gap 215→233 truy vết đủ: M2.4 +2 · M3.7 +14 · M4.2.5 +2.

**Delta:** DB +2 secdef projection RPC + ACL hardening (mig `20260810074045` + `20260810074214`) · FE 1 commit `8c0ca0a0` · Edge 0 · Bunny 0 · tooling net 0. **Rollback = DROP 2 RPC + FE revert.** Endpoint: RULES **D347** · SYSTEM_MAP **v1.35** · HANDOFF **V127-M4.2.5** · code HEAD **`8c0ca0a0`** · migration tail **`20260810074214`**.


---

## 🧾 V128-M0 — MISSION CONTROL OS SKELETON (Object Workspace Contract) (SYSTEM_MAP v1.36 · D348 · CANONICALIZED, 2026-08-10)

> **Khối này nối tiếp V127-M4.2.5 làm authority hiện trạng tầng admin / mission-control.** Endpoint: RULES **D348** · SYSTEM_MAP **v1.36** · HANDOFF **V128-M0** · code HEAD **`be04f4b`** · migration tail **`20260810074214`** (BẤT BIẾN từ D347). Provenance: `get_diff` 3 commit + `read_file`@SHA + `list_edits` + live DB re-pin. Các tầng trước (V124 media · V125 tooling · V126 parent-journey · V127 admin/teacher/people) GIỮ nguyên authority.

### Mission Control introduction
Admin đang chuyển từ **module-centric** (grid `admin.modules` + registry `admin_modules`/`admin_module_groups`/`admin_module_links`) sang **object-centric OS**: Find object → Understand context → Evaluate health → Execute permitted action → Review history. B0/B0.1 dựng khung renderer + contract, **fixture-only, zero backend**. Module grid GIỮ nguyên (chưa retire — chờ B4 parity).

### B0/B0.1 completed state (frontend-only)
- **Shell:** `MissionControlShell` (nav Trung tâm + 2 demo object) + `CommandBarPlaceholder` (disabled, "sắp mở") — route `admin.mission-control` + `admin.mission-control.index`.
- **Generic renderer route:** `admin.object.$type.$id` → `findDemoObject(type)` (fixture) → `ObjectWorkspace`.
- **Nav:** `admin.tsx` +1 "Mission Control" (icon LayoutGrid).
- **Fixtures:** `demoObjects.ts` — `demo-alpha` (đủ 5 dải) + `demo-beta` (thiếu context/health) — trung tính, **0 PII, 0 domain type thật**.

### ObjectWorkspaceModel foundation (contract seam)
`ObjectWorkspace` render **5 dải cố định, KHÔNG rẽ nhánh theo type**: Identity · Context · Health · Actions · History. Contract v1 (sau B0.1 revision):
- `ObjectIdentity {type, id, title, subtitle?}` — `type` open string.
- `ObjectContext {kind, label, href?, meta[]}` — **href = navigation** (có = navigable, không = informational).
- `HealthSignal {key, label, status: ok|watch|risk|unknown, detail?}`.
- `ObjectAction {key, label, intent: primary|default|danger, enabled, disabledReason?}` — **descriptor thuần, KHÔNG handler** (seam cho capability).
- `HistoryEntry {id, at, actor?, summary, kind?}` — trả lời What/Who/When.
- **B0→B0.1 revision:** `ObjectContext` key/value→kind/href/meta (thêm nav) · `intent` neutral/caution→default/danger · `HistoryEntry` key/label/detail→id/actor/summary/kind (thêm actor). Đây là **contract migration**, không phải "apply".

### Governance relationship
- **B1 (Object Intelligence Architecture) = DESIGN-ONLY**, chưa shipped. Object Registry (D348.3) + Capability boundary (D348.4) là **design-locked chỉ thị cho B2+**, KHÔNG có code. Resolver thật sẽ mở rộng `admin_lookup_search` (hiện resolve profiles/children/media/capsules; mù School/Class/Subscription/Support) — B2 backend.
- **Privacy/soul:** Forbidden objects (child journey/journal/skills/badges, family memory content, raw media bytes) KHÔNG BAO GIỜ là Mission Control object (LINH HỒN + D347.3 privacy-by-source). Child-workspace open = reason-mandatory + logged (D345.2).

### Inventory (verified live — BẤT BIẾN từ D347)
**89 tables · 233 functions · 222 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron · 16 Edge Functions.** Migration tail **`20260810074214`** KHÔNG đổi. B0/B0.1 = 0 DB delta.

**Delta:** DB **0** (frozen) · FE 3 commit `a117bfd5`→`8d70281d`→`be04f4b` · Edge 0 · Bunny 0 · tooling net 0. **Rollback = FE revert về `8c0ca0a0`.** Endpoint: RULES **D348** · SYSTEM_MAP **v1.36** · HANDOFF **V128-M0** · code HEAD **`be04f4b`** · migration tail **`20260810074214`**.


---

## 🗺️ V128-B2.0 / B2.1 / B2.2 — MISSION CONTROL DATA WIRING (Registry + Adapter + first tenant object) — CANONICALIZED v1.37→v1.39 (2026-08-11)

> **Khối authority mới** (supersede B0/B0.1 header phía trên như CURRENT). Provenance: migration statements THẬT (`20260810150209` / `20260811042807` / `20260811080037`) + live re-pin (`pg_get_functiondef` · `aclexplode` · impersonation) — RULES D349/D350/D351. B1 (Object Intelligence Architecture) = design-only, **0 migration** (nguyên tắc D348.3/4 đã canonical tại D348).

### Kiến trúc 3 tầng (data → adapter → DTO)
- **Tầng 1 — Registry (D349, v1.37):** `mission_control_object_registry` (PK object_type) = **catalog thuần, 0 behavior**. 12 cột metadata + 7 CHECK constraint đóng miền. 17 seed rows: **4 wired** (person/child/media/capsule — platform) · **7 registered** (school/subscription/support_case/class/session/program/privacy_request) · **6 forbidden** (child_journey/journal/skills/badges/family_memory/raw_media, `forbidden_groups={'*'}`). Dispatch KHÔNG sống trong table (COMMENT chốt) — sống ở adapter CASE. ACL: client roles 0 privilege, `service_role` SELECT-only, owner `postgres` full (adapter đọc qua definer). RLS deferred (non-blocking).
- **Tầng 2 — Adapter (D350, v1.38):** `get_object_workspace(text,uuid,text)` SECURITY DEFINER `search_path=''` = **single privacy chokepoint**. Gate order: is_admin → unknown → forbidden → not_available(registered) → scope_not_wired → reason_required → access-log → **static CASE dispatch** (KHÔNG dynamic SQL) → allowlist filter → forbidden-leak RAISE → DTO. **Privacy moat kép:** allowlist theo `discovery_fields` + forbidden-leak guard theo `forbidden_groups`; projector trả rich payload nhưng adapter chỉ đọc `v_raw->'<identity_key>'` rồi strip → sensitive groups (ở top-level khác) KHÔNG bao giờ chạm renderer (privacy-by-source D347.3 + adapter allowlist). Logger `admin_workspace_access_log` +capsule (additive); open object không log.
- **Tầng 3 — DTO `WorkspaceProjectionDTO/v1`:** `{ok,dto,object_type,object_id,kind,scope,privacy_policy,projector_status,fields,capabilities,reason_logged}`. Hợp đồng data↔renderer; FE `ObjectWorkspaceModel` (B0.1 contract v1) tiêu thụ = **track FE riêng**. DTO/v1 **KHÔNG bump** qua B2.2 (school khít sẵn).

### B2.2 — School = tenant-scope object đầu tiên được wire (D351, v1.39)
- Candidate `get_school_overview` **REJECTED** (zero-param self-scoped `current_school_id`, không is_admin gate, sai shape) — tenant-operator self-view RPC không tái dùng làm admin projector.
- Projector mới `admin_lookup_school(uuid)` identity-only `{ok,school:{id,name,state}}` (zero child/media/person).
- Adapter 2-edit byte-preserve: scope gate widen `platform` → `{platform,tenant}` (NULL-safe) + CASE `WHEN 'school'`. **Status gate đứng trước scope gate** ⇒ tenant registered (class) vẫn `not_available`.
- Registry align: `discovery_fields [name,status]→[name,state]` (schools có cột `state`, không `status`). ⭐ **Governance rule: school discovery_fields KHÔNG BAO GIỜ chứa định danh child/person/media.**
- School `privacy=open` ⇒ no reason, no access-log (nhất quán person/media). **0 FE, 0 Edge, 0 Bunny, 0 logger.**

### Inventory chain (verified live)
| Endpoint | tables | fn | secdef | pol | trg | cron | migration tail |
|---|---|---|---|---|---|---|---|
| D348 (B0.1) | 89 | 233 | 222 | 166 | 33 | 1 | `20260810074214` |
| **D349 (B2.0)** | **90** | 233 | 222 | 166 | 33 | 1 | `20260810150209` |
| **D350 (B2.1)** | 90 | **234** | **223** | 166 | 33 | 1 | `20260811042807` |
| **D351 (B2.2)** | 90 | **235** | **224** | 166 | 33 | 1 | `20260811080037` |

**Registry state (v1.39):** post-B2.2 = **5 wired** (person/child/media/capsule = platform + **school = tenant**) · 6 registered · 6 forbidden. ACL toàn bộ MC surface (`get_object_workspace`, `admin_lookup_school`) = `{authenticated,postgres,service_role}` · 0 anon/public. FE HEAD `be04f4b` BẤT BIẾN toàn arc (backend-only).

**Rollback toàn arc:** DROP `admin_lookup_school` → restore adapter pre-B2.2 → registry school→registered/[name,status] (B2.2); DROP `get_object_workspace` + restore logger child/parent (B2.1); DROP TABLE registry (B2.0). **Endpoint: RULES D351 · SYSTEM_MAP v1.39 · HANDOFF V128-B2.2 · code HEAD `be04f4b` · migration tail `20260811080037`.**


---

## 🧭 V128-B3.0 / B3.1 / B3.1.5 — MISSION CONTROL CONTEXT SEAM (validator + CLASS consumer + assignment scope foundation) — CANONICALIZED v1.40→v1.42 (2026-08-12)

> **Khối authority mới** (supersede B2.0/B2.1/B2.2 header phía trên như CURRENT). Provenance: `pg_get_functiondef` (core + validator + 2 wrapper + `admin_lookup_class`) + `aclexplode(coalesce(proacl,acldefault))` + registry `context_requirements` re-pin + synthetic proof (rollback-safe) — RULES D352/D353/D354. Migration statements THẬT `20260811110749`/`20260811111257`/`20260812023155`/`20260812051609`. **Backend-only toàn arc; FE `be04f4b` BẤT BIẾN.**

### Topology — Context Seam (data → validate → core → DTO)

Trước B3.0: `get_object_workspace(text,uuid,text)` = adapter authenticated-executable, chokepoint inline. B3.0 **tách CORE nội bộ khỏi wrapper công khai**:

```
client (authenticated)
   │
   ├── get_object_workspace(text,uuid,text)        [3-arg legacy wrapper, LANGUAGE sql]  ──┐  empty context '{}'
   └── get_object_workspace(text,uuid,jsonb,text)  [4-arg context wrapper, LANGUAGE sql]  ──┤  passthrough context
                                                                                            ▼
                                    _mission_control_workspace_core(text,uuid,jsonb,text)   [INTERNAL-ONLY {postgres,service_role}]
                                            │ gate: is_admin → unknown → forbidden → not_available(registered)
                                            │       → scope_not_wired → VALIDATE CONTEXT → [authz slot NO-OP] → reason/log
                                            │       → CASE dispatch → allowlist → forbidden-leak → DTO/v1
                                            ▼
                                    validate_mission_control_object_context(text,jsonb)     [INTERNAL-ONLY {postgres,service_role}]
                                            reads registry.context_requirements → {valid,errors[],normalized_context}
```

- **Core = internal boundary (D352.1):** `_mission_control_workspace_core` SECURITY DEFINER `search_path=''` owner `postgres`, EXECUTE `{postgres,service_role}` **0 authenticated/anon/PUBLIC**. Authenticated direct call → `42501`. Toàn bộ privacy chokepoint (D350) + double moat (allowlist + forbidden-leak) sống trong core, giờ sau boundary internal.
- **Wrapper = public surface (D352.2):** 2 overload `get_object_workspace` (3-arg legacy delegate empty context · 4-arg context passthrough), LANGUAGE sql SECURITY DEFINER `search_path=''` EXECUTE `{authenticated,postgres,service_role}`. Object cần context qua legacy path → `context_invalid` (fail-closed). Backward-compat cho object platform (0 requirement).
- **Registry gains `context_requirements jsonb` (D352.3):** `{version, keys:{<key>:{type,required}}, allow_unknown}`; default `{version:1,keys:{},allow_unknown:false}`. Requirement = data, không hardcode.
- **Validator = data-binding, fail-closed (D352.4):** internal-only. malformed req / non-object → `context_requirements_malformed`/`context_not_object`; type-check `uuid|text|int|bool` → `context_type_mismatch`; required thiếu → `context_missing_required_key`; unknown khi `allow_unknown=false` → `context_unknown_key`. Trả `normalized_context` (chỉ key hợp lệ, uuid btrim). **Validate SHAPE, KHÔNG cấp quyền.**
- **Context ≠ authorization (D352.5):** authz slot `PERFORM v_ctx` = **NO-OP** ở B3.0. `is_admin()` là boundary DUY NHẤT. DTO/v1 KHÔNG bump. Dispatch đọc `v_ctx=normalized_context`, không đọc raw input.

### B3.1 — CLASS = first context consumer (D353, v1.41)
- CLASS registry: registered→**wired** (core/tenant), discovery `[name]`, context_requirements `{keys:{school_id:{type:uuid,required:true}},allow_unknown:false}`. Object tenant thứ 2 wired (sau school) nhưng **đầu tiên cần context**.
- Projector `admin_lookup_class(uuid p_class_id, uuid p_school_id)` identity-only `{ok,class:{name}}`, self-gated `is_admin()`. **Bound lookup: class_id AND school_id trong MỘT predicate** (không derive class→school trước); wrong-school ≡ nonexistent-class → cùng `not_found` (indistinguishable). **0 school_id leak qua discovery.**
- Core CASE `WHEN 'class'` → `admin_lookup_class(p_object_id, (v_ctx->>'school_id')::uuid)` (dùng validated context). Legacy 3-arg → empty context → `context_invalid`. Package v2 (mig `..._class_context_seam_v2`) mới ALL PASS.

### B3.1.5 — assignment scope gate foundation (D354, v1.42)
- Scope vocabulary của core: `platform / tenant` → **`platform / tenant / assignment`** (explicit allowlist 3 giá trị, NULL-safe `IS DISTINCT FROM`). Scope lạ vẫn `scope_not_wired`.
- Status gate (registered→`not_available`) ĐỨNG TRƯỚC scope gate (giữ D351.3).
- Scope = **containment metadata, KHÔNG authorization**; context authz seam vẫn NO-OP. **ZERO SESSION wiring.**
- Wired-nhưng-thiếu-dispatch → `dispatch_missing`. Synthetic proof (rollback-safe): synthetic-wired SESSION PRE `scope_not_wired` → POST `dispatch_missing` — chứng minh assignment qua scope vocabulary, KHÔNG nghĩa SESSION wired.
- ⭐ **ASSIGNMENT domain = containment bởi `class_distribution`:** `lesson_sessions → class_distribution_id → class_distributions → class_id → classes → school_id → schools`. class_distribution = binding vận hành program/distribution vào class (assignment/lifecycle context riêng). ⇒ SESSION **assignment-scoped**, KHÔNG re-scope tenant.

### Inventory chain (verified live)
| Endpoint | tables | fn | secdef | pol | trg | cron | migration tail |
|---|---|---|---|---|---|---|---|
| D351 (B2.2) | 90 | 235 | 224 | 166 | 33 | 1 | `20260811080037` |
| **D352 (B3.0)** | 90 | **238** | **227** | 166 | 33 | 1 | `20260811111257` |
| **D353 (B3.1)** | 90 | **239** | **228** | 166 | 33 | 1 | `20260812023155` |
| **D354 (B3.1.5)** | 90 | 239 | 228 | 166 | 33 | 1 | `20260812051609` |

B3.0 = +3 fn (core + validator + 4-arg wrapper; 3-arg = REPLACE) + registry +1 column. B3.1 = +1 fn (`admin_lookup_class`). B3.1.5 = 0 net (core REPLACE, scope allowlist +assignment).

### Registry state (v1.42) — CURRENT LIVE
**6 wired** · **5 registered** · **6 none/forbidden** (17 rows):
- **wired (6):** `person`·`child`·`media`·`capsule` (platform) · `school` (tenant) · **`class`** (tenant, context=`{school_id}`).
- **registered (5):** `subscription`·`support_case`·`program`·`privacy_request` (platform) · **`session`** (assignment).
- **none/forbidden (6):** `child_journey`·`journal`·`skills`·`badges`·`family_memory`·`raw_media` (`forbidden_groups={'*'}`, LINH HỒN + D347.3).

ACL toàn MC surface: core + validator = `{postgres,service_role}` (internal); 2 wrapper + `admin_lookup_class`/`admin_lookup_school` = `{authenticated,postgres,service_role}`. **0 anon/PUBLIC** khắp nơi. Security regression PASS: authenticated direct core→`42501` · admin wrapper→works · non-admin wrapper→`not_authorized`.

### SESSION — CURRENT LIVE (unwired)
`object_type=session` · kind=**supporting** · scope=**assignment** · projector_status=**registered** · context_requirements `{version:1,keys:{},allow_unknown:false}` (default-empty) · `admin_lookup_session` **ABSENT** · normal workspace path → `not_available`.

### NEXT / PROPOSED — V128-B3.2 (⚠️ NOT APPLIED)
> **KHÔNG phải production state.** Design input duy nhất cho SESSION = second context consumer. Trước khi wire cần Owner Gate.

- Proposed SESSION context contract (**NOT APPLIED**): `{version:1,keys:{class_distribution_id:{type:uuid,required:true}},allow_unknown:false}`.
- Sẽ cần: projector `admin_lookup_session(...)` (chưa tồn tại) + core CASE `WHEN 'session'` + registry session registered→wired + context_requirements UPDATE. **Chưa có code, chưa có authorization.**

**Rollback toàn arc:** restore core scope 2-value (B3.1.5) → DROP `admin_lookup_class` + restore core CASE pre-class + registry class→registered (B3.1) → DROP core+validator+4-arg wrapper + restore `get_object_workspace(text,uuid,text)` B2.2 body + DROP COLUMN context_requirements (B3.0). **Endpoint: RULES D354 · SYSTEM_MAP v1.42 · HANDOFF V128-B3.1.5 · code HEAD `be04f4b` · migration tail `20260812051609`.**

---

## 🧭 V128-B3.2 — MISSION CONTROL · SESSION = SECOND CONTEXT CONSUMER — CANONICALIZED v1.43 (2026-08-12)

> **Endpoint:** D355 / SYSTEM_MAP v1.43 / HANDOFF V128-B3.2 · migration tail `20260812070542` (`v128_b3_2_session_context_consumer`) · code HEAD `be04f4b` (backend-only, 0 FE). Khối "NEXT / PROPOSED — V128-B3.2 (NOT APPLIED)" phía trên = HISTORICAL SNAPSHOT (đã APPLIED).

### B3.2 — SESSION wiring (D355, v1.43)
- SESSION registry: registered→**wired**, `kind=supporting` · `scope=assignment` (assignment object đầu tiên wired; KHÔNG re-scope tenant). discovery `[title,state]`, context_requirements `{version:1,keys:{class_distribution_id:{type:uuid,required:true}},allow_unknown:false}`. capability_vocab `{view:null}` + forbidden_groups `[]` unchanged.
- Projector `admin_lookup_session(p_session_id uuid, p_class_distribution_id uuid)` identity-only `{ok,session:{id,title,state}}`, self-gated `is_admin()`→`not_admin`. **Bound lookup: `lesson_sessions.id AND class_distribution_id` trong MỘT predicate** (không derive session→cd trước); wrong-cd ≡ nonexistent-session → cùng `not_found` (indistinguishable). `title` nullable giữ NULL; `state` emit text. **0 cd/class/school/teacher/child leak qua discovery.**
- Core CASE `WHEN 'session'` → `admin_lookup_session(p_object_id, (v_ctx->>'class_distribution_id')::uuid)` (validated/normalized context). Static dispatch, KHÔNG dynamic. CLASS branch unchanged. Legacy 3-arg → empty context → `context_invalid`.

### Topology — SESSION context consumer
```
session (object_type)
  → Mission Control registry (scope=assignment, projector_status=wired)
  → validate_mission_control_object_context(text,jsonb) [class_distribution_id uuid required · fail-closed]
  → normalized_context.class_distribution_id
  → _mission_control_workspace_core static CASE 'session'
  → admin_lookup_session(session_id, class_distribution_id)  [bound-lookup lesson_sessions]
  → discovery allowlist [title,state]  → forbidden-leak guard
  → WorkspaceProjectionDTO/v1
```
- **Dependency chain (containment, grounded live):** `lesson_sessions.class_distribution_id → class_distributions.id → class_distributions.class_id → classes.id → classes.school_id → schools.id`.
- ⭐ **NHƯNG projector B3.2 bind SESSION TRỰC TIẾP vào `class_distribution_id`** — KHÔNG resolve `school_id`, KHÔNG re-scope tenant. Chain trên chỉ là bản đồ containment (assignment domain D354.6), không phải đường projector.
- ⭐ **Current-containment (KHÔNG immutable):** `class_distribution_id` NOT NULL + FK-bound nhưng KHÔNG có immutability guard proven → context = CURRENT containment (A→B ⇒ ctx A `not_found`, ctx B `ok`). KHÔNG historical, KHÔNG reassignment, KHÔNG FK immutability trigger.

### Inventory chain (verified live)
| Endpoint | tables | fn | secdef | pol | trg | cron | migration tail |
|---|---|---|---|---|---|---|---|
| D354 (B3.1.5) | 90 | 239 | 228 | 166 | 33 | 1 | `20260812051609` |
| **D355 (B3.2)** | 90 | **240** | **229** | 166 | 33 | 1 | `20260812070542` |

B3.2 = +1 fn (`admin_lookup_session`) + core REPLACE (1 SESSION branch) + registry session-row UPDATE. tables/policies/triggers/cron/RLS/roles/FE BẤT BIẾN.

### Registry state (v1.43) — CURRENT LIVE
**7 wired** · **4 registered** · **6 none/forbidden** (17 rows):
- **wired (7):** `person`·`child`·`media`·`capsule` (platform) · `school`·`class` (tenant, class context=`{school_id}`) · **`session`** (assignment, context=`{class_distribution_id}`).
- **registered (4):** `subscription`·`support_case`·`program`·`privacy_request` (platform).
- **none/forbidden (6):** `child_journey`·`journal`·`skills`·`badges`·`family_memory`·`raw_media` (LINH HỒN + D347.3).

ACL: core + validator = `{postgres,service_role}` (internal); 2 wrapper + `admin_lookup_class`/`admin_lookup_school`/**`admin_lookup_session`** = `{authenticated,postgres,service_role}`. **0 anon/PUBLIC** khắp nơi. `admin_lookup_session`: owner postgres · secdef · `search_path=""` exact.

### SESSION — CURRENT LIVE (wired)
`object_type=session` · kind=**supporting** · scope=**assignment** · projector_status=**wired** · discovery `[title,state]` · context_requirements `{version:1,keys:{class_distribution_id:{type:uuid,required:true}},allow_unknown:false}` · `admin_lookup_session(uuid,uuid)` **PRESENT** · normal path (4-arg + đúng context) → ok/DTO/v1/scope=assignment/fields={title,state}.

### Failure semantics (verified S1–S14)
`wrong cd → not_found` · `nonexistent → not_found` (≡) · `missing/malformed/unknown-key ctx → context_invalid` · `legacy 3-arg → context_invalid` · `non-admin wrapper → not_authorized` · `non-admin direct projector → not_admin` · `forbidden object → forbidden_object` · `assignment-wired-no-dispatch → dispatch_missing`. Six-object PRE≡POST diffcount=0 (capsule = real success-path).

**Rollback B3.2:** DROP `admin_lookup_session(uuid,uuid)` → restore core CASE pre-session (remove `WHEN 'session'` branch) → registry session→registered / discovery `[]` / context default-empty. **Endpoint: RULES D355 · SYSTEM_MAP v1.43 · HANDOFF V128-B3.2 · code HEAD `be04f4b` · migration tail `20260812070542`.**

## 🧭 V128-B3.3 — MISSION CONTROL · PROGRAM = THIRD CONTEXT CONSUMER — CANONICALIZED v1.44 (2026-08-12)

> **Endpoint:** D356 / SYSTEM_MAP v1.44 / HANDOFF V128-B3.3 · migration tail `20260812190653` (`v128_b3_3_program_context_consumer`) · code HEAD `be04f4b` (backend-only, 0 FE). Khối B3.2 (v1.43) phía trên = HISTORICAL SNAPSHOT (đã APPLIED, superseded).

### B3.3 — PROGRAM wiring (D356, v1.44)
- PROGRAM registry: registered→**wired**, `kind=supporting` · `scope=platform` · `privacy_policy=open`. discovery `[name]`, context_requirements `{version:1,keys:{},allow_unknown:false}` (**zero-context**). capability_vocab `{edit:"program.edit",view:null}` + forbidden_groups `[]`.
- Projector `admin_lookup_program(p_program_id uuid)` identity-only `{ok,program:{id,name}}`, self-gated `is_admin()`→`not_admin`. discovery strip → chỉ `[name]`; **0 description/slug/curriculum/lesson/pricing/IP leak**.
- Core CASE `WHEN 'program'` → `admin_lookup_program(p_object_id)` (zero-context, KHÔNG dùng v_ctx). Static dispatch, KHÔNG dynamic. CLASS+SESSION branches unchanged. **Legacy 3-arg → ok** (zero-context ⇒ empty context hợp lệ; KHÁC class/session vốn fail-closed `context_invalid`).

### Topology — PROGRAM context consumer (zero-context)
```
program (object_type)
  → Mission Control registry (scope=platform, projector_status=wired, zero-context)
  → validate_mission_control_object_context(text,jsonb) [0 required key · empty context pass · unknown-key fail-closed]
  → _mission_control_workspace_core static CASE 'program'
  → admin_lookup_program(program_id)  [identity-only lookup programs]
  → discovery allowlist [name]  → forbidden-leak guard
  → WorkspaceProjectionDTO/v1
```
- ⭐ **Zero-context = fresh-domain proof:** PROGRAM cần 0 containment context ⇒ chứng minh seam phục vụ cả object không cần context (khác class=`school_id`, session=`class_distribution_id`). `programs` = platform-level catalog, KHÔNG tenant/assignment containment.
- ⭐ **IP-safe projection:** identity-only `[name]`; content/curriculum/pricing KHÔNG bao giờ chạm renderer (adapter allowlist + privacy-by-source).

### Inventory chain (verified live)
| Endpoint | tables | fn | secdef | pol | trg | cron | migration tail |
|---|---|---|---|---|---|---|---|
| D355 (B3.2) | 90 | 240 | 229 | 166 | 33 | 1 | `20260812070542` |
| **D356 (B3.3)** | 90 | **241** | **230** | 166 | 33 | 1 | **`20260812190653`** |

B3.3 = +1 fn (`admin_lookup_program`) + core REPLACE (1 PROGRAM branch) + registry program-row UPDATE. tables/policies/triggers/cron/RLS/roles/FE BẤT BIẾN.

### Registry state (v1.44) — CURRENT LIVE
**8 wired** · **3 registered** · **6 none/forbidden** (17 rows):
- **wired (8):** `person`·`child`·`media`·`capsule` (platform) · `school`·`class` (tenant) · `session` (assignment) · **`program`** (platform, zero-context).
- **registered (3):** `privacy_request`·`subscription`·`support_case` (platform).
- **none/forbidden (6):** `badges`·`child_journey`·`family_memory`·`journal`·`raw_media`·`skills` (LINH HỒN + D347.3).

ACL: core + validator = `{postgres,service_role}` (internal); 2 wrapper + `admin_lookup_school`/`admin_lookup_class`/`admin_lookup_session`/**`admin_lookup_program`** = `{authenticated,postgres,service_role}`. **0 anon/PUBLIC** khắp nơi. `admin_lookup_program`: owner postgres · secdef · `search_path=""` exact.

### PROGRAM — CURRENT LIVE (wired)
`object_type=program` · kind=**supporting** · scope=**platform** · projector_status=**wired** · privacy=**open** · discovery `[name]` · context_requirements `{version:1,keys:{},allow_unknown:false}` (zero-context) · `admin_lookup_program(uuid)` **PRESENT** · normal path (3-arg legacy HOẶC 4-arg empty context) → ok/DTO/v1/scope=platform/fields={name}.

### Scope vocabulary proof (three live consumers)
| Milestone | Object | Scope | Context mode |
|---|---|---|---|
| B3.1 (D353) | CLASS | tenant | required (`school_id`) |
| B3.2 (D355) | SESSION | assignment | required (`class_distribution_id`) |
| **B3.3 (D356)** | **PROGRAM** | **platform** | **zero-context** |

Seam admit cả ba scope vocabulary (platform/tenant/assignment) VÀ cả hai context mode (required + zero) trên live consumers — KHÔNG renderer change, KHÔNG dynamic dispatch.

### Failure semantics (verified P1–P14)
`correct → ok` (fields exactly `{name}`) · `nonexistent → not_found` · `legacy 3-arg → ok` (zero-context) · `unknown-key ctx → context_invalid` · `non-admin wrapper → not_authorized` · `non-admin direct projector → not_admin` · `forbidden object → forbidden_object` · CLASS/SESSION unchanged · assignment-wired-no-dispatch synthetic → `dispatch_missing`. Seven-object PRE≡POST diffcount=0.

### Explicit registered debt (NOT wired B3.3)
...
- **privacy_request:** discovery hiện valid NHƯNG future growth về child/requester identity = D48 hazard · cần guardrail tường minh.

**Rollback B3.3:** DROP `admin_lookup_program(uuid)` → restore core CASE pre-program (remove `WHEN 'program'` branch) → registry program `projector_status` wired→registered (discovery `[name]`/zero-context/capability/forbidden BẤT BIẾN — set từ registration). **Endpoint: RULES D356 · SYSTEM_MAP v1.44 · HANDOFF V128-B3.3 · code HEAD `be04f4b` · migration tail `20260812190653`.**

## 🧭 V128-B6.1 · Task 5.2 — MISSION CONTROL · request_id INTENT LIFECYCLE (FE) — CANONICALIZED v1.45 (2026-08-14)

> **Endpoint:** RULES **D357** · SYSTEM_MAP **v1.45** · HANDOFF **V128-B6.1-Task-5.2** · **FE HEAD `e1c2ea8f`** · backend BẤT BIẾN (0 migration / 0 RPC / 0 schema). Khối B3.3 (v1.44) phía trên = HISTORICAL SNAPSHOT.
> *(v1.45 = kế tiếp tip on-disk v1.44. Nếu bản live đã vượt v1.44, bump theo tip thật.)*

### Scope (FE-only, 1 file)
- **File:** `src/features/mission-control/class/hooks/useAssignClassAction.ts` (duy nhất).
- **Frozen contract:** `execute_mission_control_action(p_action_key,p_object_id,p_context,p_input,p_request_id)`; `missionControlAdapter.executeAction` forward `requestId → p_request_id` 1:1 (KHÔNG đổi). Adapter/RPC không sinh/không cache request_id.

### Behavior — request_id lifecycle
- Owner FE của `request_id` = hook: `requestIdRef` + **`lastIntentRef` (mới)**.
- `intentKey = JSON.stringify(built.payload)`; payload = `{context:{school_id}, input:{program_id, lead_teacher_id?}}` (builder deterministic ⇒ key ổn định).
- Mint id mới ⇔ `requestIdRef===null` **HOẶC** `lastIntentRef !== intentKey`; else reuse. SUCCESS → clear cả 2 ref · onError/business-failure → giữ cả 2 · drawer Đóng (`resetRequest`) → clear cả 2.
- **Idempotency:** same intent retry ⇒ same id (backend replay nguyên); new intent ⇒ new id.

### QA verdict (preview, evidence request_id thật) — 4/4 PASS
- A same-intent `0ecac41d…26526c1` ×2 · B new-intent `c035d931…2027e669` ≠ A · C ledger `bc1f2b36…` ≠ `d20dc916…` · D `0bb20e70…2d14e` ≠ `1a41fc63…ef233b`.
...
### Tooling integrity (incident + fix)
- HEAD `b2ec6ad3` (hook fix, agent-mode) re-float `@lovable.dev/vite-tanstack-config 2.8.5→2.12.0` (package.json + bun.lock manifest+resolution) → gate `assert-tooling-governance.mjs` G1+G3 fail. **HEAD `e1c2ea8f`** restore pin `2.8.5` (literal, no non-frozen install). Gate G1–G4 PASS. **Deploy HEAD = `e1c2ea8f`**, KHÔNG `b2ec6ad3`. (Chi tiết rule: D357.3.)

### QA infra (rollback sạch)
- Seed lớp QA `eeeeeeee-0000-4000-8000-000000000001` ("ZZ_QA_V128_B6.1_T5.2") @ Trường Demo Dế Mèn (`b6a4ac35-2e0a-4667-9eea-756f615c29eb`) → **rollback toàn bộ** (class + distributions + lesson_sessions + 2 ledger rows). Live: `total_classes=7`, 0 dấu vết QA.

### Registry / consumers / inventory — BẤT BIẾN
- Task 5.2 = FE-only ⇒ backend không đổi so với B3.3: registry 17 rows (wired 8 / registered 3 / none 6), inventory **90/241/230/166/33/1**, migration tail `20260812190653`.

### Deploy
- `demenart.lovable.app` — `is_published=true`, `publish_visibility=public`, published commit **`e1c2ea8f`**. Cloudflare/`demenart.com` build từ GitHub main qua gate (pin `2.8.5` ⇒ G1–G4 PASS).

**Rollback Task 5.2:** revert `useAssignClassAction.ts` về HEAD `d1d5b774` (bỏ `lastIntentRef` + revert `submit`/reset); GIỮ pin `2.8.5`. Single-file, trivial. **Endpoint: RULES D357 · SYSTEM_MAP v1.45 · HANDOFF V128-B6.1-Task-5.2 · FE HEAD `e1c2ea8f`.**
...
## 🧭 V128-B6.2-A1 · MISSION CONTROL EXECUTION INTEGRITY BOUNDARY (ledger finalization) — CANONICALIZED v1.47 (2026-08-14)

> **Endpoint:** RULES **D359** · SYSTEM_MAP **v1.47** · HANDOFF **V128-B6.2-A1** · backend tail **`20260814114948`** (`v128_b6_2_a1_ledger_finalization_boundary`) · FE main pin `2.8.5` (0 FE). Khối B6.1.5 (v1.46) phía trên = HISTORICAL SNAPSHOT.
> **Bản chất:** production hardening APPLIED. Đóng ledger-forge gap (RULES D358.9.4). Chỉ tầng finalization/privilege — KHÔNG generic adapter, KHÔNG intent fingerprint, KHÔNG registry-authority (vẫn DEFERRED).

### Execution integrity flow (LIVE)

```
authenticated
     ↓
execute_mission_control_action        (SECURITY INVOKER · search_path='' · public contract BẤT BIẾN)
     │  = EXECUTION ORCHESTRATION BOUNDARY
     ├─ validate auth/profile/action_key/context/input/school-match   (server-owned gates)
     ├─ INSERT processing ledger row      (authenticated INSERT + insert_own_processing)      ┐
     ├─ assign_class_distribution(uuid,uuid,uuid)   (SECURITY DEFINER · UNCHANGED)             │ β2 subtransaction
     ├─ build server execution result (evidence)                                              │ (BEGIN…EXCEPTION)
     └─ mc_internal._mc_commit_action(request_id, result)                                      ┘
              ↓  = FINALIZATION PRIVILEGE BOUNDARY
     mc_internal._mc_commit_action        (SECURITY DEFINER · owner postgres · search_path='')
              │  completed-only · derive actor internally · validate ownership (FOR UPDATE)
              ↓
     mission_control_action_requests → status='completed' + result_payload    (server-owned)
```

### Two boundaries (ownership model)

- **Execution orchestration boundary** = `execute_mission_control_action` (INVOKER). Sở hữu: validation, adapter invocation, evidence build, idempotency claim (INSERT processing). **KHÔNG** sở hữu terminal privilege. Public contract (signature + return envelope) BẤT BIẾN.
- **Finalization privilege boundary** = `mc_internal._mc_commit_action` (DEFINER/postgres). Sở hữu **DUY NHẤT** quyền ghi terminal ledger state. completed-only. Reachable **chỉ** qua `execute` (schema `mc_internal` unexposed PostgREST; EXECUTE authenticated cần cho INVOKER-caller nhưng không REST-reachable).

### Ledger ownership model (post-A1)

| Role | INSERT processing | SELECT own | UPDATE terminal (status/result_payload/error_code/completed_at) |
|---|---|---|---|
| `authenticated` (client) | ✅ (`insert_own_processing`) | ✅ (`select_own`) | ❌ REVOKED (col-grant gỡ + `finish_own` DROP) |
| `mc_internal._mc_commit_action` (DEFINER postgres) | — | — | ✅ completed-only (sole writer) |

Forge-completed-ledger (D358.9.4) = **CLOSED**: client không còn đường ghi terminal state; "completed + result_payload" giờ **server-authoritative**.

### Failure semantics (β2, completed-only)

adapter fail → commit-core KHÔNG gọi → subtransaction rollback → **no orphan processing, no `failed` persistence** → client `{ok:false, error_code}` graceful. `failed` lifecycle schema-compatible nhưng **UNUSED** (không remove). Replay-sau-fail = fresh; success-replay bất biến.

### New object

| Schema / Object | Vai trò | Posture |
|---|---|---|
| `mc_internal` (schema) | Internal namespace, unexposed PostgREST | USAGE `authenticated` only; REVOKE PUBLIC/anon |
| `mc_internal._mc_commit_action(uuid,jsonb)` | Finalization privilege boundary (completed-only) | SECURITY DEFINER · owner postgres · `search_path=''` · EXECUTE `{authenticated,postgres}` |

### Inventory (post-apply)

**`92 tables · 248 functions · 236 SECURITY DEFINER · 168 policies · 33 triggers · 1 cron`** · migration tail **`20260814114948`**.
(Δ vs v1.46 `92/248/236/169/33/1`: **−1 policy** (`finish_own` dropped) · `execute` REPLACE net 0 · `authenticated` ledger column-UPDATE revoked.)
**Public inventory convention GIỮ NGUYÊN:** `mc_internal` là **internal schema, KHÔNG đếm trong public function inventory**. Ngoài đếm public: `mc_internal` {1 fn / 1 secdef}.

### Invariant preserved

`assign_class_distribution(uuid,uuid,uuid)` — definition + grants BẤT BIẾN; shared bởi Mission Control execution + School Portal direct (`school.manage.tsx` `handleAssign`). Hai write path KHÔNG hợp nhất (G4b DEFERRED). `execute` giữ INVOKER (INV-B62-A1-03). Authority boundary vận hành (`is_admin()` + role/school scope) unchanged; context authz slot vẫn NO-OP (D352.5).

**Rollback A1:** DROP `mc_internal._mc_commit_action` + DROP SCHEMA `mc_internal` → CREATE OR REPLACE `execute` về body pre-A1 (self-finalize) → CREATE POLICY `finish_own` (nguyên văn) → GRANT UPDATE(4 cols) `authenticated` → `NOTIFY pgrst`. **Endpoint: RULES D359 · SYSTEM_MAP v1.47 · HANDOFF V128-B6.2-A1 · backend tail `20260814114948` · FE main pin `2.8.5`.**

---

## 🧭 V128-B6.2-A2 · MISSION CONTROL LEDGER BOUNDARY — RUNTIME QA VERIFICATION STATE — CANONICALIZED v1.48 (2026-08-14)

> **Endpoint:** RULES **D360** · SYSTEM_MAP **v1.48** · HANDOFF **V128-B6.2-A2** · backend tail **`20260814114948`** (BẤT BIẾN — QA-only) · FE main pin `2.8.5`. Khối B6.2-A1 (v1.47) phía trên = HISTORICAL SNAPSHOT (BẤT BIẾN).
> **Bản chất:** QA verification state. Bản đồ hoá kết quả runtime QA cho ledger finalization boundary (A1/D359). KHÔNG schema/topology change — topology giữ nguyên v1.47.

### Runtime QA verification (ledger finalization boundary)

| QA | Invariant | Actor path | Result |
|---|---|---|---|
| QA-1 Replay | idempotency (single request = single side effect) | real authenticated (impersonated master_admin) | **PASS** — replay `replayed=true`, same distribution, Δ side-effect 0 |
| QA-2 Conflict | β2 rollback-only (no orphan / no duplicate) | real authenticated | **PASS** — `MC_ACTION_CONFLICT`, no ledger row, no duplicate, no audit |
| QA-3 Forge | terminal ownership (client cannot forge) | real authenticated direct UPDATE | **PASS** — `42501 permission denied` (grant layer), row immutable |

⇒ D359 hai mục "NOT VERIFIED" (replay idempotency · backend conflict injection) = **VERIFIED**. Forge-denied (INV-B62-A1-02) = negative-verified. Ledger finalization boundary **runtime-proven**, không chỉ structural.

### Boundary state (unchanged from v1.47, nay QA-confirmed)

- Finalization privilege boundary `mc_internal._mc_commit_action` (DEFINER/postgres, completed-only) = sole terminal writer → **confirmed**: client direct UPDATE denied ở grant layer (`42501`).
- Idempotency via `UNIQUE(request_id)` + execute conflict-branch stored-result → **confirmed**: replay trả stored payload, KHÔNG re-execute.
- β2 subtransaction (INSERT-processing + adapter + commit-core) rollback-on-failure → **confirmed**: conflict → no orphan processing, no `failed` persistence.

### Inventory (BẤT BIẾN từ v1.47)

**`92 tables · 248 functions · 236 SECURITY DEFINER · 168 policies · 33 triggers · 1 cron`** · migration tail **`20260814114948`**. QA-only: 0 schema, 0 migration. (QA **data** persisted — fixture + 1 distribution + 1 completed ledger + 1 audit; xem RULES D360.)

### Persisted QA fixture (no cleanup)

class `eeeeeeee-0000-4000-8000-000000000005` · program `eeeeeeee-0000-4000-8000-0000000000a5` · entitlement `eeeeeeee-0000-4000-8000-0000000000e5` · distribution `2a40a52f-6991-4093-ae89-6273f0e733c5` (active) · ledger `ec486402…` (completed). Cleanup = Owner Gate riêng, KHÔNG implicit.

**Rollback:** block v1.48 thuần tài liệu (xoá block); QA data cleanup tùy chọn Owner Gate. **Endpoint: RULES D360 · SYSTEM_MAP v1.48 · HANDOFF V128-B6.2-A2 · backend tail `20260814114948` · FE main pin `2.8.5`.**

---

## 🧭 V128-B6.3 PHASE 1 · ACTION CONTROL PLANE — REGISTRY AUTHORITY FOUNDATION — CANONICALIZED v1.49 (2026-08-15)

> **Endpoint:** RULES **D361** · SYSTEM_MAP **v1.49** · HANDOFF **V128-B6.3-PHASE-1** · backend tail **`20260815080313`** (`v128_b6_3_p1_registry_authority_foundation`) · FE main pin `2.8.5`. Khối v1.48 (B6.2-A2) phía trên = HISTORICAL SNAPSHOT (BẤT BIẾN).
> **Bản chất:** Phase 1 apply state. **PHASE 1 CLOSED, KHÔNG PHẢI B6.3 CLOSED.** Registry chuyển descriptor → dispatchability authority foundation; execute chưa cutover.

### Architecture transition

**BEFORE (≤v1.48):** `mission_control_action_registry` = descriptor / discovery catalog (đọc bởi discovery, KHÔNG tham chiếu bởi execute).

**AFTER PHASE 1 (v1.49):** `mission_control_action_registry` = **dispatchability authority foundation**:

```
Mission Control Action Registry
  ├─ adapter_key          (class.assign → 'class.assign.v1'; class.edit → NULL)
  ├─ execution_mode       ('single_domain_rpc')
  ├─ required_context     ({"keys":["school_id"],"exclusive":true})
  └─ input_schema         (MissionActionInputSchema/v1 skeleton, options_source=dynamic)
        │
        ▼
  mc_internal._mc_lookup_action(object_type, action_key)
        │   DEFINER · search_path='' · ACL {authenticated,postgres} · A9 8-field whitelist
        │   three-way verdict: dispatchable / present-not-dispatchable / absent
        ▼
  controlled authority projection
```

**BUT:** `execute_mission_control_action` **vẫn literal-dispatch** `class.assign` (chưa consume `_mc_lookup_action`). ⇒ **Control Plane Foundation = PARTIAL · Adapter Resolver Cutover = Phase 2.**

### Discovery topology (post-Phase-1)

- `get_available_actions` (DEFINER · ACL `{postgres,service_role}`): predicate = `status='active' AND adapter_key IS NOT NULL` (dispatchable-only, registry-driven). **Internal gate** cho `get_mission_control_actions`. Loại class.edit tự động.
- `get_mission_control_actions` (DEFINER · ACL `{postgres,authenticated}`) = **FROZEN hardcoded FE-facing projection** (md5 `e4fdd0138e27f40eb9c76aa4f4b6ab65`) — gate trên `get_available_actions`(class.assign present), emit hardcoded `class.assign` item (label 'Assign Program' hardcoded) + dynamic program/teacher options. **Giữ nguyên tới Phase 2 equivalence cutover (E12).** FE nhận envelope byte-compatible; registry label ('Assign Teacher') vẫn DEAD, reconcile = Phase 2.

### Registry state (post-apply)

| action_key | status | adapter_key | dispatchable |
|---|---|---|---|
| class.assign | active | class.assign.v1 | ✅ |
| class.edit | disabled | NULL | ❌ (registered, no executor) |

### Frozen (md5 unchanged)

`execute_mission_control_action` `771470473d17c31a247999a51e499094` (INVOKER, literal-dispatch) · `mc_internal._mc_commit_action` `ce36c5fe109e99a919158a4482940c6a` · `assign_class_distribution` `03a1510bd827c03a650a3a88312fbe3a` · `get_mission_control_actions` `e4fdd0138e27f40eb9c76aa4f4b6ab65`. Ledger no `intent_fingerprint`, authenticated terminal UPDATE=0. FE pin 2.8.5.

### Gap disposition

G1 = **PARTIAL** (dispatchability authority + discovery internal truth LIVE; execute literal-dispatch remains) · G2 = **OPEN** (adapter resolver cutover) · G3 = **OPEN** (intent_fingerprint) · G4 = **CLOSED** (B6.2, no regression). Latent FE coupling (`ClassWorkspaceScreen` submit = class.assign-specific) = future debt, NOT fixed.

### Two boundaries (ownership model — unchanged, restated)

- **Execution orchestration** = `execute_mission_control_action` (INVOKER, literal-dispatch, public contract BẤT BIẾN). KHÔNG own terminal privilege.
- **Finalization privilege** = `mc_internal._mc_commit_action` (DEFINER/postgres, completed-only, sole terminal writer).
- **NEW — Authority projection** = `mc_internal._mc_lookup_action` (DEFINER/postgres, read-only registry authority verdict, A9-bounded). Chưa nằm trên đường execute (Phase 2 sẽ wire vào).

### Inventory (post-apply)

**Public `92 tables · 248 functions · 236 SECURITY DEFINER · 168 policies · 33 triggers · 1 cron` BẤT BIẾN** · `mc_internal` {2 fn / 2 secdef} (ngoài public count) · migration tail **`20260815080313`**. Δ vs v1.48: registry **+5 columns** (additive) · **+1** mc_internal fn/secdef · `get_available_actions` predicate replace (net 0 public fn) · 2 registry rows updated. FE/Edge/Bunny 0.

### Next gate

**V128-B6.3 Phase 2 — Adapter Resolver Cutover.** No implementation authorized by this block.

**Rollback:** DROP `_mc_lookup_action` → restore `get_available_actions` (md5 `626d84d21dc0442ba2b7ef5a09194eaa`, predicate `status='active'`) + ACL `{postgres,service_role}` → revert 2 registry rows → DROP 5 columns → NOTIFY pgrst; 0 data-repair. **Endpoint: RULES D361 · SYSTEM_MAP v1.49 · HANDOFF V128-B6.3-PHASE-1 · backend tail `20260815080313` · FE main pin `2.8.5`.**

---

## 🧭 V128-B6.3 PHASE 2 · ACTION CONTROL PLANE — ADAPTER RESOLVER CUTOVER — CANONICALIZED v1.50 (2026-08-15)

**Migration** `20260815085223` (`v128_b6_3_p2_adapter_resolver_cutover`). Endpoint: RULES **D362** · SYSTEM_MAP **v1.50** · HANDOFF **V128-B6.3-PHASE-2** · backend tail `20260815085223` · FE main pin `2.8.5`.

### Architecture transition

**Before Phase 2 (v1.49):** registry = dispatchability authority foundation; `execute` = literal `class.assign` dispatch (hardcoded `assign_class_distribution`).

**After Phase 2 (v1.50):**

```
mission_control_action_registry
   | adapter_key · required_context (typed) · input_schema (v1)
   v
mc_internal._mc_lookup_action('class', action_key)      [DEFINER · A9 8-field whitelist · authority only]
   |  verdict: found / dispatchable
   v
public.execute_mission_control_action                   [SECURITY INVOKER · search_path='']
   +-- registry verdict gate       (found=false ∨ dispatchable=false → MC_ACTION_NOT_FOUND)
   +-- bounded context validator   (keys/exclusive/types from required_context → CONTEXT_DENIED)
   +-- bounded v1 input validator  (declared-keys/required/uuid → INPUT_INVALID)
   +-- object lookup (RLS-filtered, INVOKER) → OBJECT_NOT_FOUND / context match → CONTEXT_DENIED
   +-- ledger INSERT processing / replay branch (structural-triple only)
   +-- static CASE allowlist        (adapter_key='class.assign.v1' ; ELSE → EXECUTION_FAILED)
   v
public.assign_class_distribution   [DEFINER · WHO-authz: is_admin ∨ role∧school]   (unchanged)
   v
mc_internal._mc_commit_action      [DEFINER · completed-only terminal writer]        (unchanged)
```

### Explicit invariants (v1.50)

- `execute_mission_control_action` remains **SECURITY INVOKER** (`search_path=''`), public signature BẤT BIẾN.
- `object_type` remains constant **`'class'`** — Phase 2 did **NOT** generalize to multi-object execution.
- **No dynamic SQL** (no `EXECUTE format` / no function-name-from-registry / no regprocedure).
- Adapter authz (`assign_class_distribution`) unchanged; **registry = WHAT, adapter = WHO**.
- Ledger lifecycle unchanged; replay-conflict still **structural-triple-only** (action_key + object_type + object_id); **no `intent_fingerprint`** (G3 OPEN).
- `_mc_lookup_action` / `get_available_actions` / `_mc_commit_action` / `assign_class_distribution` md5 unchanged.

### FE-facing projection architecture (`get_mission_control_actions`, DEFINER)

- **Authority from registry** (via `_mc_lookup_action`): action key · risk level · input_schema structure (field key/control/required/nullable/value_type).
- **Object/context gate preserved** (`get_available_actions` dispatchable-only discovery).
- **Server-owned presentation:** label **`'Assign Program'`** · description `'Create a class-program distribution'` · field labels `'Program'` / `'Lead teacher'` · dynamic program/teacher options (ORDER BY preserved).
- Registry label `'Assign Teacher'` = **non-FE presentation data** (không surface). Whitelist `_mc_lookup_action` không thêm label.

### md5 (v1.50)

- execute `cf446902587cd6d0dea9d578d645bf71` · get_mission_control_actions `3596633c6f7f1b7ecdc81822691475d4`
- FROZEN: lookup `5d940037687be0a398a232cf987bfcf6` · get_available_actions `fd874243a90e20d171058f3ddb648356` · commit `ce36c5fe109e99a919158a4482940c6a` · adapter `03a1510bd827c03a650a3a88312fbe3a`

### Verification

- **PRE/POST E1–E12 equivalence PASS** (EMPTY semantic diff; JWT-impersonation, BEGIN…ROLLBACK). execute: semantic jsonb + side-effect + ledger equality; get_mission_control_actions: semantic jsonb + array-order equality.
- **E2b runtime correction:** no-school-visibility actor → `MC_ACTION_OBJECT_NOT_FOUND` (INVOKER object lookup RLS-filtered before adapter authz). Design-corrected-by-live-evidence, not a semantic change.
- Public inventory `92·248·236·168·33·1` BẤT BIẾN; mc_internal {2 fn / 2 secdef} BẤT BIẾN.

### Gap disposition

G1 = **CLOSED** · G2 = **CLOSED** · **G3 = OPEN** · G4 = **CLOSED**.

### Next gate

**V128-B6.3 Phase 3 — Intent Integrity.** No implementation authorized by this block.

**Rollback:** restore `execute_mission_control_action` (md5 `771470473d17c31a247999a51e499094`) + `get_mission_control_actions` (md5 `e4fdd0138e27f40eb9c76aa4f4b6ab65`) + class.assign.required_context → `{"keys":["school_id"],"exclusive":true}` + re-harden ACL `{authenticated,postgres}` ×2 + NOTIFY pgrst; 0 ledger/adapter/commit/column repair. **Endpoint: RULES D362 · SYSTEM_MAP v1.50 · HANDOFF V128-B6.3-PHASE-2 · backend tail `20260815085223` · FE main pin `2.8.5`.** Khối v1.49/D361 (Phase 1) = HISTORICAL SNAPSHOT (BẤT BIẾN).

---

## 🧭 V128-B6.3 PHASE 3 · ACTION CONTROL PLANE — INTENT INTEGRITY — CANONICALIZED v1.51 (2026-08-15)

**Migration** `20260815101138` (`v128_b6_3_p3_intent_integrity`). **V128-B6.3 milestone CLOSED — Owner-stamped 2026-08-15.** Endpoint: RULES **D363** · SYSTEM_MAP **v1.51** · HANDOFF **V128-B6.3-PHASE-3** · backend tail `20260815101138` · FE main pin `2.8.5`.

### Before Phase 3 (v1.50)
`execute` registry-driven + static adapter resolver (G1+G2 CLOSED); ledger `mission_control_action_requests` **no** `intent_fingerprint`; replay-conflict structural-triple-only; ledger authenticated grants INSERT,SELECT; G3 OPEN.

### After Phase 3 (v1.51)
- **Intent forge** `mc_internal._mc_begin_action` (SECURITY DEFINER, `search_path=''`) — sole ledger INSERT owner; computes `intent_fingerprint = sha256_hex(canonical_intent_v1)` + `intent_hash_version=1`; INSERT `status='processing'` `ON CONFLICT (request_id) DO NOTHING`.
- **Ledger** gains `intent_fingerprint text` (nullable) + `intent_hash_version smallint` (nullable), no default, **no backfill** (12 legacy rows stay NULL).
- **Client write closure** — authenticated INSERT revoked; ledger INSERT policy dropped; client grants SELECT-only (policy `..._select_own`). Both forge + terminal writes DEFINER-exclusive.
- **execute** gains intent-integrity replay guard: on `request_id` reuse, recompute fp + verify structural-triple/fp/hash_version vs stored → mismatch `MC_ACTION_REQUEST_CONFLICT`; in-flight `MC_ACTION_REQUEST_IN_PROGRESS`; terminal → stored `result_payload` + `replayed:true`. Remains SECURITY INVOKER.

### Action execution pipeline (v1.51, live-verified)
`execute (INVOKER)` → `_mc_lookup_action` (registry authority: dispatchable/adapter_key/required_context/input_schema) → declarative context+input validation → RLS object gate → `_mc_begin_action` (DEFINER: forge fp + INSERT, ON CONFLICT) → **if inserted**: static CASE resolver `class.assign.v1 → assign_class_distribution` → `_mc_commit_action` (DEFINER: terminal write) → **if not inserted**: intent-integrity replay guard.

### Explicit invariants (v1.51)
1. `execute_mission_control_action` = SECURITY INVOKER (verified: no SECURITY DEFINER, `search_path=''`).
2. `_mc_commit_action` = SECURITY DEFINER, sole terminal writer (md5 `ce36c5fe…` frozen).
3. `_mc_begin_action` = SECURITY DEFINER, sole ledger INSERT owner (new, md5 `f47260ef…`).
4. Static CASE adapter resolver — single `class.assign.v1` branch; NO dynamic SQL / EXECUTE format / regprocedure.
5. `assign_class_distribution` unchanged (md5 `03a1510b…`); registry unchanged; `get_mission_control_actions` unchanged (md5 `3596633c…`); FE unchanged (pin `2.8.5`, read-contract byte-stable).
6. No legacy fingerprint backfill (nullable, no default; 12/12 legacy rows NULL).
7. No terminal client write (authenticated UPDATE/DELETE=false) + no client forge (INSERT=false).
8. No dynamic SQL dispatch (execute + begin bodies verified).

### md5 (v1.51)
execute `7a526354c820ab5f767ee7403c6e917d` · `_mc_begin_action` `f47260ef3f06811ac2e83807989b26c7` · `_mc_commit_action` `ce36c5fe109e99a919158a4482940c6a` · `_mc_lookup_action` `5d940037687be0a398a232cf987bfcf6` · `assign_class_distribution` `03a1510bd827c03a650a3a88312fbe3a` · `get_mission_control_actions` `3596633c6f7f1b7ecdc81822691475d4`.

### Live inventory (v1.51)
public: tables 92 · fns 248 · secdef 236 · policies **167** (168→167) · triggers 33. `mc_internal` {3 fn / 3 secdef}: `_mc_begin_action`, `_mc_commit_action`, `_mc_lookup_action`. Ledger indexes: pkey `id`, UNIQUE `request_id`, `actor_id_idx` (no unique index on `intent_fingerprint`).

### Gap disposition (FINAL)
G1 CLOSED · G2 CLOSED · G3 CLOSED · G4 CLOSED. **V128-B6.3 milestone CLOSED.** Deferred: Decision layer · approval workflow · human-in-loop · multi-object expansion · generic action marketplace · FE expansion. Debt: FE `ClassWorkspaceScreen` class.assign-specific submit path. Note: positive-path fingerprint proof unpersisted (mechanism structurally verified).

### Next gate
V128-B6.3 CLOSED. No successor phase within B6.3. Next Mission Control work (Decision layer / FE decoupling / multi-object) = new milestone, not authorized by this block.

**Rollback:** DROP `mc_internal._mc_begin_action` → restore `execute_mission_control_action` (md5 `cf446902587cd6d0dea9d578d645bf71`, P2) + re-harden ACL → re-CREATE ledger INSERT policy + re-GRANT authenticated INSERT → ALTER TABLE DROP COLUMN `intent_fingerprint`,`intent_hash_version` → NOTIFY pgrst; 0 data-repair. **Endpoint: RULES D363 · SYSTEM_MAP v1.51 · HANDOFF V128-B6.3-PHASE-3 · backend tail `20260815101138` · FE main pin `2.8.5`.** Khối v1.50/D362 (Phase 2) = HISTORICAL SNAPSHOT (BẤT BIẾN).

---

## 🧭 V128-B7 — DECISION CONTROL PLANE (v1.52 · Phase A/B/C · approval gate integrated)

**Scope:** approval **Decision Gate** integrated into `execute_mission_control_action`. HIGH/CRITICAL actions require same-school admin approval before adapter; LOW/MEDIUM unchanged (auto). Governed path **DORMANT** (no HIGH/CRITICAL dispatchable action; class.assign = MEDIUM → auto). Phase A/B/C CLOSED; **B7 milestone = PENDING OWNER**.

### Decision domain (Phase A · `v128_b7_pa_decision_foundation_table`)
Table `public.mission_control_decisions`: id · action_request_id→ledger.request_id · **intent_fingerprint UNIQUE** · intent_hash_version · action_key · object_type · object_id · risk_level · state{pending,approved,rejected,expired,cancelled}=pending · requested_by · decided_by · decision_reason · expires_at=now()+72h · created/updated_at. RLS on; policy `mission_control_decisions_select_own` (SELECT authenticated WHERE requested_by ∈ my profiles); writes DEFINER-exclusive.

### Decision operations (Phase B · `v128_b7_pb_decision_helpers`)
mc_internal DEFINER: `_mc_open_decision` (open/find, ON CONFLICT(fp) DO NOTHING, RAISE no_eligible_approver/decision_not_required/object_scope_unsupported/intent_not_forged/ledger_row_not_found), `_mc_expire_decisions` (lazy, no cron). public DEFINER: `resolve_mission_control_decision` (self_decision_forbidden · not_authorized_to_decide = role ∧ same-school ∧ ≠requester), `cancel_mission_control_decision` (requester ∨ same-school admin · not_authorized_to_cancel), `get_mission_control_decision_inbox` (master/sub · own-school pending excl. own · runs lazy expire).

### Execute gate (Phase C · `v128_b7_pc_execute_decision_gate`)
Gate: after `_mc_begin_action`, before adapter CASE (fresh); after fp-match, before generic in-progress (replay). LOW/MEDIUM→auto; HIGH/CRITICAL→decision. 5 codes: DECISION_{REQUIRED,REJECTED,EXPIRED,CANCELLED} + NO_ELIGIBLE_APPROVER. Governed-return = normal RETURN in `begin…exception` subtxn (ledger stays 'processing' + decision kept — C-4 A). Resume = same request_id + original-actor guard → adapter+commit in β2 subtxn. Remains SECURITY INVOKER, search_path=''.

### Action execution pipeline (v1.52, live-verified)
`execute (INVOKER)` → `_mc_lookup_action` (registry: dispatchable/adapter_key/required_context/input_schema/**risk_level**) → declarative context+input validation → RLS object gate → `_mc_begin_action` (DEFINER: forge fp + INSERT, ON CONFLICT) → **if inserted**: [**HIGH/CRITICAL → `_mc_open_decision` → read decision (INVOKER,RLS): pending/rejected/expired/cancelled = normal RETURN (park, ledger processing) · approved = fall-through**] → static CASE `class.assign.v1 → assign_class_distribution` → `_mc_commit_action` (DEFINER: terminal) → **if not inserted**: fp verify → [**HIGH/CRITICAL → read decision: pending/…terminal RETURN · approved → original-actor guard → resume adapter+commit (β2)**] → in-flight `REQUEST_IN_PROGRESS` / terminal stored payload + `replayed:true`.

### Explicit invariants (v1.52)
1. `execute_mission_control_action` = SECURITY INVOKER (`search_path=''`); reads decisions only — NO INSERT/UPDATE/DELETE on `mission_control_decisions` (VERIFY-enforced).
2. `_mc_commit_action` DEFINER sole terminal writer (`ce36c5fe…` frozen); `_mc_begin_action` DEFINER sole ledger INSERT owner (`f47260ef…` frozen).
3. `_mc_open_decision` / `_mc_expire_decisions` = mc_internal DEFINER (decision write + lazy expiry).
4. resolve / cancel / inbox = public DEFINER; authz keys on caller (`auth.uid`), NOT the escalated DEFINER role.
5. `intent_fingerprint` UNIQUE → single decision per intent; approval binds to opener (RLS-enforced, C-3 B).
6. Ledger status domain unchanged {received,processing,completed,failed} — NO `awaiting_decision`; decision authoritative (C-4 A).
7. LOW/MEDIUM bypass gate (class.assign MEDIUM → auto, decisions Δ0 — live-confirmed rolled-back).
8. Static CASE adapter, no dynamic SQL; registry / adapter / `get_mission_control_actions` / FE unchanged (pin `2.8.5`).

### md5 (v1.52)
execute `09ef5f48f3318bfb53e126f3bc81d40a` (INVOKER) · `_mc_begin_action` `f47260ef3f06811ac2e83807989b26c7` · `_mc_commit_action` `ce36c5fe109e99a919158a4482940c6a` · `_mc_lookup_action` `5d940037687be0a398a232cf987bfcf6` · `_mc_open_decision` `e5b12a9675093ad2d93a2c33943bea77` · `_mc_expire_decisions` `2f6ca50546a438b2c2ee826d38765926` · `resolve_mission_control_decision` `bfcc7203b6f90f8f97deab23e9e6ad76` · `cancel_mission_control_decision` `315b5bbfac19f3eae1b00d0b215236b2` · `get_mission_control_decision_inbox` `47ca0947e7334ca8804a5c65c7b5f538` · `assign_class_distribution` `03a1510bd827c03a650a3a88312fbe3a` · `get_mission_control_actions` `3596633c6f7f1b7ecdc81822691475d4`.

### Live inventory (v1.52)
public: tables **93** (92→93) · fns **251** (248→251) · secdef **239** (236→239) · policies **168** (167→168) · triggers 33 · cron 1. `mc_internal` {5 fn / 5 secdef}: `_mc_begin_action`, `_mc_commit_action`, `_mc_lookup_action`, `_mc_open_decision`, `_mc_expire_decisions`. Decisions table: RLS on, 1 policy (`select_own`), **0 rows**, `intent_fingerprint` UNIQUE.

### Gap disposition
Decision Control Plane technical scope **COMPLETE** (Phase A/B/C CLOSED). **B7 milestone = PENDING OWNER** (not stamped CLOSED). Governed path **dormant** (no HIGH+ action registered). Deferred: activate an action at HIGH+ to arm the gate · decision inbox FE · notifications · multi-approver quorum · decision analytics · generic object decision expansion. Debt: FE `ClassWorkspaceScreen` class.assign-specific submit path (D363 carry-over).

### Next gate
No successor authorized by this block. Arming the gate (register an action HIGH+), decision FE, or **B8** = new milestone; NOT authorized here. Do not open B8. Do not stamp B7 milestone CLOSED automatically.

**Rollback:** Phase C = restore `execute` md5 `7a526354c820ab5f767ee7403c6e917d` + re-harden ACL `{authenticated,postgres}` + NOTIFY pgrst. Full B7 = + DROP resolve/cancel/inbox + DROP `_mc_open_decision`/`_mc_expire_decisions` + DROP TABLE `mission_control_decisions`. 0 data-repair (decisions 0 rows). **Endpoint: RULES D364 · SYSTEM_MAP v1.52 · HANDOFF V128-B7-CLOSEOUT · backend tail `20260815161535` · FE main pin `2.8.5`.** Khối v1.51/D363 (B6.3-P3) = HISTORICAL SNAPSHOT (BẤT BIẾN).

---

## 🧭 V128-B8-I1 — DECISION LIFECYCLE FOUNDATION (v1.53 · governance evidence + centralized transition controller · technical complete)

**Scope:** governance layer over B7 Decision Control Plane — **immutable lifecycle evidence** + **centralized transition controller** + **terminal-state protection** + **lifecycle audit trail**. Decision **state domain UNCHANGED** {pending,approved,rejected,expired,cancelled}; **NO execution states** (executing/completed/failed stay ledger). Governed path **DORMANT** (class.assign MEDIUM → auto). Migration `v128_b8_i1_decision_lifecycle_foundation`, tail `20260815182235`.

### Architecture relationship (v1.53 — invariant preserved)
**Registry = WHAT** (dispatchable/adapter_key/risk_level) · **Adapter = HOW** (`assign_class_distribution`) · **Decision = WHETHER** (approval gate + now lifecycle-governed) · **Ledger = execution truth** (`mission_control_action_requests`) · **Memory = downstream projection**. Decision layer chỉ đổi `mission_control_decisions.state` + ghi evidence; KHÔNG business mutation, KHÔNG gọi adapter, KHÔNG đụng ledger. Approve KHÔNG thực thi — execution vẫn là resume qua original actor (B7 C-2).

### Lifecycle evidence layer (new · `public.mission_control_decision_transitions`)
Append-only history. `id` · `decision_id` FK→`mission_control_decisions(id)` **ON DELETE NO ACTION** · `from_state` (NULL chỉ cho NULL→pending) · `to_state` · `actor_id` (profile identity; NULL cho system expiry) · `reason` · `created_at`. **KHÔNG denormalize** — Decision là source-of-truth. CHECK `mcdt_to_state_check` + **`mcdt_edge_check`** (closed edge graph: `NULL→pending` · `pending→{approved,rejected,expired,cancelled}`). RLS on; policy `mission_control_decision_transitions_select_own` (SELECT authenticated đọc history decision mình là requested_by); grants authenticated SELECT-only. **Immutability:** trigger `trg_block_decision_transition_mutation` → `mc_internal._mc_block_transition_mutation()` (search_path='') RAISE `transition_evidence_is_append_only`, chặn UPDATE/DELETE mọi role kể cả service_role (D89).

### Transition controller boundary (new · `mc_internal._mc_transition_decision`)
Biên mutation state DUY NHẤT (DEFINER, search_path='', ACL `{postgres}` only). `validate to_state (invalid_target_state chặn *→pending)` → `SELECT … FOR UPDATE` → `terminal protection (decision_not_pending)` → `UPDATE decisions` → `ROW_COUNT<>1 → transition_conflict` → `INSERT evidence`. resolve/cancel delegate write về controller (authz bất biến); `_mc_open_decision` giữ creation authority + ghi evidence NULL→pending chỉ khi create thật (idempotent no-dup); `_mc_expire_decisions` bulk-safe UPDATE…RETURNING → INSERT evidence set-based, actor NULL. C4: resolve/cancel bỏ dead lazy-expire write (không persist dưới txn rollback) → detect overdue → RAISE decision_expired; expiry evidence chỉ từ `_mc_expire_decisions()`.

### Explicit invariants (v1.53)
1. Decision state domain UNCHANGED {pending,approved,rejected,expired,cancelled}; NO exec states.
2. `_mc_transition_decision` = sole state-mutation boundary (FOR UPDATE + ROW_COUNT asserted); resolve/cancel/expire delegate; open = creation authority (NULL→pending evidence, no dup).
3. Transition evidence append-only (grants SELECT-only + RLS + block-trigger incl service_role); FK no-cascade.
4. `execute_mission_control_action` unchanged (INVOKER, reads decisions only) — Decision/execution separation intact.
5. Terminal states terminal — no reopen (`invalid_target_state`), terminal write blocked (`decision_not_pending`).
6. Governed path dormant (class.assign MEDIUM → auto); registry/adapter/ledger/FE (pin 2.8.5) unchanged.

### md5 (v1.53)
`_mc_transition_decision` `fe0eea599db711fb8472b37a72fa25e4` (DEFINER, NEW) · `_mc_block_transition_mutation` `f53eed4ddcc958faaad487232fb1392e` (non-DEFINER, NEW) · `_mc_open_decision` `b376edd70cc6fe4e83bf000dfa456954` · `_mc_expire_decisions` `c57c40b88b77e838da137dd3e9cf59e7` · `resolve_mission_control_decision` `c6208c6dbd8b1dac946735629e52d7d0` · `cancel_mission_control_decision` `4c4eecc55eb9467aee6c88f41ed6f0b6`. **FROZEN:** execute `09ef5f48f3318bfb53e126f3bc81d40a` · inbox `47ca0947e7334ca8804a5c65c7b5f538` · `_mc_begin_action` `f47260ef3f06811ac2e83807989b26c7` · `_mc_commit_action` `ce36c5fe109e99a919158a4482940c6a` · `_mc_lookup_action` `5d940037687be0a398a232cf987bfcf6` · `assign_class_distribution` `03a1510bd827c03a650a3a88312fbe3a`.

### Live inventory (v1.53)
public: tables **94** (93→94) · fns **251** (net 0) · secdef **239** (net 0) · policies **169** (168→169) · triggers **34** (33→34) · cron 1. `mc_internal` {**7** fn / **6** secdef}: `_mc_begin_action`, `_mc_commit_action`, `_mc_lookup_action`, `_mc_open_decision`, `_mc_expire_decisions`, `_mc_transition_decision`, `_mc_block_transition_mutation`. `mission_control_decision_transitions`: RLS on, 1 policy (`select_own`), append-only trigger, **0 rows**. `mission_control_decisions`: unchanged, **0 rows**.

### Verification
Structural VERIFY (in-migration, fail-closed) PASS · lifecycle rehearsal 14/14 PASS (create/approve/reject/cancel/expire + invalid-edge/terminal-reopen/edge-check + client-mutation-blocked/service_role-blocked) · zero residue PASS (decisions 0, transitions 0, registry restored). **Pending:** two-session concurrency rehearsal (not claimed complete).

### Next gate
No successor authorized by this block. **B8.2 / authority resolver expansion / delegation / notification / inbox UI / workflow builder / AI recommendation / child.transfer / object-scope beyond class = new milestone; NOT authorized here.** Do not open B8.2. Await CTO next-milestone decision.

**Rollback:** CREATE OR REPLACE open/expire/resolve/cancel về md5 B7 + re-apply ACL → DROP TRIGGER `trg_block_decision_transition_mutation` → DROP `mc_internal._mc_block_transition_mutation()` → DROP `mc_internal._mc_transition_decision(uuid,text,uuid,text)` → DROP TABLE `mission_control_decision_transitions` (cascade policy) → NOTIFY pgrst. 0 data-repair (0 rows, dormant). **Endpoint: RULES D365 · SYSTEM_MAP v1.53 · HANDOFF V128-B8-CLOSEOUT · backend tail `20260815182235` · FE main pin `2.8.5`.** Khối v1.52/D364 (B7 Decision Control Plane) = HISTORICAL SNAPSHOT (BẤT BIẾN).

---

## 🧭 V128-B9.2 — AUTHORITY RESOLVER FOUNDATION CONTRACT (v1.54 · governance ratification · design-frozen primitive · 0 implementation)

**Scope:** Canonicalizes the B9.1 design freeze (`DMA_V128-B9.1_AUTHORITY_CONTRACT_DESIGN_FREEZE.md`). **Governance ratification ONLY** — 0 implementation · 0 SQL · 0 migration · 0 DB mutation · 0 FE · 0 UI · 0 object created/modified. Backend tail **UNCHANGED** `20260815182235`. Ratifies an **independent Authority Resolver primitive** that B9 identified as **missing**: B8 centralized decision **state mutation** (`_mc_transition_decision`); B8 did **not** centralize **authority determination**, which today remains distributed across (1) action projection guards, (2) RLS/object visibility, (3) adapter guards, (4) decision RPC guards. B9.2 freezes the conceptual contract to consolidate authority **policy** into one primitive without touching enforcement layers.

### Authority Resolver architectural position (v1.54 — frozen)

The resolver is the identified-missing primitive on the authority dependency chain — it **consumes** the chain above it, emits an explainable verdict, and is invoked **before** decision/execution:

```
Identity → Scope → Context → Capability → AUTHORITY POLICY VERDICT → Decision / Execution
```

Full frozen dependency ordering:

```
Actor
  → Identity            (auth.uid → single authority-bearing profile — Option A, Part 5)
  → Role / Grants       (coarse classification + explicit entitlement — inputs / possible sources)
  → Organization Scope  (school / membership)
  → Object Context      (resolved via existing B3 registry + context resolver — DEPENDENCY, not rebuilt)
  → Capability          (action_key / capability vocabulary — PRIMARY authority input)
  → AUTHORITY RESOLVER  (the missing primitive; consumes all of the above)
  → { eligible + authority_source + reason_codes }   (deterministic, explainable verdict)
...
The resolver **never re-implements** object resolution: the B3 context seam (`resolve_mission_control_object_context` / `validate…` / `…match`) is a **dependency input**, not something the resolver rewrites.

### Explicit invariants (v1.54 — design-frozen)

1. Authority Resolver = **single deterministic source of the authority policy VERDICT** — NOT "single source of authorization." RLS + adapter domain-safety + lifecycle controller still enforce; authority policy ≠ complete enforcement (CTO Correction 1).
2. `capability`/`action_key` = **PRIMARY** authority input; `decision_type` = secondary/downstream context only (present across both direct-execute and decision-gated paths → the only stable anchor).
3. Resolver is **read-only · deterministic · explainable · consumed-not-embedded**. Callers invoke; they do not re-derive authority.
4. **Identity Option A** — one authenticated user → exactly one authority-bearing profile; delegation/representation = authority *sources* on a single identity, never multiple profiles. `UNIQUE(user_id)` enforcement **deferred** (reconciliation precondition: duplicates=0 re-verify + D93 invite/link flow non-breakage).
5. **Ownership boundaries (no parallel authority engine):** RLS = row visibility (coarse pre-filter; object-not-found ≠ not-authorized) · Adapter = domain invariants + transactional safety (defense-in-depth, trusts the verdict for authority) · Executor = orchestration + context validation + risk routing + **consumes verdict** · Lifecycle controller = state mutation + immutable evidence ONLY (authority resolved before it; no role/school/delegation logic pushed in — B8 boundary preserved).
6. **Design-frozen only** — 0 implementation, 0 enum minted, 0 constraint, 0 function, 0 migration. Governed path remains dormant.

### Vocabulary status (v1.54 — CTO Correction 2)

`authority_source` and `reason_codes` = **reserved DRAFT candidates only** (illustrative, non-canonical). Final enum + taxonomy **minted post-B9.3 evidence**, NOT here. This **supersedes** B9.1 §8.7's recommendation to mint at B9.2 (recorded at D366.6). Contract principle retained: verdict deterministic + explainable + machine-readable so no caller hardcodes exception strings.

### Live inventory (v1.54)

**UNCHANGED from v1.53** (no object touched): public tables **94** · fns **251** · secdef **239** · policies **169** · triggers **34** · cron **1**; `mc_internal` {**7** fn / **6** secdef}. `mission_control_decisions` 0 rows · `mission_control_decision_transitions` 0 rows (append-only trigger armed). Governed path dormant (class.assign MEDIUM → auto). Migration tail `20260815182235` (no migration). FE main pin `2.8.5`, 0 change.

### Verification

Governance-only: endpoint + artifact-lineage audit PASS (RULES D365 · SYSTEM_MAP v1.53 · B8-CLOSEOUT · tail `20260815182235` — B9.1 STEP-0 self-declaration matches live, zero drift). Append-only integrity PASS (original bytes preserved, tail additions only). No implementation / SQL / migration / minted invariant beyond ratified conceptual scope. No DB audit required (design ratification, no live-state claim beyond the recorded unchanged inventory).

### Next gate

**B9.3 — Phase-1 resolver skeleton** (first implementation: resolver authorizes one *new* governed action end-to-end · verify matrix · zero legacy touch). **NOT authorized by this block.** Deferred: final enum/taxonomy mint · `UNIQUE(user_id)` enforcement · delegation · object-scope expansion (`child.transfer`, session/school) · multi-approver/quorum · notification · inbox UI · workflow builder · AI recommendation · HIGH/CRITICAL activation. Await CTO next-milestone decision.

**Rollback:** none (no state change); reverse this governance decision via a superseding version block. **Endpoint: RULES D366 · SYSTEM_MAP v1.54 · HANDOFF V128-B9.2-CANONICAL-RATIFICATION · backend tail `20260815182235` · FE main pin `2.8.5`.** Khối v1.53/D365 (B8-I1 Decision Lifecycle Foundation) = HISTORICAL SNAPSHOT (BẤT BIẾN).

---

## 🗂️ SYSTEM_MAP v1.55 — V128-B9.3 · AUTHORITY RESOLVER PHASE-1 SKELETON (RECONCILIATION · retroactive canonicalization of live implementation · 0 mutation) — 2026-08-16

> **Authority block hiện hành** (supersede v1.54 làm current endpoint; v1.54/D366 B9.2 Foundation Contract Ratification = HISTORICAL SNAPSHOT BẤT BIẾN). Reconcile skeleton đã sống live (migration `20260815201925`) vào canonical — documentation only, **0 DB mutation**. Canonical rule: **RULES D367**.

### Authority Resolver runtime relationship (v1.55)

```
Mission Control
    |
    +-- governed actions  (registry-driven; execute_mission_control_action, INVOKER)
    |
    +-- authority decision primitive  (mc_internal._resolve_authority — STABLE, read-only verdict)
            |
            +-- CURRENT: probe harness only
            |       public.execute_authority_probe → _resolve_authority → mc_internal.authority_probe_log
            |       (isolated; zero legacy-executor touch; action authority.probe / LOW / adapter authority.probe.v1)
            |
            +-- FUTURE (deferred, NOT built): production governed actions consume the verdict
                    (Strangler Phase 2+ — decision paths, adapter authority-policy migration, legacy RPC migration)
```

Resolver = **authority POLICY verdict primitive** (D366.1/D366.4): trả `{ eligible, authority_source, reason_codes }`, deterministic + explainable + read-only. **KHÔNG** phải "single source of authorization" — RLS (visibility) + adapter (domain-safety) + lifecycle controller (`_mc_transition_decision`, B8 mutation boundary) vẫn enforce độc lập.

### Contract conformance + verify matrix (v1.55)

Skeleton khớp CHÍNH XÁC B9.1/D366 frozen contract: signature `resolve_authority(actor, action_key, object_context, decision_context) → {eligible, authority_source, reason_codes}` · output key `eligible` · STABLE read-only · actor=`profiles.id` (D88) · self-approval precedence trước platform grant (Risk 4). **Verify matrix 8/8 PASS** (resolver-direct, 0 write): platform→granted(platform_role) · master same-school→granted(organization_role) · master cross-school→organization_scope_mismatch · teacher/parent→role_not_eligible · missing school→object_context_mismatch · unknown actor→actor_unresolved · self-approval→self_decision_forbidden.

### Legacy isolation (v1.55 — MUST NOT over-claim)

- `execute_mission_control_action` md5 `09ef5f48f3318bfb53e126f3bc81d40a` **UNCHANGED** (INVOKER · VOLATILE); **KHÔNG** gọi resolver.
- `_mc_transition_decision` md5 `fe0eea599db711fb8472b37a72fa25e4` **UNCHANGED**.
- **KHÔNG claim:** `child.transfer` complete · legacy executor migrated · authorization replaced. Resolver hiện chỉ được exercise qua probe harness; production governed actions CHƯA consume verdict.

### Live inventory (v1.55 — reconciled-to-live)

Từ v1.54 baseline (94/251/239/169/34/1; `mc_internal` {7/6}): public tables **94** (probe_log ở `mc_internal`) · fns **252** (+1 `execute_authority_probe`) · secdef **240** (+1) · policies **169** · triggers **34** · cron **1**; `mc_internal` **{8 fn / 7 secdef}** (+1 `_resolve_authority`) **+1 table** `authority_probe_log` (RLS on, 0 policy, deny-all trừ definer, `request_id` unique). `authority_probe_log` 1 row · `mission_control_action_requests` 12 · `mission_control_decisions` 0. Governed production path dormant. Migration tail `20260815182235` → **`20260815201925`** (`v128_b93_authority_resolver_skeleton`, additive-only). FE main pin `2.8.5`, 0 change.

### Vocabulary status (v1.55 — nối D366.6)

`authority_source`/`reason_codes` lưu **controlled-text/jsonb**, **CHƯA mint pg-enum** (dù có evidence 8/8, chỉ 1 action exercise → hoãn mint tới ≥2 action; OD-02). Draft subset in-use: sources {platform_role, organization_role, none}; reason_codes {actor_unresolved, object_context_mismatch, organization_scope_mismatch, role_not_eligible, self_decision_forbidden, authority_granted}.

### Open decisions (v1.55 — RECORDED, not resolved)

**OD-01** synthetic `authority.probe` registry (action registry có, object registry không) → recommend synthetic-probe-exempt · **OD-02** vocabulary mint timing → recommend defer · **OD-03** probe ACL `authenticated` test surface → no hardening in reconcile · **OD-04** migration provenance → retroactive reconciliation, no rogue assertion. Tất cả **OPEN**.

### Verification (v1.55)

Reconciliation-only: baseline audit PASS (RULES D366 · SYSTEM_MAP v1.54 · B9.2 · tail `20260815182235`); live discovery PASS (tail `20260815201925`); contract conformance vs B9.1 PASS; verify matrix 8/8 PASS (resolver-direct, 0 write); legacy isolation PASS (executor+controller md5 unchanged, resolver caller-set = probe only); residue PASS (3 md5 unchanged post-matrix, probe_log 1 row unchanged, DML=0). Append-only integrity PASS (original bytes preserved, tail addition only). **0 SQL · 0 migration · 0 DB mutation · 0 minted invariant/enum.**

### Next gate boundary (v1.55)

B9.3 skeleton = **Strangler Phase 1 reconciled**. **Deferred (NOT authorized):** Phase 2 (decision paths open/inbox/resolve/cancel consume resolver) · Phase 3 (adapter authority-policy migration) · Phase 4 (legacy RPC migration) · enum/taxonomy mint · `UNIQUE(user_id)` enforcement · delegation · object-scope expansion (`child.transfer`, session/school) · multi-approver/quorum · notification · inbox UI · HIGH/CRITICAL activation. **Await explicit CTO next-milestone authorization. KHÔNG mở B9.4, KHÔNG thiết kế child.transfer, KHÔNG apply migration.**

**Endpoint: RULES D367 · SYSTEM_MAP v1.55 · HANDOFF V128-B9.3-SKELETON-RECONCILE · backend tail `20260815201925` · FE main pin `2.8.5`.** Khối v1.54/D366 (B9.2 Authority Resolver Foundation Contract — Ratification) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.56 — V128-B9.4 · AUTHORITY RESOLVER CONSUMPTION PHASE-1 LIVE (class.edit governed via Authority Resolver) — 2026-08-16

> **Authority block hiện hành** (supersede v1.55 làm current endpoint; v1.55/D367 B9.3 Skeleton Reconcile = HISTORICAL SNAPSHOT BẤT BIẾN). Authority Resolver nay được **consume trong production** bởi action đầu tiên `class.edit` tại executor seam. Canonical rule: **RULES D368**.

### Authority Resolver runtime relationship (v1.56)
...
Mission Control
    ...
    +-- governed actions  (registry-driven; execute_mission_control_action, INVOKER)
    |       |
    |       +-- class.assign  MEDIUM · authority_gated=false · legacy adapter self-authorizes (Phase-3 target)
    |       +-- class.edit    LOW    · authority_gated=true  · GATED via Authority Resolver (Phase-1 LIVE)
    |
    +-- authority decision primitive  (mc_internal._resolve_authority — STABLE, read-only verdict; {postgres})
            |
            +-- consumer 1: probe harness  (public.execute_authority_probe → authority_probe_log)
            +-- consumer 2: authority GATE  (mc_internal._mc_authority_gate — DEFINER STABLE, granted authenticated)
                    |
                    +-- executor seam:  ... object resolve → GATE → (eligible? deny=0-ledger : begin → adapter → mutation → audit)
```

Gate = **consume-only pass-through** of the resolver verdict; deny translated by executor to `MC_ACTION_PERMISSION_DENIED` + `reason_codes`, **before** `_mc_begin_action` (0 ledger, 0 mutation). Resolver = single-source **verdict** (D366.1/D366.4), not single-source authorization — RLS + adapter domain-safety + lifecycle controller still enforce.

### Contract conformance + verify (v1.56)

Gate matches B9.1/D366 frozen contract (`{eligible, authority_source, reason_codes}`, capability-primary, decision_context optional). **Gate resolver-tier 7/7 PASS** (platform/org grant · cross-school/role/context/actor deny). **class.edit integration (JWT-impersonated, rolled back):** happy (master) → `ok=true` + real mutation + audit `CLASS_UPDATED` + `changed_fields=[name]`; deny (teacher) → `MC_ACTION_PERMISSION_DENIED`/`role_not_eligible` + **0 mutation + 0 ledger**. **class.assign regression** → `ok=true` + distribution + audit, **0 residue** (executor rewrite safe).

### Legacy isolation (v1.56 — MUST NOT over-claim; D367.4 SUPERSEDED)

- Executor `execute_mission_control_action` md5 `09ef5f48…` → **`da733e9a636e9efd4457123e923057dd`** (CHANGED — intentional Phase-1 consumption; "executor unchanged" no longer true).
- `_mc_transition_decision` md5 `fe0eea599db711fb8472b37a72fa25e4` **UNCHANGED**.
- `_resolve_authority` body + ACL **UNCHANGED** ({postgres}); caller-set `{execute_authority_probe, _mc_authority_gate}`.
- **KHÔNG claim:** class.assign migrated · legacy authority replaced · decision-path consumption. class.assign runtime behavior **preserved** (flag=false; regression proven).

### Live inventory (v1.56 — reconciled-to-live)

From v1.55 baseline (94/252/240/169/34/1; `mc_internal` {8/7} +1 table): public tables **94** · fns **253** (+1 `class_edit_v1`) · secdef **241** (+1) · policies **169** · triggers **34** · cron **1**; `mc_internal` **{9 fn / 8 secdef}** (+1 `_mc_authority_gate`) · mc_internal tables **1** (`authority_probe_log`). `mission_control_action_registry` **+1 col** `authority_gated boolean NOT NULL default false`. Registry: class.assign MEDIUM active ungated · class.edit LOW **active GATED** (adapter `class.edit.v1`) · authority.probe LOW active ungated. Decisions/transitions **0/0** (lifecycle dormant). Migration tail `20260815201925` → **`20260816094225`** (5 migrations incl. Mig-A external+reconcile and a class_edit array_append fix). FE main pin `2.8.5`, 0 change.

### Provenance + bug record (v1.56 — honest)

Mig-A drift: external `20260816083423` (nullable) → reconcile `20260816090017` (NOT NULL); retroactive reconciliation, no rogue assertion (D367.7). Bug: `class_edit_v1` v1 `||` array defect (22P02) caught by rehearsal **pre-production-use**, fixed `20260816094225` (array_append). Both recorded in D368.7/D368.8.

### Vocabulary status (v1.56 — nối D366.6/D367.6)

`authority_source`/`reason_codes` controlled-text/jsonb; **pg-enum still deferred** (only class.edit consumes gate in production). In-use: sources {platform_role, organization_role, none}; reason_codes {actor_unresolved, object_context_mismatch, organization_scope_mismatch, role_not_eligible, self_decision_forbidden(resolver-tier only), authority_granted}.

### Verification (v1.56)

Baseline audit PASS (D367/v1.55/tail 20260815201925); live re-pin PASS pre-each-migration; D92 VERIFY guards PASS (atomic) on Mig-A-reconcile, Mig-B, Mig-C, fix; gate 7/7 PASS; class.edit happy+deny PASS; class.assign regression PASS + 0 residue; append-only canonical integrity PASS (original bytes preserved, tail addition only).

### Next gate boundary (v1.56)

Strangler **Phase-1 DONE**. **Deferred (NOT authorized):** Phase-2 (decision paths consume resolver) · Phase-3 (legacy adapter authority migration incl. assign) · Phase-4 (legacy admin RPCs) · enum mint · `UNIQUE(user_id)` · delegation · object-scope (`child.transfer`, session/school) · multi-approver/quorum · notification · inbox UI · HIGH/CRITICAL activation · **FE wiring for class.edit**. Await explicit CTO next-milestone authorization.

**Endpoint: RULES D368 · SYSTEM_MAP v1.56 · HANDOFF V128-B9.4-IMPLEMENTATION-CLOSEOUT · backend tail `20260816094225` · FE main pin `2.8.5`.** Khối v1.55/D367 (B9.3 Authority Resolver Skeleton Reconcile) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.57 — V128-B10.1 · DECISION AUTHORITY RESOLUTION TOPOLOGY (design-freeze ratification; 0 DB mutation) — 2026-08-18

> **Authority block hiện hành** (supersede v1.56 làm current endpoint; v1.56/D368 B9.4 Phase-1 consumption LIVE = HISTORICAL SNAPSHOT BẤT BIẾN). Ratifies the CTO-approved B10.1 freeze: approver-side Decision Authority migrates into the same `_resolve_authority` primitive (Strangler **Phase-2**). Canonical rule: **RULES D369**. Live DB **unchanged** this milestone (documentation-only).

### Decision authority topology (v1.57)

Two authority questions, ONE primitive `_resolve_authority(actor, action_key, object_context, decision_context) → {eligible, authority_source, reason_codes}`:
- **Requester authority** — Gate A `_mc_authority_gate`, before `_mc_begin_action`, LIVE (D368). `decision_context` absent.
- **Approver authority** — inside decision RPCs, `decision_context` present. Today embedded (`master/sub + same-school + not-self`); B10.1 freezes migration to resolver consumption.

Single-source = **policy verdict** (D366.4), not sole authorization — RLS + adapter domain-safety + `_mc_transition_decision` still enforce independently.

### Two-phase execution ordering (v1.57 — Option B frozen)

```
Request → Requester Authority (resolver) ─DENY→ 0 ledger·0 mutation
        → begin → decision-required?(risk_level)
             ├ LOW/MED  → adapter → commit                        (auto)
             └ HIGH/CRIT → open decision (park; ledger processing)
                    → Approver Authority (resolver, phase) ─DENY→ no transition
                    → _mc_transition_decision(approved)  [B8, UNCHANGED]
                    → resume → adapter → commit
```
Option A (lifecycle-then-single-authority) **rejected** — no seam to gate the approve action; would pollute the B8 mutation-only boundary (Risk 6); permits inbox pollution / orphan evidence.

### decision_context contract (v1.57)

Minimal immutable = **`{ decision_phase, requester_id }`** on top of `(actor, action_key, object_context)`. `requester_id` meaningful only at `phase ∈ {approve, reject}`; bound at decision-open, passed as data (resolver never re-derives). `decision_id` / `request_id` are **NOT** resolver inputs (lifecycle/ledger handles, not policy inputs).

### authority_gated orthogonality (v1.57)

`authority_gated` = **verdict source = resolver** (authority axis). ORTHOGONAL to `risk_level` (decision axis: LOW/MED auto · HIGH/CRIT decision). Action space = product `{gated,ungated} × {auto,decision}`: class.edit = {gated,auto}; future child.transfer = {gated,decision}; class.assign (today) = {ungated,auto}. Meaning-A complete only AFTER B10.1 implementation migrates approver side.

### Self-approval + RPC migration boundary (v1.57)

Self-approval = phase-conditional resolver reason_code **`self_decision_forbidden`** (approve/reject FORBIDDEN; cancel ALLOWED) — single mechanism, no scattered strings. **MIGRATE:** `resolve_mission_control_decision` + cancel authority. **PRESERVE:** `_mc_open_decision`, `_mc_transition_decision` (md5 `fe0eea59…`), append-only transitions + block trigger, terminal-state protection, C-2 original-actor guard + `intent_fingerprint` UNIQUE, RLS `select_own`. Inbox → resolver-consistent (verify; full rewrite may defer). Target = **consume-only** (`_resolve_authority` body byte-unchanged; verify at impl-D1).

### Live inventory (v1.57 — UNCHANGED from v1.56; documentation-only)

public tables **94** · fns **253** · secdef **241** · policies **169** · triggers **34** · cron **1**; `mc_internal` {**9** fn / **8** secdef} + **1** table (`authority_probe_log`). `mission_control_decisions` **0** · `mission_control_decision_transitions` **0** (lifecycle dormant). Registry: class.edit LOW active GATED · class.assign MEDIUM active ungated · authority.probe LOW active ungated. Backend tail **`20260816094225`** unchanged. FE main pin `2.8.5`, 0 change. No enum/invariant minted (vocabulary draft, D366.6/D368.9).

### Honest dormancy (v1.57 — MUST NOT over-claim)

B10.1 does **NOT** arm Gate B. Approver-migration is verifiable only by rolled-back JWT-impersonated rehearsal (B7/B8 method); **NO** "proven on committed data." First committed decision = deferred (arm one contained reversible HIGH action, separate authorization).

### Deferred (v1.57)

Arming Gate B / first committed decision · object-scope (`child.transfer`, session, school) · multi-approver/quorum · enum mint · `UNIQUE(user_id)` · delegation · notification · inbox UI · workflow builder · AI recommendation · FE class.edit · Strangler Phase-3 (adapter authority incl. `assign_class_distribution`) · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D369 · SYSTEM_MAP v1.57 · HANDOFF V128-B10.1-DESIGN-FREEZE · backend tail `20260816094225` · FE main pin `2.8.5`.** Khối v1.56/D368 (B9.4 Authority Resolver Consumption — Phase-1 LIVE) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.58 — V128-B10.1 · DECISION AUTHORITY RESOLUTION — IMPLEMENTATION CLOSEOUT (Phase-2 runtime-verified) — 2026-08-19

> **Authority block hiện hành** (supersede v1.57; v1.57/D369 B10.1 design-freeze ratification = HISTORICAL SNAPSHOT BẤT BIẾN). Strangler **Phase-2 LIVE**: approver-side Decision Authority consumes `_resolve_authority` in production. Canonical rule: **RULES D370**.

### Decision authority topology (v1.58 — runtime verified)

Two authority questions, ONE primitive `_resolve_authority`, both LIVE:
- **Requester authority** — Gate A `_mc_authority_gate` (before begin), unchanged (D368).
- **Approver authority** — `resolve_mission_control_decision` (approve/reject, phase='resolve') + third-party `cancel_mission_control_decision` (phase='cancel') now **resolver-backed** (D370). Opener-cancel = ownership fast-path (not authority).

Invariants preserved: **Resolver = WHO · Decision Lifecycle = WHETHER · Adapter = HOW · Ledger = execution truth · Memory = projection.** `_mc_transition_decision` mutation-only boundary intact.

### Resolver-backed consumers (v1.58)

| Path | Authority source | phase | proven |
|---|---|---|---|
| resolve approve/reject | `_resolve_authority` | resolve | self-forbidden + role/school (matrix + R1–R4) |
| cancel (third-party) | `_resolve_authority` | cancel | scope/role (C2/C3) |
| cancel (opener) | ownership fast-path (RLS-like) | — | C1 allow |
| requester (Gate A) | `_resolve_authority` | — (absent) | D368 |

### Frozen anchors (v1.58 — regression-proven UNCHANGED)

executor `da733e9a…` · `_mc_transition_decision` `fe0eea59…` · `_resolve_authority` body `56b5e3f5…` (consume-only). Adapters/registry/policies/triggers/tables unchanged.

### Live inventory (v1.58 — function-body-only delta)

public tables **94** · fns **253** · secdef **241** · policies **169** · triggers **34** · cron **1**; `mc_internal` {**9** fn / **8** secdef} + **1** table. Changed: `resolve_mission_control_decision` `bb79a521…` · `cancel_mission_control_decision` `924c3f9a…` (ACL `{authenticated,postgres,service_role}` preserved). `mission_control_decisions` **0** · `mission_control_decision_transitions` **0** (dormant). Registry: class.edit LOW active GATED · class.assign MEDIUM active ungated · authority.probe LOW active ungated. Migration tail **`20260819043435`**. FE main pin `2.8.5`.

### Honest dormancy (v1.58 — MUST NOT over-claim)

Verified through resolver-direct matrix + rolled-back RPC rehearsal only. Lifecycle **DORMANT** (0/0); no committed production decision; no HIGH/CRITICAL action activated. First committed decision = deferred.

### Deferred (v1.58)

Inbox projection alignment (`get_mission_control_decision_inbox` platform-role divergence) · first committed decision lifecycle run (arm reversible HIGH action) · `child.transfer` / object-scope · quorum/multi-approver · enum mint · `UNIQUE(user_id)` · delegation · notification · inbox UI · FE · Strangler Phase-3 (`assign_class_distribution`) · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D370 · SYSTEM_MAP v1.58 · HANDOFF V128-B10.1-IMPLEMENTATION-CLOSEOUT · backend tail `20260819043435` · FE main pin `2.8.5`.** Khối v1.57/D369 (B10.1 Decision Authority Resolution Design Freeze Ratification) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.59 — V128-B11.1 · DECISION INBOX AUTHORITY RECONCILIATION (inbox eligibility resolver-backed; decision-projection single-source) — 2026-08-19

> **Authority block hiện hành** (supersede v1.58; v1.58/D370 B10.1 implementation closeout = HISTORICAL SNAPSHOT BẤT BIẾN). Decision **inbox visibility** now consumes `_resolve_authority` (phase='resolve'); all decision-path authority questions single-source through the resolver. Canonical rule: **RULES D371**.

### Decision authority topology (v1.59 — runtime verified)

Three authority questions, ONE primitive `_resolve_authority`, all LIVE:
- **Requester authority** — Gate A `_mc_authority_gate` (before begin), unchanged (D368).
- **Approver authority** — `resolve_mission_control_decision` (approve/reject, phase='resolve') + third-party `cancel_mission_control_decision` (phase='cancel') resolver-backed (D370). Opener-cancel = ownership fast-path (not authority).
- **Inbox eligibility** — `get_mission_control_decision_inbox` per-candidate `_resolve_authority` (phase='resolve') resolver-backed (D371). Coarse row-selection (pending/class/expire-sweep/self-exclusion) beneath the resolver.

Invariants preserved: **Resolver = WHO · Decision Lifecycle = WHETHER · Adapter = HOW · Ledger = execution truth · Memory = projection.** `_mc_transition_decision` mutation-only boundary intact.

### Resolver-backed consumers (v1.59)

| Path | Authority source | phase | proven |
|---|---|---|---|
| resolve approve/reject | `_resolve_authority` | resolve | self-forbidden + role/school (D370) |
| cancel (third-party) | `_resolve_authority` | cancel | scope/role (D370) |
| cancel (opener) | ownership fast-path (RLS-like) | — | C1 allow (D370) |
| requester (Gate A) | `_resolve_authority` | — (absent) | D368 |
| **inbox eligibility** | `_resolve_authority` | resolve | matrix 5/5 + inbox↔resolver equivalence 5/5 (B11.1) |

### Frozen anchors (v1.59 — regression-proven UNCHANGED)

executor `da733e9a…` · `_mc_transition_decision` `fe0eea59…` · `_resolve_authority` body `56b5e3f5…` (consume-only) · resolve `bb79a521…` · cancel `924c3f9a…` · expire `c57c40b8…`. Adapters/registry/policies/triggers/tables unchanged.

### Live inventory (v1.59 — function-body-only delta)

public tables **94** · fns **253** · secdef **241** · policies **169** · triggers **34** · cron **1**; `mc_internal` {**9** fn / **8** secdef} + **1** table. Changed: `get_mission_control_decision_inbox` `af21682d…` (from `47ca0947…`; ACL `{authenticated,postgres,service_role}` preserved, anon absent). `mission_control_decisions` **0** · `mission_control_decision_transitions` **0** (dormant). Registry: class.edit LOW active GATED · class.assign MEDIUM active ungated · authority.probe LOW active ungated. Migration tail **`20260819092150`**. FE main pin `2.8.5`.

### Divergence closed (v1.59)

Pre-B11.1 the inbox embedded `master/sub + same-school` (no `platform_role`) → platform admins under-surfaced (empty inbox despite resolve eligibility). Two causes removed (role-list early gate + caller-school predicate NULL-trap). Post-B11.1 inbox membership == resolve verdict **by construction**. No enforcement privilege added (projection catches up to enforcement).

### Honest dormancy (v1.59 — MUST NOT over-claim)

Verified through resolver-direct matrix + rolled-back inbox↔resolver equivalence rehearsal only. Lifecycle **DORMANT** (0/0); no committed production decision; no HIGH/CRITICAL action activated. First committed decision = deferred.

### Deferred (v1.59)

First committed Gate B decision (arm reversible HIGH action) · `child.transfer` / object-scope · quorum/multi-approver · enum mint · `UNIQUE(user_id)` · delegation · notification · inbox UI · FE · Strangler Phase-3 (`assign_class_distribution` / class.assign executor authority) · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D371 · SYSTEM_MAP v1.59 · HANDOFF V128-B11.1-IMPLEMENTATION-CLOSEOUT · backend tail `20260819092150` · FE main pin `2.8.5`.** Khối v1.58/D370 (B10.1 Decision Authority Resolution — Phase-2 LIVE) = HISTORICAL SNAPSHOT (BẤT BIẾN).
...
## 🗂️ SYSTEM_MAP v1.61 — V128-B11.2-B1 · APPROVED PAYLOAD BINDING (intent_fingerprint binds the governed class.edit payload; WHAT identity) — 2026-08-20

> **Authority block hiện hành** (supersede v1.60; v1.60/D372 B11.2-A-AMENDMENT executor resume-path = HISTORICAL SNAPSHOT BẤT BIẾN). `intent_fingerprint` now binds the governed class.edit payload (`name`/`age_group_id`/`level_id`), so an approved decision authorizes a **specific** change; drift → `MC_ACTION_REQUEST_CONFLICT`. **INERT:** class.edit stays LOW, lifecycle DORMANT 0/0, Gate β CLOSED. Canonical rule: **RULES D373**.

### Fingerprint / WHAT-identity topology (v1.61 — runtime verified)
```
caller → executor (INVOKER)
       → [input_schema validate; unknown keys → INPUT_INVALID]
       → class.edit: build server-normalized fp_input {name?, age_group_id?, level_id?}   (absent≠null; deterministic jsonb; never raw text)
         class.assign: fp_input = NULL (historical formula)
       → _mc_begin_action(..., p_fp_input)   canonical.input = fp_input (edit) | {program_id, lead_teacher_id} (assign)
       → ledger.intent_fingerprint = sha256(canonical)
       → _mc_open_decision copies ledger fp → decision.intent_fingerprint (verbatim, unchanged)
       → resume: begin recomputes fp; executor guard `v_existing.intent_fingerprint IS DISTINCT FROM v_fp`
            ├─ same WHAT  → continue (decision lookup → approved → adapter)
            └─ diff WHAT  → MC_ACTION_REQUEST_CONFLICT (0 mutation, pre-adapter)
```

### Architecture invariants (v1.61)
**Resolver = WHO · Decision Lifecycle = WHETHER · Intent Fingerprint = WHAT identity · Adapter = HOW · Ledger = execution truth · Memory = projection.** The fingerprint's WHAT-identity role is now explicit: approving a decision approves a specific `(action, object, school, governed-values)` tuple.

### Changed vs unchanged (v1.61)
**Changed:** `_mc_begin_action` `f47260ef…`→**`54b5af575f1a0aa8217e6eac979d6014`** (signature +`p_fp_input jsonb DEFAULT NULL`; DEFINER preserved; ACL `{authenticated,postgres}`) · executor `2228611a…`→**`954bcc4087ea0646c60c01cc28717e79`** (INVOKER preserved; builds+passes fp_input). **Unchanged:** authority (resolver/gate/resolve/cancel/inbox), decision lifecycle (open_decision/transition/commit), adapter (class_edit_v1/assign_class_distribution), registry, decision+transition tables, FE. No schema change.

### Frozen anchors (v1.61 — regression-proven UNCHANGED)
commit `ce36c5fe…` · open_decision `b376edd7…` · transition `fe0eea59…` · resolver `56b5e3f5…` · gate `bc5ea1a2…` · class_edit_v1 `63f3ab5a…` · resolve `bb79a521…` · cancel `924c3f9a…` · inbox `af21682d…` · assign_class_distribution `03a1510b…`.

### Canonicalization semantics (v1.61)
Governed keys only (`name`/`age_group_id`/`level_id`) · **absent ≠ explicit null** (key present iff in p_input) · name as adapter consumes (untrimmed) · uuid-canonicalized age/level · deterministic jsonb (key/text order irrelevant) · raw `p_input::text` never hashed · no `jsonb_strip_nulls` · unknown keys rejected upstream. **class.assign fingerprint byte-identical pre/post** (A3).

### Verification (v1.61 — rolled-back rehearsals, zero residue)
class.assign: A1 conflict PASS · A2 fresh success PASS · A3 fp byte-identical. class.edit: E1 identical PASS · E2 changed value → `REQUEST_CONFLICT` · E3 key-order PASS · E4 one-field → `REQUEST_CONFLICT` · E5 unknown → `INPUT_INVALID` · E6 absent≠null (different fp) · E7 adapter validation unchanged. Flag 1 replay → no duplicate mutation.

### Honest dormancy (v1.61 — MUST NOT over-claim)
Verified through rolled-back rehearsal + runtime hash/ACL only. Lifecycle **DORMANT** (0/0); class.edit LOW; class.edit action_requests 0; no committed production decision; no HIGH/CRITICAL activated; one-way door NOT opened.

### D372.8 preconditions (v1.61)
**Flag 1 (approved-resume idempotency) = CLOSED/PASS** · **Flag 2 (approved WHAT payload binding) = CLOSED/PASS.** Technical preconditions for the first committed governed decision are resolved; **Gate β activation remains a separate, NOT-authorized CTO gate** (V128-B11.2-B2).

### Deferred (v1.61)
**Gate β (V128-B11.2-B2, separate re-gate):** dedicated synthetic fixture seed (G4; pilots off-limits) · registry class.edit LOW→HIGH · first COMMITTED decision (opens one-way door 0/0→1/2 permanent) · data rollback + disarm + Gate β closeout. **Beyond:** child.transfer / object-scope · Strangler Phase-3 (assign_class_distribution) · quorum/multi-approver · enum mint · UNIQUE(user_id) · delegation · notification · inbox UI · FE wiring for class.edit · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D373 · SYSTEM_MAP v1.61 · HANDOFF V128-B11.2-B1-IMPLEMENTATION-CLOSEOUT · backend tail `20260820021437` · FE main pin `2.8.5`.** Khối v1.60/D372 (B11.2-A-AMENDMENT Executor Resume-Path class.edit Support) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.62 — V128-B11.2-B2 · FIRST COMMITTED GOVERNED DECISION (governance lifecycle ACTIVE — proven by committed evidence; one-way door OPENED 0/0 → 1/2 PERMANENT) — 2026-08-20

> **Authority block hiện hành** (supersede v1.61; v1.61/D373 B11.2-B1 approved-payload binding = HISTORICAL SNAPSHOT BẤT BIẾN). The governed decision lifecycle is now **ACTIVE / PROVEN BY COMMITTED EVIDENCE** — the first committed governed decision was approved and resumed end-to-end; lifecycle is permanently **1 decision / 2 transitions** (no longer DORMANT). `class.edit` was reversibly armed LOW→HIGH for the proof and disarmed back to LOW. No function-body/schema/migration change (tail `20260820021437`). Canonical rule: **RULES D374**.

### Governance lifecycle — NOW ACTIVE (v1.62 — committed, not dormant)
- **decisions 1 (approved) · transitions 2** (`NULL→pending`, `pending→approved`) — **PERMANENT**, append-only via `trg_block_decision_transition_mutation`. The 0/0 dormant era is closed; **never 0/0 again**.
- End-to-end flow proven on committed data:
```
requester R → execute_mission_control_action(class.edit, C, {school_id:S}, {name}, reqid)   [HIGH, authority_gated]
  → _mc_authority_gate/_resolve_authority (R organization_role → eligible)
  → _mc_begin_action (server-built fp_input binds governed name → intent_fingerprint; ledger 'processing')
  → _mc_open_decision (risk HIGH → decision 'pending' + transition NULL→pending; opener enumeration counts P)
  → resolve_mission_control_decision (P; _resolve_authority phase='resolve' → eligible; not self)
      → _mc_transition_decision (pending→approved; FOR UPDATE; append transition)
  → R resume SAME reqid, EXACT payload → decision approved + actor match
      → class_edit_v1 (UPDATE classes.name; write_audit_log CLASS_UPDATED)
      → _mc_commit_action (FOR UPDATE; status 'processing'→'completed')
```

### First committed decision evidence (v1.62 — immutable)
- Decision `5d3b8897-ab2b-45ab-ac44-359e335d8ea3` — approved · requested_by R `…0ac7…001` · decided_by P `…0ac7…002` · object C `…c1a5…001` · risk HIGH · fp `28d71f14af533edc039ba09a3f8010932ca9806d2cc556e343833d28123ae0d1`.
- Transitions: `5d6e1297-2a47-40f7-8acb-30aec7cdc495` (NULL→pending / R / 06:04:16) · `ee4ac2cd-5044-4b8e-bdab-672fc29ce92d` (pending→approved / P / 06:05:26).
- Proof request `b112b2b2-9e00-4000-8000-0000000000f1` — completed (06:06:08). Audit `0a9f3528-dcd8-4da2-8ffe-2dcec2ca594b` (CLASS_UPDATED, actor R, entity C, `changed_fields:[name]`).

### Committed invariant proofs (v1.62)
- **Intent-fingerprint / WHAT-identity (D372.8 Flag 2):** drifted governed payload on resume → `MC_ACTION_REQUEST_CONFLICT`, 0 mutation (pre-adapter guard). Approved WHAT = executed WHAT, proven on committed data.
- **At-most-once effective mutation (D372.8 Flag 1):** replay of the completed request → `MC_ACTION_EXECUTION_FAILED` (`_mc_commit_action` `commit_core_not_processing` → executor subtxn rollback of the re-mutation); no duplicate mutation/audit/decision/transition.

### Arm / disarm posture (v1.62)
- `class.edit` LOW→HIGH (arm, proof window) → HIGH→LOW (disarm). **Final registry posture: class.edit LOW · active · authority_gated=true · adapter `class.edit.v1` · object `class` · audit `CLASS_UPDATED`.** Registry activation reversible; committed evidence irreversible. class.edit is NOT permanently HIGH.

### Permanent synthetic proof fixture (v1.62 — KEEP)
- Ru `b92bfb6d-2745-4cae-84c5-8a05bc9f81c5` (auth.users, **Owner-created via Auth Admin**) · Pu `5029da7b-ef4d-47dc-b69f-648aae12c9c2` (auth.users) · S `b112b2b2-5c00-4000-8000-000000000001` · R `b112b2b2-0ac7-4000-8000-000000000001` (→Ru, master_admin@S) · P `b112b2b2-0ac7-4000-8000-000000000002` (→Pu, master_admin@S) · C `b112b2b2-c1a5-4000-8000-000000000001` (AFTER: `MC-B11.2-B2-FIXTURE-PROOF`). Evidence-bound provenance — do not delete/reset/revert (Option A).

### Auth identity dependency (v1.62 — corrected canonical fact)
- **`public.profiles.user_id` → `auth.users(id)` ON DELETE SET NULL** (authoritative `pg_constraint`; `information_schema.constraint_column_usage` privilege-omits this cross-schema FK — do not rely on it for FK detection). Synthetic profiles require real `auth.users` ids; Ru/Pu were Owner-created via Auth Admin; **no raw `auth.users` SQL in B2**. 9 tables FK `auth.users` (8 CASCADE + profiles SET NULL); no `auth.users` trigger.

### Frozen anchors (v1.62 — regression-proven UNCHANGED)
begin `54b5af57…` · executor `954bcc40…` · commit `ce36c5fe…` · open_decision `b376edd7…` · transition `fe0eea59…` · resolver `56b5e3f5…` · gate `bc5ea1a2…` · class_edit_v1 `63f3ab5a…` · resolve `bb79a521…` · inbox `af21682d…` · assign_class_distribution `03a1510b…`. Migration tail `20260820021437`. No function-body/schema change in B2. Inventory unchanged (94 tables · 253 fns · 241 secdef · 169 policies · 34 triggers · 1 cron; mc_internal 9fn/8secdef +1 table) + permanent synthetic fixture (2 auth.users · 1 school · 2 profiles · 1 class).

### Honest dormancy → ACTIVE (v1.62)
All historical "DORMANT 0/0 / one-way door NOT opened" lines in v1.x blocks are immutable HISTORICAL SNAPSHOTS. Current live governance state = **1 decision / 2 transitions, ACTIVE**.

### Deferred (v1.62 — each needs explicit CTO authorization)
child.transfer / object-scope · Strangler Phase-3 (assign_class_distribution) · reject/expire/cancel committed proof · multi-approver/quorum · enum/taxonomy mint · `UNIQUE(user_id)` · delegation · notification · decision inbox UI · FE wiring for class.edit · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D374 · SYSTEM_MAP v1.62 · HANDOFF V128-B11.2-B2-FIRST-COMMITTED-DECISION-CLOSEOUT · backend tail `20260820021437` · FE main pin `2.8.5`.** Khối v1.61/D373 (B11.2-B1 Approved Payload Binding) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.63 — V128-B12 · CLASS.ASSIGN AUTHORITY STRANGLER PHASE-3 (assign_class_distribution WHO now resolver-sourced; both write paths converge; class.assign stays MEDIUM ungated) — 2026-08-20

> **Authority block hiện hành** (supersede v1.62; v1.62/D374 B11.2-B2 first committed decision = HISTORICAL SNAPSHOT BẤT BIẾN). `public.assign_class_distribution` now consumes `_resolve_authority` for WHO-authority (consume-only; resolver byte-unchanged). Both the Mission Control and direct School-Portal write paths converge on the single resolver. `class.assign` stays MEDIUM · authority_gated=false; decision lifecycle stays **1/2** (no new decision). Canonical rule: **RULES D375**.

### class.assign authority topology (v1.63 — runtime verified)
```
Mission Control:  caller → execute_mission_control_action('class.assign')   [authority_gated=false ⇒ no gate; MEDIUM ⇒ auto-execute fresh]
                          → assign_class_distribution
                              → mc_internal._resolve_authority(current_profile(),'class.assign',{school_id},null)   ⭐ WHO
                              → domain safety (entitlement · lead-teacher · duplicate)                              HOW
                              → INSERT class_distributions · write_audit_log(CLASS_ASSIGNMENT_CREATED)
Direct School Portal: authenticated → assign_class_distribution → SAME resolver → SAME domain safety → SAME insert
```
`class_distributions` has SELECT-only RLS and no INSERT grant → these two SECURITY DEFINER RPC paths are the ONLY creation paths; both now single-source WHO through the resolver.

### Architecture invariants (v1.63)
**Resolver = WHO · Adapter = HOW · Registry = WHAT/risk metadata · Decision Lifecycle = WHETHER · Intent Fingerprint = WHAT identity · Ledger = execution truth · Memory = projection.** class.assign creation authority is now resolver-sourced across all callers.

### Changed vs unchanged (v1.63)
**Changed:** `assign_class_distribution` `03a1510b…`→**`069cb93c0f7de2e5a933662b8cb9e644`** (embedded WHO block → resolver consume-only; domain safety byte-preserved; SECURITY DEFINER/search_path/ACL preserved; external error `not_authorized_for_school` preserved). **Unchanged:** resolver `56b5e3f5…` (consume-only), executor `954bcc40…`, authority_gate `bc5ea1a2…`, decision lifecycle (begin/commit/open/transition), class_edit_v1, resolve/inbox, registry, decision+transition tables, FE. No schema change.

### No privilege broadening (v1.63)
Legacy `is_admin()` role-set ≡ resolver platform-role set (byte-identical); org branch (`master_admin`/`sub_admin` same-school) equivalent. Verdict matrix: master same-school ALLOW · sub_admin same-school ALLOW (code; 0 live) · cross-school DENY (organization_scope_mismatch) · teacher/parent DENY (role_not_eligible) · platform ALLOW (platform_role).

### Current state (v1.63)
- **class.assign:** MEDIUM · active · **authority_gated=false** · adapter class.assign.v1 · resolver-sourced WHO.
- **class.edit:** LOW · active · **authority_gated=true** (armed HIGH then disarmed at B11.2-B2).
- **decision lifecycle:** **1 decision / 2 transitions** (permanent; B11.2-B2 one-way door). No new decision in B12.
- **backend tail:** **20260820070120** (migration `v128_b12_assign_class_distribution_resolver_who`).

### Adjacent deferred divergence (v1.63)
`public.set_distribution_lead` md5 `1dcc700f…` UNCHANGED — still carries the legacy embedded WHO idiom on the lead-edit path. **KNOWN / DEFERRED** to a separate future gate. B12 closes class.assign **creation** authority divergence only; it is NOT full class-distribution authority convergence.

### Verification (v1.63 — authority matrix + rolled-back rehearsals, zero residue)
Authority ALLOW/DENY matrix matches pre-B12. Domain: class_not_found · subject_not_entitled · lead_teacher_invalid · distribution_exists · fresh success + CLASS_ASSIGNMENT_CREATED. MC path: MEDIUM auto-execute, no decision, ledger unchanged. Residue: class_distributions 21 · class.assign action_requests 12 · lifecycle 1/2. First migration attempt failed on VERIFY typo → atomic rollback (zero residue); corrected re-apply PASS.

### Honest dormancy → still ACTIVE (v1.63)
Governance lifecycle remains ACTIVE at **1/2** (from B11.2-B2). B12 added no decision. Historical "DORMANT 0/0" lines remain immutable HISTORICAL SNAPSHOTS.

### Deferred (v1.63 — each needs explicit CTO authorization)
set_distribution_lead resolver consumption · child.transfer / object-scope · multi-approver/quorum · reject/expire/cancel committed proof · enum mint · UNIQUE(user_id) · delegation · notification · decision inbox UI · FE wiring for class.edit · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D375 · SYSTEM_MAP v1.63 · HANDOFF V128-B12-IMPLEMENTATION-CLOSEOUT · backend tail `20260820070120` · FE main pin `2.8.5`.** Khối v1.62/D374 (B11.2-B2 First Committed Governed Decision) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.64 — V128-B13.1 · AUTHORITY VOCABULARY (class.assignment.lead.edit registered; vocabulary-only · non-dispatch; 0 function change; 1 registry INSERT) — 2026-08-20

> **Authority block hiện hành** (supersede v1.63; v1.63/D375 B12 class.assign Strangler Phase-3 = HISTORICAL SNAPSHOT BẤT BIẾN). Registers the CTO-ratified authority vocabulary key **`class.assignment.lead.edit`** as a **vocabulary-only, non-dispatch** row in `public.mission_control_action_registry`. No function body changed; the resolver is byte-unchanged and does not branch on `action_key`. B13.1 establishes **vocabulary provenance, not behaviour change** — the `set_distribution_lead` migration is B13.2. Canonical rule: **RULES D376**.

### Action registry topology (v1.64)
```
mission_control_action_registry  (4 rows)
  ├─ class.assign                 object_type class        · MEDIUM · ungated · adapter class.assign.v1     · DISPATCHABLE (production, resolver-sourced WHO)
  ├─ class.edit                   object_type class        · LOW    · GATED   · adapter class.edit.v1       · DISPATCHABLE (production)
  ├─ authority.probe              object_type authority    · LOW    · ungated · adapter authority.probe.v1  · DISPATCHABLE (synthetic resolver harness)
  └─ class.assignment.lead.edit   object_type class_assignment · MEDIUM · ungated · adapter NONE (NULL)     · NON-DISPATCH (vocabulary-only)   ⭐ NEW
```

### New row — class.assignment.lead.edit (v1.64)
- **id:** `6ad88885-7b1a-49be-9da4-6d032e222adb` · **object_type:** `class_assignment` · **capability:** `class.assignment.lead.edit`.
- **adapter_key / execution_mode / input_schema / required_context:** NULL (all four) → **dispatchable = NO**.
- **risk_level:** MEDIUM (INERT on non-dispatch row) · **authority_gated:** false · **status:** active · **audit_event:** `distribution_lead_changed`.
- **metadata:** `{non_dispatch:true, authority_vocabulary_only:true, consumed_by:"public.set_distribution_lead", gate:"V128-B13.1"}`.
- **consumer planned:** `public.set_distribution_lead` (B13.2 resolver consumption).

### Non-dispatch proof (v1.64)
`_mc_lookup_action('class','class.assignment.lead.edit')` → `found=false / dispatchable=false` (executor hardcodes object_type `class`). `_mc_lookup_action('class_assignment','class.assignment.lead.edit')` → `found=true / dispatchable=false` (adapter_key NULL). `get_available_actions('class',…)` filter (`object_type='class' AND status='active' AND adapter_key IS NOT NULL`) still exposes exactly **class.assign, class.edit**. `class_assignment` is absent from `mission_control_object_registry` (orphaned object_type — accepted OD-01 pattern, as with `authority`).

### Architecture invariants (v1.64)
**Resolver = WHO (currently action_key-INDEPENDENT) · Adapter = HOW · Registry = WHAT/risk + authority vocabulary · Decision Lifecycle = WHETHER · Intent Fingerprint = WHAT identity · Ledger = execution truth · Memory = projection.** `_resolve_authority` accepts `action_key` but does not branch on it; the registry is not read by the resolver. Therefore B13.1 changes no verdict and no runtime behaviour — it records vocabulary provenance for B13.2 to consume.

### Changed vs unchanged (v1.64)
**Changed:** `mission_control_action_registry` +1 row (`class.assignment.lead.edit`); total **3 → 4**. **Unchanged (byte-identical):** `_resolve_authority` `56b5e3f5…`, `execute_mission_control_action` `954bcc40…`, `assign_class_distribution` `069cb93c…`, `set_distribution_lead` `1dcc700f…`, `class_edit_v1` `63f3ab5a…`, `_mc_lookup_action`, `get_available_actions`, all decision-lifecycle fns, the existing three registry rows, decision/transition tables, FE. No schema change; no migration (registry DML only).

### Current state (v1.64)
- **class.assign:** MEDIUM · active · authority_gated=false · resolver-sourced WHO.
- **class.edit:** LOW · active · authority_gated=true.
- **class.assignment.lead.edit:** MEDIUM · active · authority_gated=false · **NON-DISPATCH (adapter NULL)** · vocabulary-only · consumer planned `set_distribution_lead`.
- **set_distribution_lead:** LEGACY embedded WHO check STILL PRESENT (md5 `1dcc700f…`) — B13.2 not implemented.
- **decision lifecycle:** **1 decision / 2 transitions** (permanent; B11.2-B2 one-way door). No new decision in B13.1.
- **backend tail:** **20260820070120** UNCHANGED (registry DML, not migration).

### OD-02 disposition (v1.64) — PARTIALLY CLOSED
Closed: ≥2-action timing threshold satisfied; authority action taxonomy established (`<object>.<operation>[.<qualifier>.<verb>]`); `class.assignment.lead.edit` registered. Still deferred: pg-enum materialization (action_key remains documented free-text string-set; no enum minted). Full closure NOT claimed.

### Verification (v1.64 — registry DML; rehearsed rolled-back then guarded apply)
Pre-audit: target absent, unique `(object_type, action_key)` compatible, four operational fields nullable, no object-registry row required, class discovery 2. Rehearsal: all 19 assertions true, forced rollback → zero residue. Apply: guarded `DO` block committed on full pass. Post-apply: row present (id `6ad88885-…`), registry total 4, non-dispatch proven, class discovery unchanged (class.assign, class.edit), frozen md5s unchanged, lifecycle 1/2, B11.2-B2 evidence intact.

### Deferred (v1.64 — each needs explicit CTO authorization)
**B13.2 `set_distribution_lead` resolver consumption (next)** · child.transfer / object-scope · multi-approver/quorum · reject/expire/cancel committed proof · pg-enum mint · UNIQUE(user_id) · delegation · notification · decision inbox UI · FE wiring for class.edit · Phase-4 (legacy admin RPCs) · 6 adjacent embedded-WHO RPCs (cancel/create/update_lesson_session, set_session_teachers, get_child_parents, provision_parent_and_link).

**Endpoint: RULES D376 · SYSTEM_MAP v1.64 · HANDOFF V128-B13.1-IMPLEMENTATION-CLOSEOUT · backend tail `20260820070120` · FE main pin `2.8.5`.** Khối v1.63/D375 (B12 class.assign Strangler Phase-3) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.65 — V128-B13.2 · CLASS-DISTRIBUTION AUTHORITY CONVERGENCE (set_distribution_lead WHO now resolver-sourced via class.assignment.lead.edit; both class-distribution write paths converge) — 2026-08-20

> **Authority block hiện hành** (supersede v1.64; v1.64/D376 B13.1 vocabulary registration = HISTORICAL SNAPSHOT BẤT BIẾN). `public.set_distribution_lead` now consumes `_resolve_authority` for WHO-authority (consume-only; resolver byte-unchanged), using the B13.1 vocabulary key `class.assignment.lead.edit`. With B12's `assign_class_distribution`, both class-distribution write paths (creation + lead-edit) single-source WHO through the resolver. **Class-distribution WHO convergence is COMPLETE.** Canonical rule: **RULES D377**.

### Class-distribution authority topology (v1.65 — runtime verified)
```
Creation (Mission Control / direct):
  caller → [execute_mission_control_action('class.assign')] → assign_class_distribution
                                                                → _resolve_authority(current_profile(),'class.assign',{school_id},null)   ⭐ WHO
                                                                → domain safety → INSERT class_distributions · CLASS_ASSIGNMENT_CREATED

Lead edit (direct School-Portal RPC):
  authenticated → set_distribution_lead
                    → _resolve_authority(v_actor,'class.assignment.lead.edit',{school_id},null)   ⭐ WHO
                    → domain safety (existence · orphan · active · lead validity)                 HOW
                    → UPDATE class_distributions.lead_teacher_id · write_audit_log('distribution_lead_changed')
```
Both paths now single-source WHO through the resolver; `set_distribution_lead` remains a **direct RPC** (no ledger, no request_id, no decision) — the vocabulary key is consumed for authority provenance only, not dispatched.

### Architecture invariants (v1.65)
**Resolver = WHO (action_key-independent today) · RPC/Adapter = HOW · Registry = WHAT/risk + authority vocabulary · Decision Lifecycle = WHETHER · Ledger = execution truth · Memory = projection.** Class-distribution creation and lead-edit authority converge on `_resolve_authority`.

### Changed vs unchanged (v1.65)
**Changed:** `set_distribution_lead` `1dcc700f…`→**`cd827fb288abff4f63a62305725448de`** (embedded WHO → resolver consume-only; domain/audit byte-preserved; ordering unchanged; SECURITY DEFINER/search_path/ACL preserved; external error `not_authorized_for_school` preserved). **Unchanged:** resolver `56b5e3f5…`, executor `954bcc40…`, assign `069cb93c…`, class_edit_v1 `63f3ab5a…`, begin `54b5af57…`, commit `ce36c5fe…`, open_decision `b376edd7…`, transition/resolve/cancel/inbox, registry (4 rows), decision/transition tables, FE. No schema change.

### No privilege broadening (v1.65 — rehearsal-proven EQUIV:true)
OLD-body and NEW-body verdicts byte-identical per actor. Matrix: master same-school ALLOW · sub_admin same-school ALLOW (resolver code; 0 live) · cross-school DENY (organization_scope_mismatch → not_authorized_for_school) · teacher/parent DENY (role_not_eligible → not_authorized_for_school) · platform ALLOW (platform_role).

### Current state (v1.65)
- **class.assign:** MEDIUM · active · authority_gated=false · resolver-sourced WHO.
- **class.edit:** LOW · active · authority_gated=true.
- **class.assignment.lead.edit:** MEDIUM · active · authority_gated=false · **vocabulary-only · NON-DISPATCH** · now consumed by `set_distribution_lead`.
- **set_distribution_lead:** WHO **resolver-sourced** (md5 `cd827fb2…`); direct RPC; domain safety local.
- **decision lifecycle:** **1 decision / 2 transitions** (permanent). No new decision in B13.2.
- **backend tail:** **20260820092306** (migration `v128_b132_set_distribution_lead_resolver_who`).

### Class-distribution WHO convergence (v1.65) — COMPLETE
Creation (`assign_class_distribution`, D375) + lead-edit (`set_distribution_lead`, D377) both resolver-sourced. The known class-distribution authority divergence is closed.

### Honest scope boundary (v1.65) — NOT system-wide
Convergence is class-distribution-surface only. Six adjacent public RPCs still carry the legacy `user_school_ids() + not_authorized_for_school` idiom and remain OUTSIDE B13: `cancel_lesson_session`, `create_lesson_session`, `update_lesson_session`, `set_session_teachers`, `get_child_parents`, `provision_parent_and_link`. Do NOT present authority convergence as system-wide.

### Verification (v1.65 — rolled-back rehearsals, zero residue; D92 guarded apply)
Rehearsal: OLD vs NEW verdict equivalence (EQUIV:true) + domain matrix (D1/D4/D5/D6 pass; D2/D3 structural) then rollback → zero residue (fixture lead intact, D5 dist reverted, audit total unchanged). Apply: 14-assertion fail-closed VERIFY passed; tail advanced; md5 changed; frozen anchors unchanged; lifecycle 1/2; B11.2-B2 intact.

### Deferred (v1.65 — each needs explicit CTO authorization)
6 adjacent embedded-WHO RPCs (session/parent surfaces) · child.transfer / object-scope · multi-approver/quorum · reject/expire/cancel committed proof · pg-enum mint (OD-02 still partial) · UNIQUE(user_id) · delegation · notification · decision inbox UI · **VISIBLE FE BUILD — Class Workspace / class.edit slice (recommended next)** · Phase-4 (legacy admin RPCs).

**Endpoint: RULES D377 · SYSTEM_MAP v1.65 · HANDOFF V128-B13.2-IMPLEMENTATION-CLOSEOUT · backend tail `20260820092306` · FE main pin `2.8.5`.** Khối v1.64/D376 (B13.1 vocabulary registration) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.66 — V128-B14 · CLASS.EDIT FE-FACING DISCOVERY PRESENTATION (name-only; get_mission_control_actions now projects class.edit alongside byte-preserved class.assign) — 2026-08-20

> **Discovery presentation block hiện hành** (supersede v1.65; v1.65/D377 B13.2 class-distribution WHO convergence = HISTORICAL SNAPSHOT BẤT BIẾN). `public.get_mission_control_actions` now projects **two** action items for `class`: `class.assign` (byte-preserved, index 0) + **`class.edit` (name-only, index 1)**. Presentation-only; executor/resolver/authority-gate/registry/decision-lifecycle/`class_edit_v1`/audit FROZEN. Canonical rule: **RULES D378**.

### FE-facing discovery topology (v1.66 — runtime verified)
- `public.get_mission_control_actions(text,uuid)` (DEFINER · STABLE · `search_path=''` · ACL `{authenticated,postgres}`): md5 **`fba330c7a2d6d36171086d9761a0bf23`** (was `3596633c…`). Per-action gate decoupled — builds `class.assign` item iff discoverable (index 0, body byte-identical), then appends `class.edit` item iff discoverable (index 1, name-only).
- `class.edit` projected item: label `Edit Class` · risk `LOW` · `input_schema` v1 · fields `[{name / control:text / value_type:text / required:false / label:'Tên lớp'}]`. risk_level + version from registry via `_mc_lookup_action`; field presentation synthesized (registry input_schema = skeleton).
- Internal gate `get_available_actions` (predicate `status='active' AND adapter_key IS NOT NULL`) exposes both class.assign + class.edit (D376); the FE-facing RPC now projects both.

### Architecture invariants (v1.66)
- Presentation ≠ authority ≠ mutation. Discovery projects visibility metadata; `_mc_authority_gate → _resolve_authority` (executor seam) remains the WHO authority; `execute_mission_control_action` remains the mutation path; `class_edit_v1` the sole class-edit adapter. All md5-frozen this milestone.
- class.edit input_schema in registry stays a skeleton (`{key,required}`); the projection is the single place FE presentation is synthesized.

### Changed vs unchanged (v1.66)
- **Changed:** `get_mission_control_actions` md5 `3596633c…`→`fba330c7…` (projects class.edit; assign item + ACL preserved).
- **Unchanged (VERIFY-asserted md5):** executor `954bcc40…` · gate `bc5ea1a2…` · resolver `56b5e3f5…` · class_edit_v1 `63f3ab5a…` · lookup `eb22ad49…` · begin `54b5af57…` · commit `ce36c5fe…` · get_available_actions `fd874243…` · get_mission_control_workspace `87244baf…`. Registry 4 rows. Lifecycle 1/2.

### Class.edit surface state (v1.66)
- Backend: DISCOVERABLE + executable (LOW · gated · resolver-sourced). Discovery now returns it.
- FE: NOT yet wired (pin `2.8.5`). ⚠️ Interim: "Edit Class" button renders in admin-only MC class workspace; submit still routed to assign builder → fails closed `missing_program` (no wrong mutation) until FE #1–#6.

### Deferred (v1.66 — each needs explicit CTO authorization)
- **VISIBLE FE BUILD — Class Workspace / class.edit FE slice #1–#6 (immediate next).**
- age_group_id / level_id edit (needs `get_mission_control_workspace` current-value exposure + `get_mission_control_actions` option hydration) · dead `renderer/bands/*` cleanup · class.edit in school/teacher portals · HIGH/decision arming · 6 legacy embedded-WHO RPCs · discovery-gate WHO convergence for get_mission_control_actions.

### Verification (v1.66 — rolled-back rehearsal + committed read-only proof; D92 guarded apply)
Rehearsal (sentinel-RAISE, zero residue): OLD vs NEW under impersonated master → `assign_preserved=true` (jsonb equality item[0]), edit item shape asserted, DDL rolled back, residue md5==`3596633c…`. Apply: fail-closed VERIFY (projection-present + 9 anchor md5 + registry invariants + ACL exact-set) passed; tail advanced `20260820092306`→`20260820122518`; md5 changed; frozen anchors unchanged; lifecycle 1/2. Post-apply committed proof: `get_mission_control_actions('class',fixture)` → `[class.assign, class.edit]`, edit name-only, `ok:true`, 0 mutation.

**Endpoint: RULES D378 · SYSTEM_MAP v1.66 · HANDOFF V128-B14-BACKEND-PRESENTATION-CLOSEOUT · backend tail `20260820122518` · FE main pin `2.8.5`.** Khối v1.65/D377 (B13.2 class-distribution WHO convergence) = HISTORICAL SNAPSHOT (BẤT BIẾN).

## 🗂️ SYSTEM_MAP v1.67 — V128-B14 · CLASS.EDIT VISIBLE FE BUILD (name-only; FE wired end-to-end via action-agnostic transport) — 2026-08-20

> **FE wiring block hiện hành** (supersede v1.66; v1.66/D378 B14 backend presentation half = HISTORICAL SNAPSHOT BẤT BIẾN, completed by this FE half; v1.65/D377 B13.2 = HISTORICAL SNAPSHOT BẤT BIẾN). The admin Mission Control class workspace now wires `class.edit` (name-only) end-to-end through an **action-agnostic transport**. Backend (executor/resolver/authority-gate/registry/decision-lifecycle/`class_edit_v1`/discovery) FROZEN — 0 change this milestone. Canonical rule: **RULES D379**.

### FE class.edit wiring topology (v1.67 — runtime + rehearsal verified)
- `ClassWorkspaceScreen` renders discovery actions (D378: `class.assign@0`, `class.edit@1`); `onRunAction(key)` opens the drawer. `active = openActionKey === 'class.edit' ? edit : assign` selects the controller for the drawer's submit/result/submitting props.
- `ActionDrawer` (shared/generic) seeds `class.edit` with `initialValues = {name: identity.label}`; raw dirty-state disables submit on no-change; **emit path unchanged (no trim)**.
- `useClassEditAction` (new; byte-mirror of `useAssignClassAction`) builds payload via `buildClassEditPayload` and executes via `missionControlAdapter.executeAction(args, parseClassEditExecuteResult)`; on `ok` invalidates workspace/actions/memory query keys.
- RPC: `execute_mission_control_action('class.edit', class_id, {school_id}, {name}, request_id)` → `class_edit_v1(class_id, {name})` → audit `CLASS_UPDATED` → `{ok:true, result:{changed_fields:[name]}}`.

### Action-agnostic transport (v1.67)
- `missionControlAdapter.executeAction<T>(args, parse)` — calls the RPC, throws on transport error, hands raw data to `parse`. Imports/knows **no** action key, **no** action-specific parser.
- Domain-supplied parsers: `parseAssignExecuteResult` (renamed from `parseExecuteResult`, body byte-identical) · `parseClassEditExecuteResult` (`changed_fields`; no `class_distribution_id`).

### Name semantics (v1.67 — matches backend contract)
- `class_edit_v1` stores `p_input->>'name'` VERBATIM; validates blank via `length(btrim(name))=0 → name_invalid` (btrim test-only). FE mirrors: blank-test (trim to decide only) in `buildClassEditPayload`; dirty-test RAW/untrimmed in `ActionDrawer`; sent value verbatim. Rehearsal: trailing-space name persisted byte-identical in-txn, then rolled back (zero residue).

### Architecture invariants (v1.67)
- **Presentation ≠ enablement ≠ authorization.** Discovery projection + embedded discovery-gate WHO = visibility; drawer dirty/blank gates = enablement; `_mc_authority_gate → _resolve_authority` (executor seam) = the sole backend WHO. No frontend role condition is a security authority.
- Transport carries no domain knowledge; parsers/builders/hooks are the domain seam. `class_edit_v1` remains the sole class-edit adapter.
- Per-controller successNonce isolation: assign and edit each own an independent handled-ref + auto-close effect; no shared nonce can cross-inherit.

### Changed vs unchanged (v1.67)
- **Changed (FE only, 7 files):** `classActions.ts` (edit key/builder/describe) · `class-adapter.ts` (rename + `parseClassEditExecuteResult`) · `missionControlAdapter.ts` (`executeAction<T>`) · `useClassEditAction.ts` (new) · `ActionDrawer.tsx` (initialValues + raw dirty) · `ClassWorkspaceScreen.tsx` (per-controller dispatch) · `useAssignClassAction.ts` (pass `parseAssignExecuteResult`).
- **Unchanged (VERIFY-asserted md5):** executor `954bcc40…` · gate `bc5ea1a2…` · resolver `56b5e3f5…` · class_edit_v1 `63f3ab5a…` · get_mission_control_actions `fba330c7…` · lookup `eb22ad49…` · begin `54b5af57…` · commit `ce36c5fe…` · workspace `87244baf…`. Registry 4 rows. Lifecycle 1/2. Backend tail `20260820122518`.

### class.edit surface state (v1.67)
- Backend: DISCOVERABLE + executable (LOW · gated · resolver-sourced) — D378.
- FE: **WIRED** (name-only). Edit Class → drawer (name field, seeded, dirty/blank gated) → verbatim `{name}` + `{school_id}` context → executes; no `missing_program` path remains. FE HEAD `6a0f3504`, pin `2.8.5`.

### Tooling incident (v1.67 — DETECTED AND CONTAINED)
- During authorized agent apply, `@lovable.dev/vite-tanstack-config` floated `2.8.5 → 2.13.1` in `package.json` (tooling STOP condition). Detected via independent `get_diff`; reverted (`6a0f3504`). Final `package.json` = 2.8.5, `bun.lock` = 2.8.5 (never changed), `routeTree.gen.ts` unchanged. Recorded as drift DETECTED + CONTAINED, not "zero drift."

### Deferred (v1.67 — each needs explicit CTO authorization)
- age_group_id / level_id FE edit (needs `get_mission_control_workspace` current-value exposure + `get_mission_control_actions` option hydration) · class.edit in school/teacher portals · dead `renderer/bands/*` cleanup · HIGH/decision arming for class.edit · discovery-gate WHO convergence for `get_mission_control_actions` · 6 legacy embedded-WHO RPCs.

### Verification / QA boundary (v1.67)
- VERIFIED: typecheck `tsgo --noEmit` exit 0 · class.edit rolled-back rehearsal (`ok:true`, `changed_fields:[name]`, verbatim, zero residue) · backend frozen anchors · assign preservation (mechanical delta) · tooling final state · diff scope (7 FE files, no routeTree/bun.lock).
- PENDING OWNER VERIFICATION: full manual browser click-through · real iPhone Safari interaction · production visual auto-close/toast. No fabricated PASS.

**Endpoint: RULES D379 · SYSTEM_MAP v1.67 · HANDOFF V128-B14-FE-IMPLEMENTATION-CLOSEOUT · backend tail `20260820122518` · FE HEAD `6a0f3504` · FE main pin `2.8.5`.** Khối v1.66/D378 (B14 backend presentation half) = HISTORICAL SNAPSHOT (BẤT BIẾN). Lifecycle baseline remains **1 decision / 2 transitions**. Browser/iPhone QA = **PENDING OWNER VERIFICATION**.


## 🗂️ SYSTEM_MAP v1.68 — D380 / HANDOFF V128-P5.5-CANONICAL-CLOSEOUT (HISTORICAL SNAPSHOT — superseded by v1.69)

**Teacher-assignment staffing & taught-attribution — CANONICALIZED (2026-08-29).** V128-P5→P5.4 backend live (8 migrations, tail `20260828150205`; product HEAD `ad30f7a789b26f844affc98d80fda3b2ba1999ef`), V128-P5.5 runtime Owner-QA gate **RATIFIED PASS**, 0 mutation this phase (documentation reconcile only). Lineage prior: Khối D379/v1.67 (V128-B14-FE) = **HISTORICAL SNAPSHOT (BẤT BIẾN)**. **Permanent lifecycle baseline remains 1 decision / 2 transitions.**

- **Live inventory (read-only at closeout):** public **95 tables · 260 functions · 247 SECURITY DEFINER · 169 policies · 35 triggers** · `mc_internal` **9 fns**. New table this series: `session_teacher_attributions`.
- **Backend migration tail:** **`20260828150205`** (`v128_p5_4_wp3b_taught_attribution_runtime_and_correction`). FE main pin `2.8.5`, unchanged. No deployment this phase.

### ⭐ Distribution / Session staffing architecture (P5.4 frozen)

```
class_distributions
  └─ lead_teacher_id ─────────────► Distribution Lead
                                     (operational lead · new-staffing snapshot source ·
                                      distribution-level visibility; NEVER rewrites session history)

lesson_sessions
  ├─ session_teacher_assignments ── Planned / Responsible (temporal append/supersede)
  │     ├─ planned                  assignment_type CHECK = {planned, responsible} ONLY
  │     └─ responsible ───────────► Start Session + Submit Journal authority (current only)
  │
  ├─ session_teachers ───────────── Supporting / additional (role ∈ {lead, assist})
  │                                  SEPARATE substrate — NOT assignment_type='supporting';
  │                                  session-scoped visibility only (no Start/Submit)
  │
  ├─ session_teacher_attributions ─ Actual Taught Teacher (historical append/supersede ledger)
  │     └─ sta_attr_current_uidx    partial UNIQUE(session_id, attribution_type) WHERE is_current
  │        attribution_source ∈ {legacy_taught_by_bootstrap, runtime_start, historical_correction}
  │
  ├─ taught_by ──────────────────── compatibility / current PROJECTION (not sole truth)
  │
  └─ session_reports ────────────── journal/report content
        (Report Actor = session_journal_submitted audit actor; IMMUTABLE)
```

**Visibility projection (P5.4-WP2, secdef):** `get_teacher_classes_in_school` / `get_teacher_session_workspace` project a session iff teacher `is_lead` OR current assignment OR `session_teachers` row on that session. Raw `lesson_sessions` SELECT RLS = `same_school(cd_school_id(...))` is the raw-table door, NOT the portal contract. `is_teacher_in_school` gates the school-scope entry.

**Governance function md5 (live anchors):** `set_distribution_lead` `19376e2a…` · `is_distribution_lead` `e8ae3bf5…` · `start_session` `257f81ab…` · `correct_session_taught_teacher` `c44bb21a…` · `get_teacher_classes_in_school` `e6272473…` · `get_teacher_session_workspace` `396598bf…` · `is_teacher_in_school` `db0ec9ed…` · `get_teacher_classes` `16146fa1…` · `submit_session_journal` `09f6b7a3…`.

**Audit events (staffing):** `distribution_lead_changed` (fail-closed) · `session_started` · `session_journal_submitted` · `session_taught_teacher_corrected` (metadata `{taught_from, taught_to, reason}` — attribution-id enrichment = OPEN non-blocking debt).

**Authority summary:** Responsible → Start/Submit · Supporting → session visibility only · Distribution Lead → distribution visibility · same-school master/sub_admin → controlled append-only historical taught-correction (platform operators excluded). **Visibility ≠ Authority.**

**P5.5 QA disposition:** QA-A PASS (reversible lead sequence + 3 fail-closed audits + assignments byte-identical) · QA-B PASS-WITH-FIXTURE-LIMITATION (support→visible + workspace authority; negative via independent projection) · QA-C PASS (authority matrix; committed real Start DEFERRED) · QA-D PASS — ROLLED-BACK REAL-RPC REHEARSAL (supersede correct, projection synced, immutability all-true, zero residue) · Negative authority PASS · **defects NONE**.

**Deferred (not defects):** committed Owner Start QA · Owner UI historical-correction flow. **Next phase:** V128-P5.FINAL — Teacher Assignment Semantic Closeout.

## ⭐ CURRENT CANONICAL ENDPOINT — D381 / SYSTEM_MAP v1.69 / HANDOFF V128-P6-BUILD-SHIP-CLOSEOUT

**Parent Session Participation Visibility — BUILT + SHIPPED (2026-08-30).** V128-P6 added a parent-facing participation read-projection (Có mặt/Đi trễ/Vắng) that is independent of the learning-outcome journey. One migration `20260830050025` (tail 184→185); FE feature + Home/Journal surfaces deployed to production `demenart.com`. Lineage prior: Khối D380/v1.68 (V128-P5.5) = **HISTORICAL SNAPSHOT (BẤT BIẾN)**. **Permanent lifecycle baseline remains 1 decision / 2 transitions.**

- **Live inventory (read-only at closeout):** public **95 tables · 261 functions · 248 SECURITY DEFINER · 169 policies · 35 triggers** · `mc_internal` **9 fns**. New object this phase: `get_parent_session_participation` (no new table/policy/trigger).
- **Backend migration tail:** **`20260830050025`** (`v128_p6_2_parent_session_participation`). FE HEAD `c010edda0e24b6d33fbd5ce6badf7dab668a7d30`, pin `2.8.5`. Deployed to production.

### ⭐ Parent participation read-projection (P6 frozen)

```
child_observations (attendance ∈ {present, late, absent})   ── the ONLY enumeration source
  │   NEVER enrollments × sessions · NULL attendance ≠ absent · ls.scheduled_at IS NOT NULL
  │   ls.state ∈ {taught_report_pending, report_pending_approval, completed}
  ▼
get_parent_session_participation(p_child_id, p_limit)   [STABLE · secdef · search_path='' · md5 69058495…]
  │   is_child_parent(p_child_id) fail-closed → unauth = {ok:false, reason:'not_authorized', participation:[], has_more:false}
  │   order: scheduled_at DESC NULLS LAST, session_id DESC · clamp p_limit 1..20 · has_more = fetch +1
  ▼
row { session_id, scheduled_at, session_title, program_name, attendance, outcome_available, journey_id }
        │                                                                   │
        │ participation truth (independent)                                 └─ outcome_available = TRUE iff a real
        ▼                                                                      child_journey exists (child_id + ref_id=session_id
   Có mặt / Đi trễ / Vắng                                                      + entry_type='session' + source='demen');
                                                                               absent ⇒ journey_id=null, outcome_available=false;
                                                                               NEVER mint child_journey for absence
```

**Participation ⟂ Learning Outcome.** `get_parent_session_outcomes` (journey-keyed) is UNCHANGED; participation is a parallel read surface, never merged; pagination is independent. `submit_session_journal` still creates `child_journey` only for `attendance ∈ {present,late}` (unchanged). **Zero `child_journey` mutation** from participation reads (44 total / 19 session, verified before==after).

**FE surfaces:** `src/features/parent/session-participation/` — `ParentParticipationLatest` on Home (`/parent`, inside `ParentSessionOutcomeSection`, rendered ABOVE the learning-outcome heading: "Sau buổi học" → *Tham gia gần nhất* → "Lần gần nhất của con"; absence never replaces an outcome) · `ParentParticipationList` on Journal (`/parent/journal`, "Tham gia buổi học", a stream SEPARATE from the memory timeline — no injection into `get_child_journal`/`buildParentTimeline`/`JourneyEntry`). Labels present=Có mặt · late=Đi trễ · absent=Vắng. Hook stale-response guard mirrors `useParentSessionOutcomes` (request key childId+limit; child-switch clears immediately; late response of child A never overwrites child B).

**Tooling / process canon (P6, reusable):** Lovable `get_diff` **hides `bun.lock`** (verify via read_file+md5, baseline `79bf3f60…`) · platform **re-floats** `@lovable.dev/vite-tanstack-config` below agent tool-calls, repeatedly → verify pin+lock md5 after every agent dispatch · agent **blocks** `supabase/migrations/` writes and `git checkout`/`git add` → materialize via shell heredoc + restore via `git show <sha>:f > f` · paste-mode can **mis-land** → always verify landed source (read_file + occurrence + diff).

**P6 QA disposition:** backend behavioral evidence PASS (present/late/absent projection · unauthorized shape · limit clamp 1..20 · deterministic ordering · `demen` provenance · zero child_journey mutation) · FE Owner re-QA PASS on production (`/parent` placement + independence) · one intra-arc defect (P6.3.2 duplicate `{latest && (`) found + fixed via paste-mode · **live defects NONE**.

**Deferred / limits:** absent-case live UI QA (no absent-linked auth parent in pilot). **Next phase:** open — P7 / follow-on parent or operational capability; *Principal Action Loop* remains ranked-#2 DEFER pending future-session runtime evidence.
