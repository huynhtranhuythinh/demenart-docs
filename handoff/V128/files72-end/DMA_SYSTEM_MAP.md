...
## 🧭 V128-B9.3 — AUTHORITY RESOLVER SKELETON — RECONCILED (v1.55 · out-of-canonical live layer canonicalized · vocabulary minted · 0 new migration)

**Scope:** Reconciles + canonicalizes the Authority Resolver skeleton applied to live `2026-08-15T20:19Z` (migration `v128_b93_authority_resolver_skeleton`, tail `20260815201925`, Owner account) whose closeout was skipped → live↔canonical drift (D112). Skeleton audited directly, confirmed **conformant to the D366 frozen contract**, verify matrix **10/10 PASS** (read-only rehearsal, zero residue). Vocabulary **minted** (documented string-set). **0 new migration** — governance canonicalization + mint only. Detail: RULES **D367**.

### Authority Resolver — from frozen contract to live primitive (v1.55)

D366 froze the contract; B9.3 skeleton is the **first implementation** of the primitive — as an **isolated probe**, not yet wired into the main executor:

```
Actor → Identity → Role/Scope → Object Context → Capability
  → mc_internal._resolve_authority(actor, action_key, object_context, decision_context)
  → { eligible, authority_source, reason_codes }        ← LIVE · STABLE · DEFINER {postgres}
  → public.execute_authority_probe (isolated probe adapter → authority_probe_log)
```

The executor (`execute_mission_control_action`, md5 `09ef5f48…` UNCHANGED) and lifecycle controller (`_mc_transition_decision`, md5 `fe0eea59…` UNCHANGED) are **untouched** — zero legacy touch. `child.transfer` (first real governed action through the executor) is **NOT built**.

### Live objects (v1.55 — reconciled, canonical)

- `mc_internal._resolve_authority(uuid,text,jsonb,jsonb)` — STABLE SECURITY DEFINER `search_path=''`, md5 `56b5e3f5a14360990b51c080ff1da1cb`, ACL EXECUTE `{postgres}`. Read-only policy-verdict primitive.
- `public.execute_authority_probe(uuid,uuid)` — SECURITY DEFINER `search_path=''`, md5 `59e7c5d4a8d1dda25d9466500da82d13`, ACL `{authenticated,postgres}`. Isolated probe adapter.
- `mc_internal.authority_probe_log` — RLS on · 0 policy · grants `{postgres}` (DEFINER-write-only) · 1 row (smoke).
- registry `authority.probe` — object `authority` · LOW · active · adapter `authority.probe.v1` · required_context `{school_id}`.

### Vocabulary MINTED (v1.55 — documented string-set, closes D366.6)

`authority_source` canonical: exercised `platform_role · organization_role · none`; reserved-future `explicit_permission · assignment · delegation`. `reason_codes` canonical: exercised `authority_granted · actor_unresolved · self_decision_forbidden · object_context_mismatch · organization_scope_mismatch · role_not_eligible`; reserved-future `capability_not_granted`. Minted as **documented text** (NOT PG enum) — new codes append via canonical, no `ALTER TYPE` churn. Supersedes D366.6 deferral (which superseded B9.1 §8.7).

### Verify matrix (v1.55 — 10/10 PASS, zero residue)

C1 actor_unresolved · C2 self_decision_forbidden · C3 platform_role grant · C4 organization_role same-school grant · C5 organization_scope_mismatch cross-school · C6 object_context_mismatch null-school · C7 role_not_eligible (non-org role) · C8 ACL-not-leaked · C9 STABLE+SECDEF · C10 zero-residue (log unchanged). Read-only resolver calls against live fixtures; no writes.

### Live inventory (v1.55)

public tables **94** · fns **252** · secdef **240** · policies **169** · triggers **34** · cron **1**; `mc_internal` {**8** fn / **7** secdef} + table `authority_probe_log`. Registry rows **3**. `mission_control_decisions` 0 · `mission_control_decision_transitions` 0 · `authority_probe_log` 1. Governed decision path dormant (class.assign MEDIUM → auto). Migration tail **`20260815201925`**. FE main pin `2.8.5`, 0 change.

### Governance lesson (v1.55)

Migration-in-live-absent-from-canonical = drift (D112); every `apply_migration` MUST pair with same-session canonical closeout (D367.7). Top-of-file endpoint pointers stale (`D342/v1.30`, `D358/v1.46`) — boot pins from the **latest block**, not the top header; refresh headers at next touch.

### Next gate

**`child.transfer`** — first real governed business action wired through `execute_mission_control_action` (Strangler Phase 1→2). **NOT authorized by this block.** Deferred: resolver consumption by decision paths · `UNIQUE(user_id)` constraint decision (partial unique index live) · delegation · object-scope expansion · quorum · notification · inbox UI · HIGH/CRITICAL activation. Await CTO authorization + APPLY.

**Rollback:** DROP `execute_authority_probe` + `_resolve_authority` + TABLE `authority_probe_log` + DELETE registry `authority.probe` + NOTIFY pgrst (1 log row → trivial repair). **Endpoint: RULES D367 · SYSTEM_MAP v1.55 · HANDOFF V128-B9.3-SKELETON-RECONCILE · backend tail `20260815201925` · FE main pin `2.8.5`.** Khối v1.54/D366 (B9.2 Authority Contract) = HISTORICAL SNAPSHOT (BẤT BIẾN).
