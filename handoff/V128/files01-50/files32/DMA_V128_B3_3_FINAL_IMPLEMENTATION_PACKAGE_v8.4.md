# V128-B3.3 — FINAL IMPLEMENTATION PACKAGE v8.4

> **Narrative-only correction of exact Package v8.2 after a withheld final audit.** The correction is limited to current-migration identity wording and the operative package verdict/next action. §5 and §9 are unchanged byte-for-byte. No redesign · no architecture change · no migration business-logic change · no apply · no production mutation · no D356 canonicalization · no HANDOFF B3.3 · no B3.4/B4.
...
> **Exact base artifact:** Package v8.2, SHA-256 `77f37248d399cb570c10d7fa50c2b6133ef0cc94ffd84234e0e611aba3089905`, 101,791 bytes, 1,317 lines (verified byte-identical before editing).
> **Provenance:** v8.4 is an exact-base narrative-only patch of v8.3 (single operative sentence corrected in §2; §23 verdict retargeted v8.3→v8.4). The §5 resolver and §9 migration fenced body are unchanged byte-for-byte; migration SHA-256 re-verified identical.
> **Version-governance note:** the retained v8.2-A–K correction record below is historical evidence. Its v8.2-K verdict is explicitly non-operative; the sole operative package verdict is v8.4 §23.

---

## v8.2-A. INCIDENT / STOP RECORD + AUDIT CORRECTION

- Owner issued a prior one-shot `AUTHORIZED — APPLY V128-B3.3`; execution stopped before migration and that authorization is consumed.
- **STEP 1 FINAL DRIFT GATE (body §4): PASS.**
- **Historical STEP 2 STOP record:** an execution report attributed the STOP to a non-admin fixture identity resolving `NULL`.
...
- The prior Owner authorization is **consumed and invalid** for v8.1/v8.2.

**Incident-record accuracy correction (Owner-ratified; tightened in v8.2).** The historical STOP could not have been emitted by the literal v8/v8.1 §5 resolver because that resolver did not query either identity fixture. The STOP description is retained as an execution incident report, not claimed as output of that literal resolver. Live audit establishes the precise identity facts:
...
- `…-4f56-…` is a **historical, now-dead** identity (absent from `auth.users` and `profiles`); a resolver that actually queried it would fail. It does **not** appear literally anywhere in the frozen v8 artifact.
...
Package v8.1 already froze the corrected non-admin UID. The sole v8.2 correction is verification governance: replace §5's observational SELECT with a literal fail-closed `DO` resolver; update claims that describe that resolver. **Untouched (verified):** migration block · frozen PRE golden (§6) · PROGRAM contract (§3) · all fixture values · BLOCK-3 · harness semantics · drift-gate semantics (§4) · POST harness semantics (§12) · P1–P14 and P14 assignment scope · structural verifier (§11).

## v8.2-C. FRESH LIVE RE-PIN + DRIFT VERDICT (read-only)
...
**DRIFT VERDICT: NO MATERIAL DRIFT — v8.2 ISSUED.**

## v8.2-D. IDENTITY FIXTURE EVIDENCE (empirical)
...
The corrected §5 additionally asserts the frozen admin UID exists in `auth.users`, has exactly one `profiles.user_id` row, and returns `public.is_admin()=true` under authenticated JWT claims. For both identities, the JWT `sub` is also checked through `auth.uid()` before posture is accepted.

## v8.2-E. MIGRATION HASH IDENTITY PROOF
...
| **v8.1 migration block** | `77879867f43de276321443ca87593b4baab56bf3f1a1faa0335e23b63bf2c214` | 14,722 | exact base |
| **v8.2 migration block** | `77879867f43de276321443ca87593b4baab56bf3f1a1faa0335e23b63bf2c214` | 14,722 | **IDENTICAL — `cmp` byte-for-byte** |

The migration block contains **zero** fixture UIDs, so the resolver correction is structurally isolated from it. `MIGRATION BYTE DRIFT — v8.2 NOT ISSUED` **not triggered.**

## v8.2-F. v8.1 → v8.2 ARTIFACT DIFF PROOF

The normalized diff is confined to: (1) v8.2 identity/evidence/protocol narrative; (2) §5 resolver SQL and its observed-result text; (3) §21/§22 self-containment and execution claims. There are **no fixture-value changes** and no changes to §3, §4, §6–§20 except the explicit §5-related claims in §21/§22. The §9 fenced migration body is byte-identical. PRE golden, PROGRAM contract, BLOCK-3, POST harness, P1–P14 semantics, P14 assignment scope, drift gate, and residue protocol are unchanged. **Semantic diff = verification-artifact fail-closure only.**

## v8.2-G. RE-RUN EVIDENCE MATRIX (rollback-safe / read-only, zero mutation)

| # | Check | Method | Observed |
|---|---|---|---|
| 1 | corrected §5 resolver | exact read-only fail-closed `DO` | completed with no exception (connector result `[]`); all object/identity assertions passed |
...
| 5 | Focused P3/P4 with new fixture | exact projector created in transaction, terminal RAISE rollback | `V82_P3_P4_PASS \| is_admin=f \| P3=not_authorized \| P4=not_admin` |
...
## v8.2-H. PREDICTED STRUCTURAL DELTA (unchanged from v8.1; see body §11/§19)
...
## v8.2-I. SELF-CONTAINMENT PROOF

An applier holding **only v8.4** can run the complete sequence. §5 now literally validates every frozen object fixture and both identity fixtures before any migration call; it cannot pass on a missing identity merely because `is_admin()` returns false. No manual UID substitution is required or permitted. No v8/v8.1/v8.2/v8.3 file or old chat is needed.

## v8.2-J. APPLY-TIME EXECUTION PROTOCOL

Body §22 governs. The prior Owner authorization is consumed. This package does not request or assume a new authorization. If authorization is independently issued after audit, §5 must emit `B3.3 FIXTURE RESOLVER PASS` immediately before PRE/apply; any missing row, duplicate profile, identity posture mismatch, object mismatch, or exception means STOP with no substitution or patch-forward.

## v8.2-K. HISTORICAL / NON-OPERATIVE PACKAGE-AUTHOR VERDICT

Every required artifact is present and self-contained; migration SQL is byte-identical to v8.1 (SHA `77879867…`); the v8.1→v8.2 delta is verification-only; corrected §5 fail-closes on every object and identity fixture; admin/non-admin JWT posture is empirically true/false; P3/P4 remain `not_authorized`/`not_admin`; production remains pristine.

# HISTORICAL / NON-OPERATIVE — PACKAGE v8.2 READY FOR CTO FINAL VERIFICATION-ARTIFACT AUDIT

**HARD STOP.** No migration applied. No APPLY authorization assumed or requested (prior one-shot consumed & invalid). No D356 canonicalization. No HANDOFF B3.3. No B3.4/B4. PROGRAM architecture unchanged.
**Only next action:** independent exact-artifact audit of corrected §5, verification-only diff, unchanged migration hash, and retained P3/P4 behavior. This package itself makes no APPLY request.

---
---

# ⟦ PACKAGE v8 VERIFICATION BODY — retained; v8.1 fixture UID retained; v8.2 changes only §5 fail-closure and directly related claims ⟧
...
**Apply-time fixture resolver (run before apply; read-only; fail-closed).** Every frozen fixture is asserted literally. Both identities must exist in `auth.users`, have exactly one matching `profiles.user_id` row, and resolve to the frozen authorization posture under authenticated JWT claims. Any missing row, duplicate, mismatch, NULL, or unexpected result raises and aborts. **STOP; do not swap IDs.**

```sql
-- V128-B3.3 v8.2 §5 FIXTURE RESOLVER (read-only; literal; fail-closed)
DO $fixtures$
DECLARE
  v_admin_uid uuid := '446de75d-75b5-476d-8abd-08a98e791f40';
  v_nonadmin_uid uuid := 'eb94304a-8451-44d7-88a7-fe9e26ab0b1c';
  v_count integer;
  v_profile_id uuid;
  v_role text;
  ...
  v_program_name text;
  v_class_school uuid;
  v_session_distribution uuid;
  v_previous_claims text := current_setting('request.jwt.claims', true);
BEGIN
  -- Object fixtures: exact existence, uniqueness by primary key, and frozen correlated values.
  SELECT count(*), min(name) INTO v_count, v_program_name
    FROM public.programs WHERE id='99240fb7-8c82-4869-a522-6e0e863285d3';
  IF v_count IS DISTINCT FROM 1 OR v_program_name IS DISTINCT FROM 'Cảm Thụ Âm Nhạc Dế Mèn' THEN
    RAISE EXCEPTION 'FIXTURE_PROGRAM count=% name=%', v_count, v_program_name; END IF;

  SELECT count(*), (array_agg(school_id))[1] INTO v_count, v_class_school
    FROM public.classes WHERE id='2405fed8-1e9f-4414-b41e-b5eb5f3d8ea7';
  IF v_count IS DISTINCT FROM 1 OR v_class_school IS DISTINCT FROM 'b6a4ac35-2e0a-4667-9eea-756f615c29eb'::uuid THEN
    RAISE EXCEPTION 'FIXTURE_CLASS count=% school_id=%', v_count, v_class_school; END IF;

  SELECT count(*), (array_agg(class_distribution_id))[1] INTO v_count, v_session_distribution
    FROM public.lesson_sessions WHERE id='2fab0c56-9f56-4610-9558-216d58573c20';
  IF v_count IS DISTINCT FROM 1 OR v_session_distribution IS DISTINCT FROM 'a8088a55-b6da-481d-b4c9-e7e9c4d126da'::uuid THEN
    RAISE EXCEPTION 'FIXTURE_SESSION count=% class_distribution_id=%', v_count, v_session_distribution; END IF;

  SELECT count(*) INTO v_count FROM public.schools WHERE id='b6a4ac35-2e0a-4667-9eea-756f615c29eb';
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_SCHOOL count=%', v_count; END IF;
  SELECT count(*) INTO v_count FROM public.profiles WHERE id='e86e45d1-3d0a-4cbc-8d3a-2a07926ec913';
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_PERSON count=%', v_count; END IF;
  SELECT count(*) INTO v_count FROM public.children WHERE id='429d4fb7-67f0-4166-8ec3-fee7ad1a3666';
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_CHILD count=%', v_count; END IF;
  SELECT count(*) INTO v_count FROM public.media_assets WHERE id='614aa02e-fb27-4487-a603-daf26ddfc3d6';
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_MEDIA count=%', v_count; END IF;
  SELECT count(*) INTO v_count FROM public.discovery_capsules WHERE id='384042c1-a1a2-450c-8854-3886659cd050';
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_CAPSULE count=%', v_count; END IF;

  -- Admin identity: auth existence + exactly one profile + authenticated JWT posture.
  SELECT count(*) INTO v_count FROM auth.users WHERE id=v_admin_uid;
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_ADMIN_AUTH count=% uid=%', v_count, v_admin_uid; END IF;
  SELECT count(*), (array_agg(id))[1], min(role::text) INTO v_count, v_profile_id, v_role
    FROM public.profiles WHERE user_id=v_admin_uid;
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_ADMIN_PROFILE count=% uid=%', v_count, v_admin_uid; END IF;
  PERFORM set_config('request.jwt.claims', json_build_object('sub',v_admin_uid::text,'role','authenticated')::text, true);
  IF auth.uid() IS DISTINCT FROM v_admin_uid OR (auth.jwt()->>'role') IS DISTINCT FROM 'authenticated' THEN
    RAISE EXCEPTION 'FIXTURE_ADMIN_JWT uid=% role=%', auth.uid(), auth.jwt()->>'role'; END IF;
  v_admin_result := public.is_admin();
  IF v_admin_result IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'FIXTURE_ADMIN_POSTURE profile=% role=% is_admin=%', v_profile_id, v_role, v_admin_result; END IF;

  -- Non-admin identity: auth existence + exact seeded profile/role + authenticated JWT posture.
  SELECT count(*) INTO v_count FROM auth.users WHERE id=v_nonadmin_uid;
  IF v_count IS DISTINCT FROM 1 THEN RAISE EXCEPTION 'FIXTURE_NONADMIN_AUTH count=% uid=%', v_count, v_nonadmin_uid; END IF;
  SELECT count(*), (array_agg(id))[1], min(role::text) INTO v_count, v_profile_id, v_role
    FROM public.profiles WHERE user_id=v_nonadmin_uid;
  IF v_count IS DISTINCT FROM 1
     OR v_profile_id IS DISTINCT FROM 'd1000000-0000-4000-8000-000000000051'::uuid
     OR v_role IS DISTINCT FROM 'primary_parent' THEN
    RAISE EXCEPTION 'FIXTURE_NONADMIN_PROFILE count=% id=% role=%', v_count, v_profile_id, v_role; END IF;
  PERFORM set_config('request.jwt.claims', json_build_object('sub',v_nonadmin_uid::text,'role','authenticated')::text, true);
  IF auth.uid() IS DISTINCT FROM v_nonadmin_uid OR (auth.jwt()->>'role') IS DISTINCT FROM 'authenticated' THEN
    RAISE EXCEPTION 'FIXTURE_NONADMIN_JWT uid=% role=%', auth.uid(), auth.jwt()->>'role'; END IF;
  v_admin_result := public.is_admin();
  IF v_admin_result IS DISTINCT FROM false THEN
    RAISE EXCEPTION 'FIXTURE_NONADMIN_POSTURE profile=% role=% is_admin=%', v_profile_id, v_role, v_admin_result; END IF;

  PERFORM set_config('request.jwt.claims', COALESCE(NULLIF(v_previous_claims,''),'{}'), true);
  RAISE NOTICE 'B3.3 FIXTURE RESOLVER PASS | admin=true | nonadmin=false | objects=8';
END
$fixtures$;
```
**Live result this session:** exact block completed with no exception (Supabase executor result `[]`; notices are not surfaced by that connector). Therefore every fail-closed assertion preceding the terminal PASS notice held: admin auth/profile/JWT/`is_admin=true`; non-admin auth/profile-id/role/JWT/`is_admin=false`; all frozen object fixtures and correlations exact. No substitution.
...
| §5 fixture resolver | ✅ live | exact fail-closed `DO` completed: 8 object fixtures exact; admin auth/profile/JWT/true; non-admin auth/exact seeded profile+role/JWT/false | read-only |
...
- **Migration provenance (byte-identity):**
  - **CURRENT — v8/v8.1/v8.2/v8.3:** exact §9 fenced body = **14,722 bytes**; SHA-256 `77879867f43de276321443ca87593b4baab56bf3f1a1faa0335e23b63bf2c214`.
  - **HISTORICAL ONLY — v7 before BLOCK-3 NULL-safety correction:** SHA-256 `3938d3c16f38b879fe9c7c72b43ae5e8f04d369da90a1cf926bf6addb5e43463`.
  - BLOCK-1/BLOCK-2 business SQL remains byte-identical to the CTO-audited v6/v5/v4 form; the current whole-migration hash differs solely because v8 corrected BLOCK-3 (§10).
...
A future applier holding **only this file** (no old package, no old chat, no manual paste) can execute the entire flow. The corrected §5 is literal executable SQL and fail-closes on all frozen object fixtures plus: admin/non-admin existence in `auth.users`, exactly one profile per `user_id`, authenticated JWT identity, admin `is_admin=true`, and non-admin exact seeded profile/role plus `is_admin=false`. Thus P3/P4 cannot mask a missing non-admin identity. The frozen golden is hardcoded inside §7/§12; the drift gate (§4), migration (§9), structural verifier, POST harness, P1–P14, P14 assignment probe, and residue checks remain literal. Explicit: **No external file needed. No v8/v8.1 package needed. No old chat needed. No manual paste or runtime fixture substitution permitted.**
...
2. **§5 fail-closed fixture resolver** — exact block must complete with no exception / emit `B3.3 FIXTURE RESOLVER PASS`; all object fixtures and both identity existence/profile/JWT/posture assertions must hold.
...
If any step fails — including any §5 missing row, duplicate profile, profile-id/role mismatch, JWT mismatch, or `is_admin()` posture mismatch — **STOP. Do not substitute a fixture. Do not patch forward. Do not improvise. Report failure.**

---

## 23. PACKAGE-AUTHOR VERDICT

**HISTORICAL / NON-OPERATIVE CONTEXT:** The retained v8 implementation-evidence summary below records how the migration and verification harness reached their current form. It is not a separate package verdict or next-action instruction. The sole operative statements in this section are the v8.4 verdict, HARD STOP, and v8.4 next action that follow it.
...
# PACKAGE v8.4 READY FOR CTO FINAL NARRATIVE-ONLY AUDIT

**HARD STOP.** No migration applied. No APPLY authorization assumed or requested. No production mutation. No migration business-logic change. No D356 canonicalization. No HANDOFF B3.3. No B3.4/B4. PROGRAM architecture unchanged.

**Only next action:** Owner forwards Package v8.4 to ChatGPT/Work for the final narrative-only audit. After — and only after — CTO returns `FINAL IMPLEMENTATION PASS`, Owner may separately issue `AUTHORIZED — APPLY V128-B3.3` as a one-shot production authorization; only then is `apply_migration` (§9) + post-commit verification (§11/§12/§13/§14/§15) executed in the §22 order.
