# DMA V128-B6.1 — LOVABLE IMPLEMENTATION PACKAGE (copy-ready)
...
- ❌ Direct call to `_mission_control_workspace_core` (internal-only; authenticated direct call → `42501`). Clients call the **`get_object_workspace` wrapper** only.
...
- ✅ Malformed / unknown-additive / unsupported-schema envelopes fail closed.
- ✅ UI copy is Vietnamese (DMA UX-writing register); no raw technical strings.

---

## 2. RPC contract (exact — bind names/params from generated `types.ts`)

The adapter exposes **four** operations. Bind each to the exact identifier in
`src/integrations/supabase/types.ts → Database['public']['Functions']`.
**If a name/param below differs from the generated declaration, the generated declaration wins — STOP and report the mismatch to Claude. Do not guess.**

| Operation | RPC (expected) | Params (client-supplied) | Notes |
|---|---|---|---|
| Workspace read | `get_object_workspace` | `p_object_type='class'`, `p_object_id=<uuid>` | Authenticated public wrapper; use the client-facing overload. Client does **not** supply `p_context` here. |
| Actions | `get_mission_control_actions` | object-scoped (`p_object_type`/`p_object_id` per generated sig) | Returns executable actions for the object. |
| Memory | *(exact generated name)* | cursor `p_before` (+ object scope per generated sig) | Name is a B6.0.1 addition — **read it from generated types; do not invent.** |
...
**Execution** parses to a discriminated result:
```
...
### 2.2 Parser rules (`parseMissionControlDtos.ts`)

- Validate UUIDs, ISO timestamps, enums, required keys, and schema version.
- Reject duplicate `context`/`state`/`field` keys.
- **Strip unknown additive fields** so raw metadata cannot reach the UI.
...
- Missing identity in an otherwise `ok:true` workspace is a **contract failure**, not an empty workspace.
...
7. Memory sends the returned `next_cursor` unchanged as `p_before`.
...
│   ├── useMissionControlMemory.ts              CREATE  — infinite memory (backend next_cursor)
...
Create `missionControlDtos.ts`, `missionControlModels.ts`, `parseMissionControlDtos.ts`. Pin `MissionActionInputSchema/v1`. Add safe error normalization (§2.2).
*Accept:* pure TypeScript; **no React and no Supabase imports** in these three files.

**Task 3 — Adapter boundary**
Implement the four operations in `missionControlAdapter.ts` using the generated RPC declarations (§2).
*Accept:* this is the **only** Mission Control file importing `supabase`; names/params bound from generated types.

**Task 4 — React Query layer**
Add `missionControlQueryKeys.ts` + the three read hooks; cursor-based infinite memory (§2.4–2.5).
*Accept:* object-scoped keys; capability gating works; memory uses backend cursor.
...
*Accept:* no fixtures; no raw DTO in any renderer; State is **not** rendered via `HealthBand`; empty state is distinct from projection failure.
...
**Security:** publishable client + user JWT only; no service-role; no `.from()`; no actor/profile ID passed to RPCs; no FE permission/tenant resolver; `school_id` only from parsed workspace context; no `assign_class_distribution`; no `_mission_control_workspace_core`; raw errors never rendered; backend denial stays authoritative.

**Contract:** exact RPC names/params from generated types; `p_request_id` always present; actions only from the Actions RPC; only `MissionActionInputSchema/v1` executes; program/teacher options never synthesized; memory `next_cursor` unchanged; malformed envelopes fail closed; additive unknowns cannot leak; replay is distinct.
...
- [ ] Workspace renders Identity/Context/State/Actions/Memory from production DTOs.
- [ ] All raw RPC responses cross strict runtime parsers.
...
- [ ] Memory pagination works with no duplicates/reorder.
...
- [ ] Build passes; scoped lint passes; static audit finds no forbidden patterns.
