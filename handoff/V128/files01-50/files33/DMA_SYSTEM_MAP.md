...
## ⭐ CURRENT CANONICAL ENDPOINT — D356 / SYSTEM_MAP v1.44 / HANDOFF V128-B3.3 (Mission Control Context Seam — PROGRAM = third context consumer WIRED [platform scope · zero-context · identity-only [name] · IP-safe]; CLASS+SESSION+PROGRAM wired = 8 consumers, code HEAD `be04f4b`, migration tail `20260812190653`, 2026-08-12)
...
## 🧭 V128-B4 — MISSION CONTROL CAPABILITY MODEL — CANONICALIZED v1.45 (2026-08-13)

> **Endpoint:** D357 / SYSTEM_MAP v1.45 / HANDOFF V128-B4. Khối V128-B3.3 (v1.44) phía trên = HISTORICAL SNAPSHOT.

Mission Control separates four independent object capabilities:
...
Mission Control Object
  ...
  |
  +-- Action discovery capability
  |
  +-- Execution capability
```

The Object Registry coordinates capability discovery; it does not collapse these layers into one status implication.
...
Action discovery
  requires
Successful authoritative context validation
```

Caller-supplied context is input for validation only. Authoritative context is resolved from the object itself before action discovery. SESSION remains wired for projection/context integration and intentionally has no action surface.

**Continuity:** D356 → D357 · v1.44 → v1.45 · HANDOFF V128-B3.3 → V128-B4. V128-B5 remains unopened.


---

## 🧭 V128-B5.0.1 — MISSION CONTROL EXECUTION LAYER — CANONICALIZED v1.46 (2026-08-13)

> **Endpoint:** D358 / SYSTEM_MAP v1.46 / HANDOFF V128-B5.0.1. Khối V128-B4 (v1.45) phía trên = HISTORICAL SNAPSHOT.
...
```text
mission_control_action_requests
    =
Execution Memory Layer
```

Execution memory owns request lifecycle, idempotency, and replay protection. It does not replace business audit memory.

**Continuity:** D357 → D358 · v1.45 → v1.46 · HANDOFF V128-B4 → V128-B5.0.1. V128-B5.1 remains unopened.


---

## 🧭 V128-B6.0.1 — MISSION CONTROL PRESENTATION READ CONTRACTS — CANONICALIZED v1.47 (2026-08-13)

> **Endpoint:** D359 / SYSTEM_MAP v1.47 / HANDOFF V128-B6.0.1. Khối V128-B5.0.1 (v1.46) phía trên = HISTORICAL SNAPSHOT.

### FE-safe read topology

```text
Authenticated browser + user JWT
  |
  +-- get_mission_control_workspace(object_type, object_id)
  |     -> object identity + authoritative context + state + capabilities
  |
  +-- get_mission_control_actions(object_type, object_id)
  |     -> permitted AND executable actions
  |     -> MissionActionInputSchema/v1 + backend-projected options
  |
  +-- get_mission_control_memory(object_type, object_id, limit, before)
        -> bounded object audit memory + cursor
```

### Security and ownership

- Three presentation RPCs: `STABLE`, `SECURITY DEFINER`, owner `postgres`, `search_path=""`.
- EXECUTE: `authenticated` + `postgres`; 0 `PUBLIC`, `anon`, `service_role`.
- Functions resolve authenticated identity and domain authorization internally.
- FE does not supply actor authority, resolve school scope, join raw audit tables, or use `service_role`.

### Execution continuity

```text
get_mission_control_actions
  -> exposes class.assign only
  -> execute_mission_control_action
  -> assign_class_distribution
  -> CLASS_ASSIGNMENT_CREATED
```

`class.edit` remains non-executable and absent from the presentation action list. B5 executor/handler fingerprints and behavior remain unchanged.

### Production state

- Project: `xcvhacymrbhdhohyylyq` — Dế Mèn Art — PostgreSQL 17.6.
- Migrations: `20260813113248` presentation read contracts; `20260813113400` workspace state projection forward-fix.
- Inventory: **92 / 248 / 236 / 169 / 33 / 1** (tables / functions / SECURITY DEFINER / policies / user triggers / cron).
- Generated authority: `src/integrations/supabase/types.ts` exact production output, **186,270 B**, SHA-256 `635eec14a708208c68b8d6cbff4803f9140e557d77e862116fb9ae7056a64dee`.

**Continuity:** D358 → D359 · v1.46 → v1.47 · HANDOFF V128-B5.0.1 → V128-B6.0.1. **V128-B6.1 — Class Workspace Vertical Slice FE Implementation is NEXT; FE coding remains unopened in this closeout.**
