...
## ⭐ CURRENT CANONICAL ENDPOINT — D352 / SYSTEM_MAP v1.40 / HANDOFF V128-B3.0 (Mission Control Context Seam Foundation — Variant A three-tier adapter + context_requirements jsonb + structural validator seam; foundation-only, 0 object wired, code HEAD `be04f4b`, migration tail `20260811111257`, 2026-08-11)
...
## 🧩 V128-B3.0 CONTEXT SEAM FOUNDATION — SYSTEM_MAP v1.40 (2026-08-11)

**Endpoint:** RULES **D352** · SYSTEM_MAP **v1.40** · HANDOFF **V128-B3.0** · code HEAD **`be04f4b`** (BẤT BIẾN, backend-only) · migration tail **`20260811111257`**. Inventory **90 tables · 238 functions · 227 SECURITY DEFINER · 166 policies · 33 triggers · 1 cron**.

**Mission Control Context Contract Layer (mới):** three-tier Variant A trên chokepoint B2.x —
- `get_object_workspace(text,uuid,text)` LEGACY 3-arg (thin wrapper) + `get_object_workspace(text,uuid,jsonb,text)` NEW 4-arg (thin wrapper, `p_context` REQUIRED no-default) → **một** `_mission_control_workspace_core(text,uuid,jsonb,text)` (internal-only ACL `{postgres,service_role}`).
- Core canonical order (FREEZE): `authenticate → registry metadata → validate context → auth-slot(no-op) → privacy/reason → touch/projector/allowlist/leak/DTO`. DTO `WorkspaceProjectionDTO/v1` **không bump**.
- `validate_mission_control_object_context(text,jsonb)` = structural seam (ACL `{postgres,service_role}` 0 auth/anon/PUBLIC — v2.1 C-1 enumeration-oracle closed). Đọc `registry.context_requirements` only; verdict `{valid,errors[],normalized_context}`; core map → `context_invalid`.
- Registry column mới **`context_requirements jsonb`** + CHECK `mc_context_requirements_shape_chk` (declarative 3-key, minus-chain reject extras). 17 rows = default; **KHÔNG object nào set ≠ default ở B3.0** (foundation-only).

**Registry state (v1.40):** **5 wired** (person/child/media/capsule = platform · school = tenant) · **6 registered** (subscription/support_case/program/privacy_request platform · class tenant · session assignment) · **6 forbidden**. class + session **vẫn registered** (context two-id contract = B3.1 auth-slot, chưa wire). `context_requirements` = default toàn bộ. FE HEAD `be04f4b` BẤT BIẾN.

**MC public surface (v1.40):** đúng 2 overload `get_object_workspace/3` + `/4` (ACL `{authenticated,postgres,service_role}` 0 anon/PUBLIC). Core + validator = internal helper `{postgres,service_role}` (không reachable qua PostgREST). 5 projector + logger `admin_workspace_access_log` KHÔNG đụng.

**Rollback toàn B3.0:** DROP `get_object_workspace/4` + DROP `_mission_control_workspace_core` + DROP `validate_mission_control_object_context` + restore `get_object_workspace/3` về B2.2 monolith + DROP CONSTRAINT `mc_context_requirements_shape_chk` + DROP COLUMN `context_requirements` + NOTIFY pgrst → về tip B2.2 (tail `20260811080037`). **Endpoint: RULES D352 · SYSTEM_MAP v1.40 · HANDOFF V128-B3.0 · code HEAD `be04f4b` · migration tail `20260811111257`.**
